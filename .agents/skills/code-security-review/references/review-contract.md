...
Use pass only when reviewed_sha is a full SHA, the checkout is clean, required evidence is present, coverage gaps do not hide a blocking surface, and no validated critical or high finding remains. Medium and low findings remain visible and follow the project's declared blocking policy. If no policy exists, do not silently suppress them; report them and state the decision rationale.
