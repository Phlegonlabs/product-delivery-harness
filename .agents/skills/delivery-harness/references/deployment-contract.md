...
Use this reference when a delivery will be deployed, when verifying a deployed environment, or when moving a project between deploy platforms. The Harness RUN still ends locally or at its own run-branch push. `branch-promotion-contract.md` then controls the separately authorized `development`-to-`main` promotion; this contract maps those protected branches to environments and verifies what the platform serves.

## Model

- Production tracks `main`; the production pipeline deploys only the exact SHA promoted there after development verification.
- The persistent internal environment tracks `development`. Other run branches may receive disposable previews, but only the exact remote `development` head supplies promotion evidence for `main`.
- A git-connected or CI-connected platform maps `development` to the isolated non-production environment and `main` to production. The bootstrap deploy remains a separately authorized owner action when required; branch promotion adds no RUN authorization keys and never reuses the run-branch grant.
...
- Adding a binding follows one order: create the non-production resource, declare it, promote the exact candidate to `development`, and verify the binding on the development URL. Only then create the production resource, include its declaration in the same candidate lineage, repeat development verification if the SHA changes, and promote that exact verified SHA to `main`. Production is verified separately. Missing resources, shared production IDs, or schema-order failures block promotion. Preview secrets stay fake or dedicated; production values never enter tracked config. D1 migrations run against development first and are coordinated with the `main` promotion so code never precedes its production schema. The project instance lives in `docs/DEPLOYMENT.md`; platform commands vary, but development-first order does not.
- Cloudflare Pages, git-connected: `development` supplies the internal preview deployment, while `main` is the configured production branch at `<project>.pages.dev`. Other run branches may receive disposable previews, but they do not satisfy development verification.
- Cloudflare Workers, CI-connected: deploy `main` with `wrangler deploy` and deploy only `development` to the named non-production environment with `wrangler deploy --env development` (or `wrangler versions upload --env development`). A run branch may receive a disposable stateless preview, but it never substitutes for the exact development-head test. A version Preview URL of the production Worker shares live bindings and is not an isolated development environment. Production promotion is a separate exact-SHA `main` update, never a side effect of another branch push.
- Cloudflare Pages, CI-connected (Wrangler bootstrap): create the project with `wrangler pages project create <name> --production-branch main`, run the first deploy yourself, then map `development` to the internal preview and `main` to production in repository CI. Other branches may build disposable previews. Choose this mode at creation time — a Wrangler-created Direct Upload project cannot later become git-connected.
...
- Git-connected: `development` supplies the internally verified preview, `main` supplies production, and other run branches may build disposable previews that do not satisfy the development gate.
...
- Amplify, git-connected: map `development` to the internal non-production environment and `main` to production; optional run-branch previews stay separate from the development gate.
...
- Assume branch-build semantics when the platform supports them: `development` for internal verification, `main` for production, and optional disposable run-branch previews. Otherwise record `mode: manual` with the separately authorized deploy command and read-only check.
- Record what is known; never invent a platform capability the project has not configured.

## Post-Deploy Verification (read-only)

After each separately authorized branch promotion, verify the matching environment as a read-only gate:

1. The expected environment URL resolves — the development URL for the exact `development` head, then the production URL for the promoted `main` head.
...
5. Report the observed development or production URL with its exact SHA and every remaining human action. When the build has not finished or the URL cannot be observed, leave the record pending; never construct or guess a URL.
...
Two records carry the deployment model. `docs/DEPLOYMENT.md`, seeded during PRD creation and reconciled before promotion, is the detailed instance: branch roles, platform record, name-only configuration inventory, external-console tasks, internal-test evidence, and environment status. The seeded `AGENTS.md` summarizes the same `development`-to-`main` governance. Keep both filled from live evidence; an unfilled record means the model is unknown. `docs/ACTIVATION.md` remains separate.

## Moving Between Platforms

Migrating — Cloudflare to AWS or Vercel, or anywhere else — changes the record and platform section, not the flow: map `development` to isolated non-production and `main` to production, update the deployment record, and re-verify both exact branch heads. No promotion or authorization rule changes with the platform.
