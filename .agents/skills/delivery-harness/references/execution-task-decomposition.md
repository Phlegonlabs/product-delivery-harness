...
- Loading `frontend-design` does not change that route-task boundary. In frontend-design conformance mode it executes the frozen PRD UI surface contract and active visual source with greater visual care; any proposed addition returns as a design-input delta instead of joining the route commit.
