...
The portable default favors an isolated worktree per mission because it enables exact-head review before controlled integration into the run branch. `development` and `main` remain outside ordinary worktree execution. After RUN close, the exact candidate is separately promoted to `development`, tested on that remote head, then fast-forwarded unchanged to `main` under another authorization. Read-only work may fan out freely; parallel writes still require complete resource claims, observable completion, a fixed base, and explicit action authorization.
