...
- Every host loads the Harness skills from the user skills directory, so the shared copy there is the only Harness update surface. Use the recoverable backup-and-copy sequence above, verify that only the four current IDs remain discoverable, then start a fresh session.
