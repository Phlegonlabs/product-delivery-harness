...
Every UI route gets structure, behavior, and its exact platform-appropriate responsive set from its `PRD.md` `UI-*` entry. Approved `wireframes.html` provides the all-page, section-label, complete responsive-target, state, and browser layout-QA review projection; it helps implementation inspect the contract but never overrides `PRD.md`. Visual authority comes from exactly one mode selected by `PRD.md`'s Design System Need Gate:
...
1. Read `PRD.md`, its approved UI Design Handoff, the matching approved page in `wireframes.html`, and the active visual source before writing. Resolve each route through its `UI-*` entry's invariant `` `route`: `` field and its responsive coverage through the invariant `` `responsive`: `` field; one entry owns exactly one literal route and the package uses one exact responsive set. A missing route claim, duplicate claim, missing or mismatched responsive set, missing or mismatched approved wireframe artifact, missing target scope or browser layout evidence, or inconsistent Design System Need Gate is a blocker. A route whose entry records SEO metadata implements exactly that `<title>`, meta description, and recorded extras — the rendered `<head>` is part of the deliverable. A route with no SEO metadata record is a PRD contract gap routed to `product-definition-builder`, never an implementation-time invention.
...
6. When implementation reveals genuine cross-surface token, closed-variant, multi-theme, multi-platform, or automated-conformance needs, stop and return a Design System Need Gate delta to `product-definition-builder`; do not grow an undeclared design system inside the code mission.
