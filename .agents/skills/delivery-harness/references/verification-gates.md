...
- Every new managed code-delivery PLAN has a completed `security` review from the `code_security_verification` binding, recorded as PASS on the exact current integration head. A missing, skipped, stale, partial, or blocked security review prevents closeout.
