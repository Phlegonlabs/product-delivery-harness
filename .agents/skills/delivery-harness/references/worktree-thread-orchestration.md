...
The selector does not emit a batch base, an effective budget, or a bundled wave. The parent binds the committed `batch_base_sha` (the observed parent head), confirms the budget it applied, records the accepted wave in `RUN.md`, assigns leases, and only then creates branches, worktrees, subagents, or app tasks — through the scripted write path, under this session's run lock: `harness_transition.py record-observation` (live-Git observed snapshot), `accept-wave` (reruns selection and accepts exactly its current dispatchable mission frontier), `lease-worker` (graph/mission/task/worker/attempt in one serialized validated write; refuses paused runs and conflicting concurrent missions), `record-integration` (mission integrated plus the new integration head, proven against the live branch, clean product tree, worker head, prior integration head, and batch base), and `close-wave` (tombstone appended, wave-bounded grants reset). The clean-product-tree check ignores only the exact tracked RUN file that these transitions necessarily update; other coordination, product, source, evidence, and untracked changes still block. A selected `worker_passed` mission may close under a surviving `run_complete` execution grant and integrate afterward; a `wave_closed` execution grant must first resolve every selected mission by integration, failure, or reconciliation because that grant expires at close. Hand-editing the RUN JSON for these steps is the path the transitions replaced. Selector and validator scripts do not mutate Git, Codex, PLAN, or RUN; `harness_transition.py` is the sole scripted RUN writer.
