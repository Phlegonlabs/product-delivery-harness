...
def _retained_execution(
    ...
    return {
        ...
        **(
            {"reservation": copy.deepcopy(retained["reservation"])}
            if isinstance(retained.get("reservation"), dict)
            else {}
        ),
        **(
            {
                "dispatch_attestation": copy.deepcopy(
                    retained["dispatch_attestation"]
                )
            }
...
def _local_node_verifier_declaration(
    plan: dict[str, Any], node: dict[str, Any]
) -> tuple[str, str | None, str | None, dict[str, Any]]:
    """Resolve the declaration and owner layer for a local graph verifier."""

    verifier_id = node.get("ref")
    if not isinstance(verifier_id, str) or not verifier_id:
        raise ManifestError("local verifier node must reference a verifier declaration")
    owner = _verifier_owners(plan).get(verifier_id)
    if owner is None:
        raise ManifestError(
            f"local verifier node references unknown verifier {verifier_id!r}"
        )
    return owner


def record_local_node_verifier(
    plan: dict[str, Any],
    run: dict[str, Any],
    node: dict[str, Any],
    *,
    ...
    """Retain local verifier execution evidence and project gate status.

    Local graph verifier nodes are parent-owned commands.  Their result files
    are generated by ``verifier_runtime.py`` and are retained verbatim after
    the normal manifest validator checks the normalized execution.  Batch and
    final gate arrays remain a projection of that retained evidence; they are
    never accepted as a free-form status update.
    """
    ...
    if node.get("kind") != "verifier" or node.get("executor") not in {
        ...
        raise ManifestError(
            "record_local_node_verifier requires a non-runtime verifier node"
        )
    layer, mission_id, task_id, declaration = _local_node_verifier_declaration(plan, node)
    if layer not in {"batch", "final"}:
        # Mission/task/worker verifier results belong to mission worker
        # handoffs.  Keeping this guard prevents a generic graph attempt from
        # laundering lease-bound evidence into the wrong owner layer.
        raise ManifestError(
            f"local graph verifier {node.get('id')!r} must reference a batch or final verifier"
        )
    expected_head = run.get("integration", {}).get("integration_head_sha")
    if not isinstance(expected_head, str) or not is_full_sha(expected_head):
        raise ManifestError(
            "local batch/final verifier requires a recorded integration head SHA"
        )
    interrupted_without_result = (
        not retained_results and outcome in {"blocked", "contract_gap"}
    )
    if len(retained_results) != 1 and not interrupted_without_result:
        raise ManifestError(
            "record-node-result requires exactly one --verifier-result unless an interrupted local verifier is recorded blocked"
        )
    ...
    for retained in retained_results:
        if not isinstance(retained, dict):
            raise ManifestError("each retained verifier result must be an object")
        if retained.get("verifier_id") != declaration.get("id"):
            raise ManifestError(
                "retained verifier result must match the local graph verifier declaration"
            )
        context = retained.get("context")
        if not isinstance(context, dict):
            raise ManifestError("retained verifier context must be an object")
        # Batch/final gate evidence is intentionally unbound to a worker or
        # lease.  The graph attempt itself is retained in attempt_log; the
        # execution record keeps the canonical gate shape expected by RUN-v11.
        expected_context = {
            "layer": layer,
            "mission_id": mission_id,
            "task_id": task_id,
        ...
        for key, value in expected_context.items():
            if context.get(key) != value:
                raise ManifestError(
                    f"retained verifier context.{key} must match {value!r}"
                )
        execution_key = retained.get("execution_key")
        identity = (retained.get("verifier_id"), execution_key)
        if not isinstance(execution_key, str) or not execution_key:
            raise ManifestError("retained verifier identity is incomplete")
        if identity in seen:
            raise ManifestError(
                f"duplicate retained verifier result for {declaration.get('id')!r}"
            )
        seen.add(identity)
        # Identical verifier inputs intentionally produce the same execution
        # key. A retry on the same head must retain both the failed and passing
        # observations; `_execution_id` gives each history row a unique ID.
        # Test fixtures and older parent handoffs may omit captured streams for
        # a gate whose key document already attests an empty stream.  Retain
        # the canonical empty digests rather than rejecting an otherwise valid
        # local gate receipt.
        normalized_input = dict(retained)
        normalized_input.setdefault("stdout", "")
        normalized_input.setdefault("stderr", "")
        record = _retained_execution(run, normalized_input, [])
        normalized.append(record)

    if normalized:
        execution = normalized[0]
        execution_passes = (
            execution.get("status") == "PASS" and execution.get("exit_code") == 0
        )
        if outcome == "pass" and not execution_passes:
            raise ManifestError(
                "a passing local verifier requires an exact PASS/exit-0 retained execution"
            )
        if outcome != "pass" and execution_passes:
            raise ManifestError(
                "a non-pass local verifier outcome requires a FAIL, TIMEOUT, or ERROR execution"
            )

    run.setdefault("verifier_executions", []).extend(normalized)

    # Only batch/final arrays are projections here.  Keep every array entry and
    # replace the matching verifier's status atomically in the parent transition.
    run_key = "batch_gate_results" if layer == "batch" else "final_gate_results"
    gate_status = {
        "pass": "PASS",
        "fix_required": "FAIL",
    ...
    gates = run.get(run_key)
    if isinstance(gates, list):
        gate = next(
            (item for item in gates if isinstance(item, dict) and item.get("id") == declaration.get("id")),
            None,
        )
        if gate is not None:
            gate["status"] = gate_status
            gate["head_sha"] = expected_head
            gate["evidence"] = [
                item["evidence_key"]
