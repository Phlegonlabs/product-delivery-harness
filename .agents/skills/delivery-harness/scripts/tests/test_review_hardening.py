...
class ReviewHardeningTests(unittest.TestCase):
    ...
    def test_only_retryable_or_reconciled_missions_accept_a_new_lease(self) -> None:
        for phase, outcome in (
            ("worker_failed", "retryable_failure"),
            ("blocked", "blocked"),
        ):
    ...
    def test_contract_gap_mission_cannot_be_released_without_refinement(self) -> None:
        mission = self.run["mission_states"]["M1"]
        mission["phase"] = "blocked"
        mission["blockers"] = ["PLAN revision required"]
        node = self.run["graph_state"]["node_states"]["N-M1"]
        node.update(
            {
                "phase": "blocked",
                "last_outcome": "contract_gap",
                "blockers": ["PLAN revision required"],
            }
        )

        with self.assertRaisesRegex(ManifestError, "cannot be re-leased"):
            harness_transition._lease_worker(
                self.plan, self.run, Namespace(**LEASE_ARGS)
            )
