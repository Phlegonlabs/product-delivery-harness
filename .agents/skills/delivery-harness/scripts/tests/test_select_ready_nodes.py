...
def retain_current_worker_state(
    ...
    *,
    ...
    run["observed"]["git"]["worktrees"] = [
        {
            "path": run["observed"]["git"]["parent_worktree_path"],
            "branch_ref": run["observed"]["git"]["parent_branch"],
            "head_sha": run["observed"]["git"]["parent_head_sha"],
            "managed_by": "parent",
            "dirty": False,
        },
        {
...
class SelectReadyNodesTests(unittest.TestCase):
    ...
    def test_contract_gap_blocker_is_not_rearmed_as_a_worker_retry(self) -> None:
        plan, run = self._authorized_conflict_free_pair()
        mission_state = run["mission_states"]["M1"]
        mission_state["phase"] = "blocked"
        mission_state["blockers"] = ["PLAN revision required"]
        node_state = run["graph_state"]["node_states"]["N-M1"]
        node_state["phase"] = "blocked"
        node_state["last_outcome"] = "contract_gap"
        node_state["last_attempt_id"] = "A-CONTRACT-GAP"
        node_state["attempts"] = 1
        node_state["blockers"] = ["PLAN revision required"]
        ...
        self.assertIn("node_phase_not_ready", deferred["N-M1"])
        self.assertIn("blocker_present", deferred["N-M1"])
