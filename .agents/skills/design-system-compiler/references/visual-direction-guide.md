...
For an explicit direction-reopen request, `design-system-compiler` must load `impeccable`, `frontend-design`, and the design method selected by the PRD UI Design Handoff. Load `design-taste-frontend` only where its recorded applicability allows it. If a required skill is unavailable, stop; do not use a fallback design path. Candidate HTML, generated images, Figma frames, and other previews are non-canonical until the human owner approves one and `product-definition-builder` records it in the UI Design Handoff. They may explore visual treatment but must not add product scope or silently change the canonical PRD or wireframe contract.
