...
- When the owner declares the project or initiative complete, do not leave the finished state waiting for a future plan to trigger the move: once the Closeout Bar (`verification-gates.md`) is confirmed, archive the completed `PLAN.md` and `RUN.md` plus their `docs/goal/evidence/` directory into `docs/goal/archived/<YYYYMMDD-HHMMSS>-<initiative-slug>/` immediately. List every exact move first and proceed only when the completion instruction already authorizes those exact targets or the owner explicitly approves the list. Never move anything under `docs/product/` in this step — the current product sources stay published for future `prd-builder` enhancement runs.
...
| Approved wireframe | <wireframes.html path> | <hash or revision> | human decision owner | draft / approved / delta_accepted / revision staged | <UI-* screens, labeled regions, states, responsive rearrangement> |
...
| Approved UI target | <path or immutable version> | <SHA-256 or revision> | human decision owner | approved / provisional / n/a | <routes, states, responsive scope, tolerance, allowed deviations> |
...
- When the gate is `not_required`, the approved UI target row must carry an immutable path or version, SHA-256, routes, states, responsive scope, tolerance, and allowed deviations. Freeze it with the PRD and wireframes. No design-system pair is expected, and its absence is not `missing` or `partial`. When the target is high-fidelity HTML, freeze every approved HTML reference file's SHA-256 in that row, not just the folder path.
...
- If a PRD UI surface omits its states, resolve the gap before implementation or mark the surface `UNVALIDATED`. The required responsive set comes from `design-system.json` in system-conformance mode, or from the PRD UI target record in target-conformance mode. The harness carries no default set.
...
- A `DS-*` ID names an entry that exists in `design-system.json` when the Design System Need Gate is `required`. Resolve every `DS-*` trace against that file. A `not_required` route does not mint placeholder `DS-*` IDs; its visual acceptance traces to the approved UI target and the applicable `UI-*`, `UX-*`, and `TEST-*` rows.
- The PLAN's `ui_surfaces` and the PRD's `UI-*` surface contract name the same set. `scripts/validate_harness_plan.py --prd <path to PRD.md>` proves the join at validation time, and `prd-builder`'s wireframe checker carries the same join from `PRD.md` to `wireframes.html` — a surface that exists in only one artifact is drift, not a deliberate omission.
...
- A mission's `required_skills` names `product-design-builder` without `frontend-design`, a design-system-source write scope omits that pair, or a wireframe-source write scope omits `prd-builder`.
