...
- Closeout runs `scripts/validate_harness_plan.py --design-system <path to design-system.json>` whenever the Design System Need Gate is `required`, so its `stateMatrix` and responsive set are cross-checked against the PLAN's UI surfaces. In target-conformance mode, required screenshot coverage comes from the frozen PRD target scope and PLAN UI evidence rows; absence of a design-system argument is expected, not a skipped gate.
...
- Closeout runs `scripts/validate_harness_plan.py --prd <path to PRD.md>` whenever the PLAN declares `ui_surfaces`. A PLAN that freezes `docs/product/PRD.md` as a contract source must pass `--prd`, and the given file's bytes must match the frozen source hash, so the join cannot be omitted or redirected at a decoy document; the PLAN's surface IDs and the PRD's `UI-*` surface contract are proven to name the same set. Passing `--wireframes <path to wireframes.html>` deepens the join past the ID set: each surface's route and states must agree with its wireframe screen.
