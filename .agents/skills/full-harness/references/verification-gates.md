...
- Closeout runs `scripts/validate_harness_plan.py --prd <path to PRD.md>` whenever the PLAN declares `ui_surfaces`, so the PLAN's surface IDs and the PRD's `UI-*` surface contract are proven to name the same set; a PLAN that invents or drops a surface against its own frozen PRD source fails closeout.
