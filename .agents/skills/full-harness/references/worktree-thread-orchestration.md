...
The selector does not emit a batch base, an effective budget, or a bundled wave. The parent binds the committed `batch_base_sha` (the observed parent head), confirms the budget it applied, records the accepted wave in `RUN.md`, assigns leases, and only then creates branches, worktrees, subagents, or app tasks — through the scripted write path, under this session's run lock: `harness_transition.py record-observation` (live-Git observed snapshot), `accept-wave` (wave plus batch base), `lease-worker` (graph/mission/task/worker/attempt in one atomic validated write; refuses conflicting concurrent missions), `record-integration` (mission integrated plus the new integration head, proven against the live branch, tree, worker head, and batch base), and `close-wave` (tombstone appended, wave-bounded grants reset) once every selected mission is integrated, failed, or reconciled. Hand-editing the RUN JSON for these steps is the path the transitions replaced. Scripts must not parse human Markdown tables or mutate Git, Codex, PLAN, or RUN.
