...
6. Observe the actual Git common directory, path, branch/ref, base, head, changed files, and commit ancestry. If the accepted nested policy is enabled after its exact `worker:<id>` authorization, the task must launch its task-local reviewer child and bind a completed exact-head PASS result to the final worktree head; other helper lanes remain optional. If the policy is omitted or disabled, run no helper and route review to the parent/review graph. Regardless of nested-helper policy, the parent/review graph must produce and record the terminal exact-head review PASS before integration.
...
Nested helpers are optional at the policy level. An omitted or disabled `nested_subagent_policy` runs with no helper and routes review to the parent/review graph. If the parent explicitly enables the policy after verifying the separate exact `spawn_subagents` grant for `worker:<id>`, the task must launch its task-local reviewer child and return a completed exact-head PASS; other helper lanes remain optional. The selector's outer `required_actions` does not grow to include this optional action. A single mechanical edit may remain trivial, and any app task may continue without nested helpers when the policy is omitted, disabled, or cannot be enabled because capability or authorization is unavailable.
...
The exact-head graph review is mandatory regardless of nested-helper policy. Before integration, every mission needs a terminal `review_workers[]` PASS bound to its current worktree SHA. An omitted or disabled policy means no helper, so the parent/review runtime performs that graph review. An enabled policy with an unavailable or partial helper, or without a completed task-local exact-head PASS reviewer, blocks the mission; the parent/review graph is not a fallback for that enabled-policy obligation. The terminal graph review remains mandatory in every case; absence of a nested helper never lowers the gate. A later repair commit invalidates every earlier review and requires a fresh exact-head review.
