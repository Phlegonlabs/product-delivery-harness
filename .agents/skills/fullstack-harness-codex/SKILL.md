...
Use only the action keys in each accepted `dispatchable_nodes[].required_actions`, rechecking their exact targets before launch. Do not infer `spawn_subagents` from the selected host or from an observed nested capability: an app-task route normally needs `create_user_owned_tasks`, while a direct-subagent route needs `spawn_subagents`; an enabled task-local nested policy needs `spawn_subagents` only when the task actually launches a child. Worktree, branch, commit, and integration actions remain separate. When the user requests independent Codex tasks or worktrees, preserve that outer topology: create one top-level Codex task per selected mission/worktree before any task-local Multi-agent work. Never collapse those missions into direct subagents of the coordinator. `../fullstack-harness-engineering/references/worktree-thread-orchestration.md` states the topology rules this procedure has to preserve.
...
Nested helpers are optional. A mission may use them only when its accepted `nested_subagent_policy` is enabled, the current task exposes the child runtime, and the selected node's `required_actions` includes `spawn_subagents`; capability discovery alone never turns delegation on. A single mechanical edit may remain trivial, and any app task may continue without nested helpers when the policy is disabled or unavailable.
