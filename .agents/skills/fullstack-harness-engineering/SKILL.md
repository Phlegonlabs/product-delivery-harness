...
The normal UI handoff is `prd-builder` low-fidelity wireframes -> optional product-specific Ask User preference discovery through `frontend-design` -> exactly two or three materially different HTML directions for the same one or two representative screens -> explicit human approval of consolidated selected HTML -> `ui-architecture-builder` extraction and freeze -> Harness implementation. Wireframes remain the product authority for structure and flow. Candidate and selected HTML are non-canonical evidence; implementation begins only after the approved result has been extracted into frozen tokens, primitive contracts, registry entries, page recipes, mockups, and acceptance criteria.
...
6. Stops and reports when the route needs a primitive, variant, motion variant, or component that `ui-registry.json` does not list. A missing entry is a contract change, not a task-local fix: it goes back to the design source as a delta, gets frozen, and returns through `references/design-input-updates.md`. An implementation mission never writes `ui-registry.json` or `mockups/catalog.html` — those are frozen contract sources, so editing them mid-task invalidates the PLAN digest and every in-flight lease, including its own.
...
8. Runs the visual check across the responsive verification set the package's `ui-registry.json` carries — its `viewports` for a web target or its `sizeClasses` for a native or desktop target, in normal and reduced motion, and records the evidence per `references/verification-gates.md`. Read the set from the registry; do not carry a default set in this skill, and never substitute web pixel breakpoints for a native platform's own model.
