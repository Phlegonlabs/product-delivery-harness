...
- Record that separation in the ledger rather than leaving it to prose. A RUN-v10 `push`, `merge_pr`, or `deploy` entry carrying a target the execution-intent instruction does not reach must name the separate instruction for that exact target in `target_sources` (`references/execution-state-model.md`), and that source must differ from the entry's own. The validator fails the RUN otherwise, in every landing mode — one entry with one `source` cannot otherwise distinguish a development push from a push at the protected branch.
