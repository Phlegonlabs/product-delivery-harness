...
3. Use three as the configured plan-backed write-worker maximum as a conservative starting default, not a hard ceiling. The parent may set `max_parallel_workers` above three only when observed worker slots, workspace isolation capacity, and a genuinely large dependency-ready conflict-free frontier all support a larger wave; the same observed-capacity mechanism in `references/parallel-mission-selection.md`'s effective-budget formula (the `min(...)` of configured maximum, observed worker slots, isolation capacity, and conflict capacity) still bounds the effective wave and can only shrink it below the configured value. The user or observed capacity may also set a lower limit.
