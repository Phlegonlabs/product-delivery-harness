...
In the target repository, start every mission worktree from the current resolved integration SHA. Each worktree completes one exact-head read-only review before parent integration; repairs require a fresh review. Merge passing worktrees only into that integration branch. If the repository defines no other model, cut the run's own `codex/<short-name>` branch from the current `main` and use it for integration; never edit or commit on `main`, and never push to it.
