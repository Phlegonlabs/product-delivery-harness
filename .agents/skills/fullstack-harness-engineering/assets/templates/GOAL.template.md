...
For a large route with no usable agent capability, use `worker_runtime: parent`, `selected driver: sequential_parent`, `completion_channel: agent_result`, and preferably `workspace_mode: parent_managed_worktree` (shared checkout is an explicit one-writer fallback). The parent executes one mission at a time under the same PLAN/RUN graph and exact-head review gates. Do not request `spawn_subagents`, create a worker identity, or describe this parent loop as delegated execution.
