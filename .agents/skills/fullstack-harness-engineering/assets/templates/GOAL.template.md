...
For ordinary mission work, request only the exact local branch, commit, review, and integration actions needed to land reviewed worktrees into `development`; do not request or infer a future production promotion. After the user separately reviews the accumulated development result and explicitly approves starting `development -> production`, request the remaining `push`, `create_pr`, `manage_pr_review`, and `merge_pr` actions with exact scope. Use `future-pr:<owner>/<repo>:base=production:head=development` until the PR exists, then verify that binding and append `pr:<full-PR-URL>` before review or merge. Once that late checkpoint authorizes every remaining action, finish current-head CI and Codex review and use exact-head squash auto-merge without asking again between those already approved stages. Keep `configure_repository` separate.
...
In the target repository, start every mission worktree from the current `development` SHA. Each worktree completes one exact-head read-only review before parent integration; repairs require a fresh review. Merge passing worktrees only into `development`. Never start ordinary PRD/PLD, UI, or feature work from `production`; move `development` to `production` only after the user's separate final approval.
...
After final user approval starts an authorized `development -> production` promotion, do not stop after PR creation. Start or observe CI and Codex review concurrently for the exact development head, fix authorized in-scope findings, invalidate stale evidence after every push, restart both remote gates, and enable exact-head squash auto-merge only after every current-head gate passes. Then deploy the resulting production head and run production smoke. Ordinary development work stops before this paragraph.
