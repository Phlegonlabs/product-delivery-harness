...
`required_skills` names every installed skill (by its `name:` frontmatter, e.g. `frontend-design`) that mission's worker must load before implementing, beyond this harness core itself. Use `[]` when the mission needs no additional skill. Record the planner's explicit choice here; do not have a worker infer an implementation skill from a frontend code glob. Every launch path (`WORKER_GOAL.template.md`, a Codex app-task prompt, or a Claude Dynamic Workflow agent prompt) must carry this list verbatim so a spawned worker actually learns to load it — see `references/worktree-thread-orchestration.md`'s Worker Handoff. A design-source creation or revision mission must use `"required_skills": ["product-design-builder", "frontend-design"]`; the manifest rejects a missing member when the mission claims any canonical design-source path, even if both skills were omitted from the authored list.
...
Design-source creation uses design creation mode. The paired skills may create and revise the canonical `wireframes.md`, `design-system.md`, and `design-system.json` inside the mission's exact scope, but only from frozen product inputs and through `product-design-builder`'s human direction gates. Missing `frontend-design` blocks the mission; no fallback is allowed.
