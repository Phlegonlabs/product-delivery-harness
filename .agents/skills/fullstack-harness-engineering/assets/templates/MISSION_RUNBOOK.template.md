...
When a Claude Code parent binds a graph node to cc-codex, run `CLAUDE_CODEX_PREFLIGHT.template.js` only after `invoke_external_runtime` covers `runtime:codex`. A write node uses `external_codex_agent`, `subagent`, `app_managed_worktree`, and `agent_result`, and also requires `spawn_subagents`, `create_app_managed_worktrees`, `create_local_branches`, and `create_local_commits`. It does not use `create_user_owned_tasks`. Allocate the node attempt and worker lease before running `CLAUDE_CODEX_GRAPH_WORKFLOW.template.js`. The worker's `worktree_path` and `branch_ref` may remain null while the runtime allocation is pending, after wildcard pre-allocation authorization has been checked. Once the marked result returns, independently verify its Git common directory, path, branch, head, ancestry, diff, scope, and verifier evidence before recording those exact values in RUN. Keep failed or cancelled runtime worktrees for diagnosis; do not reset or delete them automatically.
