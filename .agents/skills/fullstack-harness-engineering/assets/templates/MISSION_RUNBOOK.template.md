...
When a Claude Code parent binds a graph node to cc-codex, run `CLAUDE_CODEX_PREFLIGHT.template.js` only after exact `runtime:codex` and `worker:preallocation` authorization. A write node uses `external_codex_agent`, `subagent`, `app_managed_worktree`, and `agent_result`, and also requires exact worker, worktree, and branch authorization for `spawn_subagents`, `create_app_managed_worktrees`, `create_local_branches`, and `create_local_commits`. It does not use `create_user_owned_tasks`. Allocate the node attempt, worker lease, worktree path, and branch ref, then run `scripts/validate_codex_wave.py` to emit canonical Workflow arguments before `CLAUDE_CODEX_GRAPH_WORKFLOW.template.js`. External Codex reviews are disabled in v1; select another allowed provider or defer. Once the marked result returns, independently verify its Git common directory, path, branch, head, ancestry, diff, scope, and verifier evidence before recording the result in RUN. Keep failed or cancelled runtime worktrees for diagnosis; do not reset or delete them automatically.
