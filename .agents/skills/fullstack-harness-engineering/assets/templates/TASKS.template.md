...
Use this template as `docs/goal/tasks.md` on demand when a human-readable mission/task view is useful (see the Project Size Gate and File Budget in `SKILL.md`). It is optional and non-canonical: regenerate it from `RUN.md`'s canonical `mission_states`, `task_states`, and `graph_state` whenever that state changes — never a second source of truth. A managed-sequential run may omit it. If this file and `RUN.md` ever disagree, `RUN.md` is right; regenerate this file from it, and never re-derive a phase, transition, or unblock rule here — cite `references/execution-state-model.md`'s "Canonical Mission Phases" and "Canonical Task Phases" instead of restating them, so this view can't silently drift out of sync with the rule it's displaying.
