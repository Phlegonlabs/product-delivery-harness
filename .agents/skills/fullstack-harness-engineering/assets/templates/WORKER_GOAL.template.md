...
The final reviewer reports the reviewed head SHA, PASS or fix-required decision, path-and-line findings, and evidence paths inside its `subagent_activity` entry. A trivial mission or unavailable child runtime may return without a task-local reviewer only when it records the exact reason; the parent must then supply an equivalent read-only review before integration.
