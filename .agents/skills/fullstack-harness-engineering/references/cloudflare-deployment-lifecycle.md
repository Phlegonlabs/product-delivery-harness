...
- Both Workers must already exist before wiring up their Git connections. Create each once with an initial `wrangler deploy --env <name>` so it appears in the dashboard, then connect the same repository separately to each Worker — each with its own branch-to-listen-to setting and its own deploy command, per Cloudflare's own advanced-setup guidance.
