...
| Release targets | <architecture.md `## Release Targets`> | <hash or revision> | human / team | draft / frozen | <expected surface inventory and stable target IDs PLAN reuses verbatim> |
...
web-app expected deployable surface from architecture.md (slug, not numbered)
web-development release target from architecture.md (slug, not numbered)
```
...
- When `architecture.md` exists, its `## Release Targets` section is the upstream contract for deployable destinations, and its stable surface and target IDs are upstream IDs like any other. PLAN-v5 reuses each target ID verbatim as `release.targets[].id` and keeps that target's `surface` assignment; a harness planner does not mint fresh release target names. This holds on the default path — `implementation-plan.md` carries the same rule but is optional, so PLAN reuse cannot depend on it.
- Every expected deployable surface `architecture.md` lists must have at least one target in PLAN's `release.targets[]`. An expected surface with no target — a mobile store target, say — is a dropped must-have contract, not a scoping choice. Retire an ID rather than reuse it when a destination's meaning changes.
