...
- The PLAN declares a `release` section while `landing.mode` is `local_only`. The production target's PASS requires a merged PR (`harness_release.py`), and `local_only` can never record one, so the run cannot reach `complete`. The manifest validator does not reject this pairing, so catch it at the Plan Readiness Gate: either move the run to `pull_request` mode or drop the release section. Discovering it at closeout means every mission is already built.
- A mission's write scope has no review-type node covering it. `graph-orchestration.md` permits an empty `required_reviews`, and the validator only checks that self-declared review types have matching nodes — it never checks that every write scope is covered. In `local_only` there is no GitHub review to fall back on, so an empty list means the run ships with no independent review at all. Verify the coverage by reading the graph, not by trusting validation to have caught it.
- A must-have `ARCH-*`, `PRD-*`, `UI-*`, `UX-*`, or `DS-*` trace has a downstream task but no verification row. The validator enforces only the task half of the coverage rule above, so an implemented-but-unverified contract passes validation and reaches closeout as an uncovered must-have.
