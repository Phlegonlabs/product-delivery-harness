...
Two kinds of stop exist. Validator-enforced stops surface when `harness_manifest.py` validates the plan or run — they are listed so a planner understands why validation or authorization fails, not because an agent must catch them by reading. Judgment stops have no validator; the agent must notice them itself and stop.

### Validator-Enforced Stops

`harness_manifest.py` blocks these. Fix the plan or run state; do not work around the validator:
...
- A mission's write scope has no review-type node (`backend_code`/`frontend_code`/`visual` as applicable) covering it. Enforced at execution authorization rather than plan validation, so an upgraded v3 projection stays a valid PLAN — it simply cannot execute until its review nodes are authored. This graph-level review is the only review gate a run has; `required_reviews` staying empty never means the run ships without review.
...
- `design-system.md` and `design-system.json` are both present but fail `product-design-builder`'s `scripts/check_design_system_pair.py --require-filled`, run from the repository root. A failing pair means the two files no longer agree — usually a hand edit to one after publication. Route the fix through `design-input-updates.md`; do not guess which file is right or implement against half a contract.
...
- A migration task reaches mission integration with `migration_classification` unset. The classification (`additive` / `destructive`) is stated when the task is authored per `execution-task-decomposition.md`; an unclassified migration at integration may be hiding a destructive change behind a default.
