...
- Contract conformance: the implementation uses only entries `ui-registry.json` lists, follows each touched route's recipe, and contains no raw visual value, page-local control, inline layout style, or unregistered motion. Run the project's UI contract check — `ui-architecture-builder` ships `scripts/check_ui_contract.py` for the source-scanning subset — plus the visual check at 390 / 768 / 1200 / 1440 px in normal and reduced motion. A drift from the registry or a recipe is a contract violation, not a stylistic difference; a passing functional test does not cover it. See `SKILL.md`'s UI Implementation Contract for what each implementation mission owes.
