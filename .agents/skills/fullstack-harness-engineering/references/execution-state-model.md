...
| `worker_passed -> integrating` | Parent rechecks ancestry, actual diff scope, forbidden files, head stability, integration authorization, and at least one read-only review PASS bound to the exact worktree head; a disabled task-local policy or graph-backed direct worker without one requires a terminal covering `review_workers[]` PASS before this transition |
