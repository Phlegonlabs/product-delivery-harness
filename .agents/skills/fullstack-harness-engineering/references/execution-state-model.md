...
| Pull-request mode | A merged landing preserves current-head checks and review. A harness-performed or auto-merge path requires exact `merge_pr` authorization for every mission and `pr:<full-PR-URL>`; an observed external human merge leaves `merge_pr` false |
