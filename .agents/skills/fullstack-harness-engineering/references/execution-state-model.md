...
A Claude Code parent may record this separate cc-codex capability only after the read-only foreground preflight succeeds:
...
This capability depends on the installed `codex:codex-rescue` Agent, not a repository-owned App Server client. Before preflight, record observed plugin metadata with status `unknown`; a plugin path or version alone is not proof of availability. `scripts/validate_codex_wave.py --mode preflight` requires exact `runtime:codex` and `worker:preallocation` authorization and emits the complete preflight arguments. Only a successful marker promotes the record to `available`. Wave mode then requires exact worker, worktree, and branch authorization. A write mission requires `spawn_subagents`, `create_app_managed_worktrees`, `create_local_branches`, and `create_local_commits`; it does not require `create_user_owned_tasks` because the Codex worker is a child Agent rather than a user-owned app thread.
...
Per-worker axes may differ from the host axes for an external adapter. A write-capable `external_codex_agent` records `subagent` + `app_managed_worktree` + `agent_result`, while its Claude Code host remains `dynamic_workflow` + `parent_managed_worktree` + `agent_result`. The parent records concrete allocation identity before the guarded launch and validates returned runtime evidence before integration. External Codex reviews are disabled in v1; the selector must use another allowed provider or defer.
