...
Compact RUN-only work is a deliberately sequential exception: its RUN plan identity fields are `null` and it makes no validated scheduling claim. "Sequential" describes the mission cadence, not the workspace: default it to `subagent` + `parent_managed_worktree`, one mission worktree at a time from the current integration SHA resolved from target-repository instructions. Require one exact-head read-only review before parent integration, then merge the passing head into that persistent integration branch and run its integration gate. Use `parent` + `shared_checkout` only when worktree creation itself is unavailable or unauthorized. The parent may set compact `plan_readiness` to `ready` after the applicable human readiness checks pass, then set `status` to `running` only with explicit execution authorization.
...
- Local integration does not authorize PR creation, repository configuration, review management, PR merge, branch deletion, worktree removal, or any `push`/`deploy` outside the integration-branch and development-target scope named above.
...
Schemas v3 through v10 require a `landing` object. Ordinary implementation, PRD, UI, branch, commit, or local integration work defaults to `local_only` on the persistent integration branch resolved from target-repository instructions. Local-only mode cannot record a pushed head or promotion PR and does not wait for GitHub CI or review, but every mission worktree still passes its exact-head pre-integration review. RUN v10 may use `integration_pull_request` when repository policy requires reviewed feature branches to enter the retained integration branch through a PR. Use `pull_request` only for a later landing-branch promotion after the user separately reviews the accumulated integration result and gives final approval to start promotion. When the repository defines no other model, new RUN-v10 files default `development` to `integration.branch` and `landing.head_branch`, and `production` to `landing.base_branch`; otherwise record its existing branches. Older schemas retain their recorded branch names.

Do not put a future protected-branch promotion into the ordinary mission run's early authorization bundle. After final user approval starts promotion, request every missing exact landing action for the resolved head and base branches; bind PR creation, review, and merge to `future-pr:<owner>/<repo>:base=<base>:head=<head>` until the PR exists. After creation, verify those branch fields, append the exact `pr:<full-PR-URL>` target, and run current-head CI and review continuously. A PR into the retained integration branch may merge under exact `merge_pr` authorization only when that branch is not also the protected base. If the integration branch resolves to `main`, the PR follows the protected-base rule: stop merge-ready and hand it to the human unless a later explicit instruction names that exact existing PR. Earlier implementation or local-integration authorization never supplies either merge grant.

For every created PR, `pr_head_sha` equals `pushed_head_sha`, checks bind `checks_head_sha` to that PR head, and review binds `review_head_sha` to that PR head with zero blocking findings and unresolved threads. CI and review are independent sibling gates and should run or be observed concurrently for the current PR head. The remaining equality depends on the landing mode. For a promotion `pull_request`, the PR head is the retained integration head, so a CI or review PASS and `merge_status: ready` require `checks_head_sha == review_head_sha == pr_head_sha == integration.integration_head_sha`. A later local integration changes that promotion candidate and immediately makes its earlier CI, review, and ready evidence stale.
