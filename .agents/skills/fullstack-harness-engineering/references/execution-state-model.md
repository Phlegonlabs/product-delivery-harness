...
Every active RUN v10 also proves that its exact integration branch is unprotected before the grouped instruction may cover a push. `local_only`, `integration_push`, and `pull_request` record the exact full ref, `unprotected` status, and a `repository:` policy source in `landing.integration_branch_protection`. Missing, stale, malformed, non-repository, or `protected` evidence fails closed and requires a separate exact target source. `merge_pr` and `deploy` remain outside the bundle under the default main-only branch model: every merge lands on `main` and every release target is production, so both always need their own instruction. A repository whose own instructions define a separate non-protected integration branch is the only case where a `merge_pr` can ride the loop, and then only for a PR whose base is that exact branch. `integration_pull_request` records the same proof in `landing.base_branch_protection` because its base is the retained integration branch. Only completed history without `action_target_contract` keeps the pre-evidence readability fallback because it cannot dispatch another action.
