...
This capability depends on the installed `codex:codex-rescue` Agent, not a repository-owned App Server client. A plugin path or version alone is not proof. `scripts/validate_codex_wave.py` requires exact `runtime:codex` and `worker:preallocation` authorization before allocation, then exact worker, worktree, and branch authorization for the launched wave. A write mission requires `spawn_subagents`, `create_app_managed_worktrees`, `create_local_branches`, and `create_local_commits`; it does not require `create_user_owned_tasks` because the Codex worker is a child Agent rather than a user-owned app thread.
