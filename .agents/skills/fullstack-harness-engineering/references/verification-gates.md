...
| Design-system pair check | `design-system.md` and `design-system.json` both exist | `product-design-builder`'s `scripts/check_design_system_pair.py --require-filled` exits 0 against the frozen pair, run from the repository root; a failure is a stop condition per `contract-and-traceability.md`, not a fix-it-inline finding | checker output |
