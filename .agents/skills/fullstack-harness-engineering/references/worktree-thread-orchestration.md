...
2. Record the observed cc-codex plugin metadata as an `unknown` external runtime without claiming availability. Run `scripts/validate_codex_wave.py --mode preflight` only when a ready node prefers or requires Codex. The guard requires exact `runtime:codex` and `worker:preallocation` authorizations and emits the complete preflight arguments, including the observed plugin version.
3. Invoke `CLAUDE_CODEX_PREFLIGHT.template.js` only with that guard output. Record `codex_rescue_agent` as available only after one read-only `codex:codex-rescue` Agent returns the exact preflight marker in foreground mode. After the parent records the active wave, lease, worker, worktree path, and branch ref, run the guard again in wave mode. The wave guard requires the proven available runtime plus exact worker, worktree, and branch authorization for `spawn_subagents`, worktree creation, branch creation, and commits. It never edits RUN, starts an Agent, or runs Git.
