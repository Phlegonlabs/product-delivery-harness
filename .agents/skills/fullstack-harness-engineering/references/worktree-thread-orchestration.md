...
- `integration.branch` is the only branch field in RUN. It names the resolved integration branch: the run's own `codex/<short-name>` branch, cut from the current default branch. Refuse a current v10 `push` without explicit remote intent, an exact branch/head grant, or known default-branch identity; also refuse when the target, integration branch, or observed default branch resolves to `main`.
