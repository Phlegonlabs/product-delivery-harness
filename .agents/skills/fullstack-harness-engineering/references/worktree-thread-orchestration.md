...
7. Validate every result against live Git facts. Require the task's own post-edit read-only reviewer to PASS on the exact returned worktree head. For a disabled task-local policy, validation may first accept the result so the covering PLAN review node becomes selectable; run the equivalent parent-owned review and record its terminal exact-head PASS in `review_workers[]` before the mission transitions to `integrating`.
8. Route findings back to the same worktree and review the changed head again. Integrate passing heads serially into the resolved target-repository integration branch.
