...
A native merge-triggered Cloudflare release always requires independent exact production `deploy` authorization bound to the current PLAN revision/digest, head, missions, and every `release:<target-id>` consequence. If the harness performs or auto-merges, it also requires separate exact `merge_pr` authorization for the PR and current head; if the human merges externally, leave `merge_pr` false and retain `external_merge_observation` from GitHub's `merged_by` actor plus a durable API/event reference bound to the exact PR URL, reviewed head, and resulting merged SHA. Never infer deployment authorization from merge authorization, or merge authorization from deployment authorization. Run the core Pre-Deploy Confirmation Checkpoint before an authorized harness merge because that merge is the provider's publication trigger.
...
A pull-request run remains non-complete when the exact current-head CI and review gates pass and the verified PR is handed to the human. At that merge-ready handoff, RUN records the PR URL, PR head, check head, review head, findings/threads, and evidence, but keeps the run non-complete while the PR is open. After the human merges, observe that GitHub reports the PR merged and inspect the current merged base state; leave `merge_pr` false because this is observed external state, record `pr_state: "merged"`, `merge_status: "merged"`, the resulting `merged_sha`, and `external_merge_observation` with GitHub's human actor and durable API/event reference bound to that exact PR URL, reviewed head, and merged SHA, then record the required merged-base evidence and then and only then set the RUN to `complete`. A boolean or merged status alone is not external-merge proof. The same merged-base observation requirement applies when the harness performed a separately authorized exact-PR merge, but that path omits `external_merge_observation`. Report any unexecuted deployment or cleanup action separately.
