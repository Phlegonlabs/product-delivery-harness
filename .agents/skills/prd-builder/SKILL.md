...
2. Conduct a complete but concise product interview before drafting, unless the user explicitly says to skip questions, make assumptions, or produce a first draft immediately. Scope this free-text message to the open-ended discovery categories in `references/interview-guide.md` (goal, users/roles, workflows, data/integrations, business rules, delivery constraints, success metrics, and confirmation/recovery expectations).
3. Immediately after the free-text interview message — not interleaved with it as separate turns — issue one or two `AskUserQuestion` calls (max 4 questions per call) resolving the interview's closed-set decisions marked `(AskUserQuestion)` in `references/interview-guide.md`: for a deployable product, the deployment platform (Cloudflare / Vercel / AWS / Self-hosted, plus the tool's built-in Other for anything else); product archetype; validation depth; and preferred visual style direction. Do not silently pick an option on the user's behalf.
4. For a UI-bearing product, capture a `Builder UX Direction Decision` before drafting wireframes. Resolve the builder to the human product/design decision owner or commissioning team. Use a second `AskUserQuestion` call (4 questions, at cap) for the closed dimensions — experience priority, guidance versus expert control, information density, and preferred layout/interaction pattern — and capture confirmation/recovery behavior and validation depth as free text or from step 3, since they vary too much by action to enumerate further. Mark every decision `selected`, `provisional`, or `assumed`.
5. For a product with a backend, persistent data, or auth requirement — evident from the free-text interview's data/integrations and business-rules answers, or the product archetype captured in step 3 — issue a third `AskUserQuestion` call (2 questions, well under the tool's 4-question cap) resolving the database category and auth strategy decisions marked `(AskUserQuestion)` in `references/interview-guide.md`'s data-and-integrations and architecture-constraints categories. Skip this call only when the product provably has no backend, persistent data, or auth surface, such as a purely static content or marketing site with no forms, accounts, or dynamic data. Do not silently pick a database category or auth strategy on the user's behalf.
...
10. Draft or repair the core Markdown package in the staging location defined by `references/artifact-lifecycle.md`:
    - `PRD.md`
    - `architecture.md`
    - `wireframes.md`
11. Produce `implementation-plan.md` only when the user explicitly asks for delivery sequencing or implementation planning.
12. Run the quality checklist in `references/output-contract.md` against the staged package. Resolve every required workflow lane and material verifier finding before finalization.
13. After validation, show the exact publish paths, overwrite targets, and archive moves. Perform them only when the original request already authorized those exact paths or the user explicitly approves them. Otherwise leave the validated staging package intact and report it as ready to publish. Never archive documents when the workflow is incomplete, paused, failing validation, or awaiting approval.
14. Report the final artifact paths and every archived path.
...
- Use `references/architecture-playbook.md` for implementation-ready architecture content across web, mobile, internal tools, and automations, including the development-to-production release contract for deployable products (Cloudflare given as the worked example; substitute the resolved platform).
