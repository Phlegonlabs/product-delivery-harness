...
A workflow result does not authorize file creation, overwrite, archive, or publication. The parent applies the normal staging lifecycle and repairs unresolved findings. For a UI-bearing product, the parent then hands the frozen product sources and Builder UX Direction to `product-design-builder`, which must use `impeccable` for bounded concept generation and `frontend-design` for visual craft while creating the wireframes and design-system contract. Research findings are applied by the parent, not the role: a finding that would widen product scope goes back to the user as a recommendation. Only after the complete package passes the output checklist does the parent present exact mutations for approval.
