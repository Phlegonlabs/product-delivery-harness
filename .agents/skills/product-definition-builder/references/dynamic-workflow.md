...
- release-target source policies naming the exact branch or ref each release builds from — for example the pushed integration-branch head for development and the default-branch head after merge for production. Signed tags and other source rules are recorded explicitly rather than left implied;
