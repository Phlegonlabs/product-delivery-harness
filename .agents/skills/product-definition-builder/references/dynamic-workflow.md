...
- release-target source policies naming remote `development` for the internally tested development release and remote `main` for production after same-SHA fast-forward under separate authorization. Initial delivery starts from `main`; enhancement starts from `development`. Signed tags or other source rules remain explicit;
