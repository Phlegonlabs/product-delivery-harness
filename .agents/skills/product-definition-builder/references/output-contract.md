...
UI grading: [Three fresh graders, rubric scope, frozen PRD path plus revision or SHA-256, current candidate HTML SHA-256, Technical Hard Gate and DOM geometry results, 0–100 grader scores plus per-dimension medians/statuses, overall score, disputed dimensions, high-fidelity H2/H4/H8 threshold results, refinement-round history with input/output hashes and findings addressed, and inline detail rows / skipped — multi-agent capability unavailable, with the same frozen identities and hard-gate result]
...
Key-surface treatments: [Per screen/region: motion-led / imagery-led / motion + imagery / quiet, with owner source]

Deferred generation handoff: [Per media/motion position: placement or trigger, dedicated prompt, source, reduced-motion expectation when applicable, generationStatus: deferred; no provider invoked]
...
Connected HTML review: [One self-contained high-fidelity HTML path; complete embedded CSS; left sidebar listing every in-scope UI-* page or screen; working page, state, modal/drawer, feedback, and approved-flow interactions; no live backend, authentication, or generation call]
...
Preview evidence: [Interactive HTML browser matrix; Technical Hard Gate, console, and element-level DOM geometry results; working-control, keyboard/focus, accessibility, responsive, and design-consistency evidence; limitations; multi-agent scores, variance reconciliation, or capability-unavailable skip]
...
The embedded page data must agree exactly with `PRD.md`; `PRD.md` wins on conflict. Every visible region action maps to exactly one `page`, `overlay`, or `feedback` flow and works locally without a network request. The data block may also project optional deferred `mediaIntent`, `UX-*` traces, and element display contracts described in `wireframe-guide.md`; they inherit the same agreement rule. Use inline CSS and JavaScript only for the low-fidelity review shell. Make no external request and include no production component, brand styling, high-fidelity visual direction, generated imagery, final animation, or design-system token decision. Human approval completes the wireframe phase; do not start visual design or implementation unless separately requested.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Development | [Exact remote `development` head after candidate promotion and read-back] | [Distinct development deployment unit] | [Isolated non-production resources] | [Development] | [Sandbox or not applicable] | [Classification, command, and ordering or not applicable] | [Internal suite, URL, deployed SHA, smoke, evidence] | [Prior development version] |
| Production | [Exact remote `main` head after the same internally verified development SHA is fast-forwarded and read back] | [Distinct production deployment unit] | [Production resources] | [Production] | [Live or not applicable] | [Classification, command, and ordering or not applicable] | [URL, deployed SHA, production smoke, evidence] | [Prior production version] |
...
- Source policy: [`development` for the internally tested development release and `main` for production after same-SHA fast-forward, or another exact branch or ref rule, including a required signed tag, stated explicitly]
...
Stop after the human owner approves `wireframes.html` unless they explicitly ask to continue into visual design. On that later request, run `ui-design-pass.md`: record Taste applicability, use `design-taste-frontend` where applicable or `frontend-design` for excluded surfaces, produce the connected interactive high-fidelity HTML with deferred image and motion handoffs, obtain human visual approval, and freeze the selected result in `PRD.md`'s `### UI Design Handoff`. Pass a known reference use as `design inspiration` or `page-faithful target`; do not infer faithful-copy intent from a URL, screenshot, or Figma frame.
...
- The hosted environment contract names remote `development` as the internally tested development source and remote `main` as production, with initial delivery based on `main`, enhancements based on `development`, exact-SHA fast-forward promotion, separate authorizations and read-backs, and internal development verification before `main`. It also names distinct deployment units, isolated resources/secrets/data/auth/payment modes, migration order, evidence, and recovery. Development never uses production customer data, sessions, or live payment mutations. Native targets remain provider-neutral.
...
- When the visual-design phase was explicitly requested, the Design System Need Gate is not `blocked`. Its UI Design Handoff records one connected self-contained high-fidelity HTML review file: the left sidebar lists every in-scope `UI-*` page or screen, every visible product control performs its recorded page, state, modal/drawer, or feedback behavior, and full CSS is present. Login, registration, recovery, and authentication-error previews are recorded `n/a` for this visual pass; the HTML starts from the main or authenticated entry without live auth. Image and motion positions remain static deferred-generation placeholders with dedicated prompts and no provider call. The handoff records its frozen PRD identity plus PRD-bound multi-agent scores, or the exact capability-unavailable skip. When the gate is `required`, the package includes the validated pair returned by `design-system-compiler` and the parent records its exact staged paths and passing checks. When it is `not_required`, no placeholder pair is present and the approved page-faithful UI target, `PRD.md`, and `wireframes.html` are explicitly named as the replacement visual contract.
