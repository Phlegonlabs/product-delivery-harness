...
Tokens, primitives, components, recipes, and the registry are not ready to draft until two or three candidate HTML directions have been compared, a consolidated selected HTML direction exists, its ordered per-screen path/SHA-256 manifest and `approval_manifest_sha256` have been recorded, and the human owner has explicitly approved those exact bytes. Immediately before Dynamic Workflow launch, the parent must read and hash each selected file, record manifest-bound byte-verification evidence, and let the workflow recompute the canonical manifest digest. When Hallmark is loaded, its selected-HTML audit must also be recorded; unresolved findings require repair or an exact human waiver. If exploration is not used, draft with explicit assumptions and do not claim approved-HTML extraction.
