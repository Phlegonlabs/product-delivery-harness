...
- the exact approved selected HTML plus its human approval record, or an explicit `visual_direction_pass.status: "not used"` or `"rejected"` record; an `approved` payload includes one or two unique non-empty `representative_ui_ids`, two or three `candidate_directions` with a unique `direction_id` and one `html_path` per representative screen, a canonical `selected_html_path` under `visual-directions/selected/`, an ordered `selected_html_files` entry per representative UI ID with canonical path and lowercase SHA-256, the lowercase `approval_manifest_sha256`, `approval_owner`, and `approval_evidence`; the workflow never launches while preference discovery, candidate comparison, selected-HTML consolidation, digest binding, or approval is still pending;
- Hallmark availability and, when loaded, the retained candidate and selected audit-report paths and dispositions; when unavailable, record that state and do not claim a Hallmark pass;
