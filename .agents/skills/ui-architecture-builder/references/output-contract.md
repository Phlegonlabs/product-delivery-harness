...
| TEST-VIS-030 | Selected-HTML immutable approval binding | when selected HTML is approved | UI-*, DS-* | Every representative UI ID maps to one canonical file under `visual-directions/selected/` with a lowercase SHA-256; `selected_html_path` equals the selected root or one approved manifest path; the parent re-reads and hashes every file immediately before launch; the workflow recomputes the canonical manifest SHA-256 and requires it in both parent verification and human approval evidence; any byte, path, order, or mapping change forces fresh audit and approval | selected-file byte hashes / parent verification / workflow digest recomputation / approval record |
