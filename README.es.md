...
| --- | --- | --- |
| Una idea de producto | `product-definition-builder` | Requisitos, un estudio estructural pulido `wireframes/4` para cada surface y state de UI, copy aprobado y vinculante para implementación, QA de layout en navegador, arquitectura, decisiones de stack, objetivos de release, tests e investigación de mercado con fuentes |
| Un paquete de wireframes aprobado que necesita diseño visual | UI Design Pass de `product-definition-builder`, luego `design-system-compiler` + `frontend-design` cuando el gate lo exige | Un Direction Checkpoint guiado por referencias y después un HTML de referencia de diseño conectado y autocontenido, con copy congelado, CSS completo y flujos clicables, más un par de design-system vinculante cuando se requiere |
...
- **La definición de producto termina en gates humanos ordenados.** Un paquete con UI cierra en un estudio responsive `wireframes/4`. El canvas neutral muestra copy real del producto y ejemplos dinámicos representativos; propósito, priority, traces y reglas responsive viven en un inspector solo para revisión. Copy Freeze registra owner, locale principal, textos exactos estáticos/de acciones/de states alternativos y contratos dinámicos completos de source/order/format/count/length/fallback. El copy se aprueba primero mientras el status estructural sigue draft; solo después se ejecutan UI grading, la revisión completa de layout en navegador y Wireframe Approval sobre esas palabras exactas. El checker mantiene legibles los `wireframes/2` y `/3` sin cambios y rechaza la aprobación estructural previa a Copy Freeze. El Reference Pass estructural registra evidencia `WREF-*` de screens y flows exactos; una galería solo complementa flows reales de primera parte.
- **Los targets visuales son HTML responsive e interactivo.** La fase visual solicitada presenta primero una dirección específica del producto en un Direction Checkpoint, normalmente sustentada por dos referencias inspeccionadas y complementarias y, como máximo, una tercera distinta. Solo la aprobación abre el HTML completo de referencia. La referencia conserva Copy Freeze de forma literal; los controles visibles navegan, cambian de state, abren overlays registrados o muestran feedback. El Motion Need Gate conserva la autoridad humana sobre motion; el motion UI funcional puede ejecutarse localmente con una ruta reduced-motion equivalente y los assets generados siguen deferred. El Technical Hard Gate rechaza errores de runtime, requests inesperadas, states inaccesibles, eventos duplicados, copy drift y fallos del motion requerido. El gate humano exige 90 global y 90 en `H2`, `H4` y `H8`; las referencias aprobadas se conservan bajo `docs/design/ui-references/` y las reemplazadas se archivan.
...
| --- | --- | --- |
| `product-definition-builder` | Descubrimiento de producto, evaluación research-first y Research Gate, requisitos, inputs de Builder UX Direction, evidencia estructural `WREF-*`, wireframes interactivos y responsive con Copy Freeze y QA en navegador, arquitectura, decisiones de stack, objetivos de release, obligaciones de test, reconciliación post-borrador de market research, Direction Checkpoint opcional guiado por referencias y HTML interactivo de referencia de diseño, y revisión de outcome post-despliegue | `PRD.md`, `research-assessment.md`, `wireframes.html` (productos con UI), `architecture.md`, `stack-decisions.md`, `market-research.md`, `outcome-review.md` |
...
Puedes empezar en cualquier etapa. Por ejemplo, usa solo el Harness para arreglar una app existente. Los skills mantienen sus responsabilidades separadas: `product-definition-builder` define el producto y termina en un `wireframes.html` con estructura aprobada y Copy Freeze; el UI Design Pass opcional y `design-system-compiler` definen el contrato visual después del Direction Checkpoint — la pass deja una referencia HTML autocontenida en `docs/design/ui-references/<run-id>/`, con todas las páginas, copy congelado, CSS completo, flujos clicables y handoffs diferidos de media/motion; el Harness implementa el resultado sin reescribir el copy; `code-security-review` revisa el candidato unificado sin editarlo; y `product-activation` configura y verifica el release entregado sin reabrir el RUN de entrega.
...
flowchart TB
    ...
    subgraph PRD["product-definition-builder — definición de producto"]
        ...
        wf["wireframes/4 HTML<br/>estructura + copy vinculante"]
        cgate{{"Copy Freeze Gate<br/>(primero se aprueban las palabras)"}}
        wgate{{"Wireframe Approval Gate<br/>(después se aprueba la estructura)"}}
...
Copy Freeze, Wireframe Approval y el merge a `main` siguen siendo gates humanos. El bucle de ejecución de la entrega queda dentro de PLAN/RUN; la Activation post-entrega arranca solo después de cerrar el RUN y aplica sus propias aprobaciones exactas de acciones externas.
...
```text
Usa $product-definition-builder para convertir esta idea en un PRD y un estudio wireframes/4 para cada página, target y state. Redacta copy exacto de interfaz y mensajes de states alternativos, define contratos de contenido dinámico, obtén Copy Freeze y completa QA de layout, arquitectura, decisiones de stack, objetivos de release y obligaciones de test.
```

```text
Usa $product-definition-builder para aprobar primero Copy Freeze desde el Copy Inventory del wireframes.html en staging, manteniendo el status estructural en draft. Después revisa cada combinación página-target-state en un navegador real y registra el Wireframe Approval posterior antes del trabajo visual o de implementación.
```

```text
Los wireframes y Copy Freeze están aprobados; continúa con el UI Design Pass de $product-definition-builder. Crea un set complementario de referencias inspeccionadas y obtén aprobación del Direction Checkpoint; después renderiza todas las páginas y states in-scope en un HTML autocontenido con CSS completo, barra lateral, copy congelado y flujos clicables. Registra los previews de auth como n/a, revisa la matriz responsive/state en navegador y conserva el archivo bajo docs/design/ui-references/; invoca $design-system-compiler solo cuando el Design System Need Gate lo exija.
```
...
- **0.36.0** — Los wireframes ahora congelan las palabras además de la estructura. Los paquetes UI nuevos o revisados estructuralmente usan `wireframes/4`: textos estáticos, labels de acciones y mensajes de states alternativos son vinculantes para implementación; los valores dinámicos llevan ejemplos reales y contratos source/order/format/count/length/fallback; cada entrada `UI-*` del PRD tiene un anchor `copy` legible por máquina; y Copy Freeze registra owner, locale principal, fecha y estado de aprobación. El checker bloquea draft copy en un archivo aprobado y conserva la lectura de `wireframes/2` y `/3` sin cambios. El template HTML pasa a ser un estudio neutral con copy de producto en el canvas, detalles solo de revisión en un inspector y un Copy Inventory completo. La investigación estructural registra evidencia estable `WREF-*` de screens y flows exactos; el UI Design Pass exige referencias complementarias inspeccionadas y un Direction Checkpoint humano antes del HTML completo. Delivery conserva el copy literal y devuelve todo cambio de wording o contrato dinámico a un copy delta de schema 4 con nueva aprobación. Cambio rompiente del skill bundle.
