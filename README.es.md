...
- **La activación se lee de vuelta.** La configuración externa queda fuera de PLAN/RUN y liga la aprobación a un action digest y release target tipado exactos. Deployment, Activation, Outcome y SEO reutilizan el mismo SHA, artifact, provider/channel, endpoint e identidades de medición.
...
9. Autoriza por separado la preparación de A usando el mismo `--archive-anchor` más request/attempt/receipt externo al checkout; un humano/trusted host ejecuta el push exacto sin force, recovery verifica non-production y el read-back de A, y después se hace fast-forward exacto de A a `main` y se verifica producción. Si falla candidate/preview, crea un replacement RUN de continuación, cierra C2 y archiva A2 con un anchor nuevo.
