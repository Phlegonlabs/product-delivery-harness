...
- **El diseño UI tiene su propia ruta de aprobación.** `ui-design-builder` pregunta primero por UI, style, motion y si cada región usa image o motion. `frontend-design` es el único autor de wireframe y HiFi; los wireframes schema 4 congelan copy exacto del producto y contratos dinámicos acotados con un owner humano antes del scoring W1–W5 y la aprobación estructural. Después, Style Integration produce una referencia HiFi conectada. Impeccable ejecuta `critique` + `audit`, mientras H1–H9 sigue siendo el gate vinculante: total 90+, H2/H4/H8 90+, cada dimensión 60+ y sin block/dispute. Motion UI simple usa CSS/WAAPI; los skills GSAP se cargan solo si hacen falta; Higgsfield MCP genera motion únicamente con autorización exacta. Los design tokens se compilan solo tras Visual Approval.
