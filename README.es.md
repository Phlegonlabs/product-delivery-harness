...
- **La definición de producto termina en un gate humano.** Un paquete con UI cierra en un `wireframes.html` responsive de baja fidelidad; cada surface, target y state que no sea `n/a` debe pasar la revisión de solapamiento, recorte, oclusión y overflow en navegador antes de la aprobación del owner. El checker autocontenido decodifica los CSS escapes antes de rechazar cargas de recursos remotos.
- **Los targets visuales son HTML responsive.** Una fase visual web solicitada renderiza cada página de alta fidelidad en la misma matriz responsive/state, conserva las referencias aprobadas bajo `docs/design/ui-references/` y archiva los conjuntos reemplazados en vez de borrarlos; el Harness construye y reverifica cada página desde su referencia aprobada.
...
| --- | --- | --- |
| `product-definition-builder` | Descubrimiento de producto, la evaluación research-first previa al borrador y su Research Gate, requisitos, inputs de Builder UX Direction, wireframes responsive de baja fidelidad con QA de layout en navegador, arquitectura, decisiones de stack, objetivos de release, obligaciones de test, la pasada de reconciliación post-borrador de market research, el UI Design Pass opcional de matriz completa cuya ruta por defecto renderiza una referencia HTML de alta fidelidad conectada de todas las páginas, y la revisión de outcome post-despliegue | `PRD.md`, `research-assessment.md`, `wireframes.html` (productos con UI), `architecture.md`, `stack-decisions.md`, `market-research.md`, `outcome-review.md` |
