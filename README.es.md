...
- **La definición de producto termina en un gate humano.** Un paquete con UI cierra en un `wireframes.html` responsive de baja fidelidad; cada surface, target, state y acción visible del PRD debe funcionar localmente y pasar la revisión de layout en navegador antes de la aprobación del owner. El checker valida flujos de page, overlay y feedback, además de handoffs `mediaIntent` diferidos. Cuando el host tiene capacidad multi-agent con navegador y el dispatch está autorizado, tres graders read-only frescos califican cada dimensión de 0 a 100 contra el PRD congelado; cuando la capacidad no existe, el registro de aprobación incluye el skip exacto y los gates de navegador, checker y owner siguen ejecutándose.
- **Los targets visuales son HTML responsive e interactivo.** La fase visual solicitada produce una referencia HTML de alta fidelidad conectada cuyos controles visibles navegan, cambian de state, abren overlays registrados o muestran feedback. Los previews de login, registro, recovery y errores de autenticación se marcan `n/a` para esta pasada. Las posiciones de imagen y motion quedan como placeholders estáticos con prompts dedicados y `generationStatus: deferred`; ningún proveedor de generación se ejecuta hasta una pasada MCP posterior y explícitamente autorizada. Un Technical Hard Gate rechaza errores de runtime, requests inesperadas, states inaccesibles y efectos de eventos duplicados. La calificación multi-agent condicional y ligada al PRD exige una puntuación global mínima de 80 y puntuaciones mínimas de 80 en layout `H2`, responsive `H4` y accessibility `H8`, con checks de elementos en cada target; también puntúa distinción creativa y consistencia de diseño sin premiar la invención de scope ni la pérdida de usabilidad. Un candidato bajo el umbral vuelve a refinement y repite la secuencia completa de gates y grading antes de la aprobación humana. Las referencias aprobadas se conservan bajo `docs/design/ui-references/` y los conjuntos reemplazados se archivan en vez de borrarse.
...
| --- | --- | --- |
| `product-definition-builder` | Descubrimiento de producto, la evaluación research-first previa al borrador y su Research Gate, requisitos, inputs de Builder UX Direction, wireframes interactivos y responsive con QA en navegador y calificación multi-agent condicional ligada al PRD, arquitectura, decisiones de stack, objetivos de release, obligaciones de test, la pasada de reconciliación post-borrador de market research, el UI Design Pass opcional con HTML interactivo de alta fidelidad y generación diferida de media y motion, y la revisión de outcome post-despliegue | `PRD.md`, `research-assessment.md`, `wireframes.html` (productos con UI), `architecture.md`, `stack-decisions.md`, `market-research.md`, `outcome-review.md` |
...
flowchart TB
    ...
    subgraph PRD["product-definition-builder — definición de producto"]
        ...
        wf["wireframes.html<br/>un archivo interactivo de baja fidelidad (productos con UI)"]
...
Clona el repositorio y copia los cinco skills de Product Delivery Harness a tu directorio de skills de usuario:
...
Si el checkout tiene directorios locales `__pycache__` bajo `.agents/skills/`, exclúyelos o bórralos de la copia — los hosts nunca necesitan el bytecode. En Windows, `Copy-Item -Recurse` hace lo mismo. No hay un script de actualización separado. Una actualización necesita aprobación explícita de install/update y ninguna sesión activa que use los skills. Antes de copiar, mueve los destinos existentes con nombres nuevos a un solo backup con timestamp bajo `~/.agents/skill-backups/product-delivery-harness/`, fuera del directorio de descubrimiento de skills. Copia los cinco directorios actuales, verifica que sus archivos coincidan con el checkout y arranca una sesión fresca del host. Restaura el backup si la verificación falla; nunca sobrescribas ni borres las copias anteriores.
...
```text
Usa $product-definition-builder para convertir esta idea en un PRD, wireframes responsive de baja fidelidad para cada página, target y state, QA de layout en navegador, arquitectura, decisiones de stack, objetivos de release y obligaciones de test.
```
...
```text
.agents/skills/                                      Fuentes canónicas de los skills
...
Edita solo las fuentes canónicas en `.agents/skills/` y luego ejecuta la suite de verificación central:
...
El CI también ejecuta el check end-to-end del spine. Ejecútalo localmente con `HARNESS_GOLDEN_PATH=1 python -m unittest discover -s .agents/skills/delivery-harness/scripts/tests -p "test_golden_path.py" -v`; recorre el spine real del CLI (`new_run.py` → joins congelados incluyendo el checker completo de wireframes del skill sibling → `validate_result.py --repo-root`) sobre un paquete de producto sintético, así el drift de contrato cross-skill aparece como un solo test en rojo.
...
1. El campo `version` de `package.json` y la versión del copied-skill en `.agents/skills/delivery-harness/VERSION`.
...
- **0.26.0** — Los contratos de UI responsive ahora bloquean desde la definición de producto hasta la entrega. Cada entrada `UI-*` declara un conjunto compartido de al menos dos viewports web o clases de tamaño nativas/desktop; `wireframes/2` proyecta cada target con orden de regiones, visibilidad, grid spans, reflow, reglas de interacción y regiones never-drop explícitos. La aprobación de wireframes y de HTML de alta fidelidad exige una matriz página-target-state en navegador real sin solapamiento, recorte, oclusión ni overflow horizontal no intencionales, mientras que los overlays intencionales documentan stacking, focus, safe-area y comportamiento de descarte. El par design-system y PLAN usan el mismo conjunto responsive, y el Harness rechaza cobertura faltante, duplicada, de un solo target, sin ordenar, extra o con drift, manteniendo legibles los schemas legacy.
...
- **0.9.0** — Hizo del HTML de alta fidelidad renderizado por skills de diseño la ruta por defecto de preview web en el UI Design Pass. Las referencias HTML aprobadas se retienen bajo `docs/design/ui-references/<run-id>/`, los conjuntos reemplazados se archivan bajo `docs/design/archived/`, y la implementación target-conformance construye cada página desde su referencia HTML aprobada con hashes congelados por archivo.
