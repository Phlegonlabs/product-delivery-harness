...
- **La promoción es main-only.** Entrega inicial y enhancements parten del `main` remoto observado. Un RUN 0.38 cierra en C como local-only y no puede hacer push. La publication autorizada de A necesita el anchor externo pre-archivo y registros inmutables de request/attempt/receipt; después de los gates del candidate, otra autorización y read-back fast-forwardean A sin cambios a `main`.
...
9. Autoriza por separado la publication de A usando el mismo `--archive-anchor` más request/attempt/receipt externo al checkout, verifica non-production aislado, fast-forward exacto de A a `main` y producción.
...
- **0.38.0** — Endurecimiento completo del contrato zero-to-one. Product Definition valida autoridad CommonMark activa, joins completos de producto/arquitectura/stack, clases de release tipadas, evidencia de repositorio e identidad exacta entre Deployment, Activation, Outcome y SEO. UI Design Builder añade shell schema 4 canónico, scope HiFi con `ui-hifi/1`, CSP restrictivo, receipts humanos sandboxed-offline, capture por plataforma y pruebas reales de publicación required/not-required; Design System publica schema 2 con seis source bindings exactos. Todo candidate command de PLAN-v6 corre solo en un sandbox Docker/Podman fijado, no-root, sin red y de solo lectura, observado antes del wave. Push, security, parity, deployment, archive e installer fallan cerrados; RUN cierra en C como local-only, el archive receipt y el anchor externo protegen el A exacto, y luego el mismo `--archive-anchor` con request/attempt/receipt immutable permite la publication opcional antes de la promoción separada a `main`. Cambio breaking del skill bundle.
