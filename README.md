...
9. Under separate authorization, prepare A through `--archive-anchor` plus external request/attempt/receipt; a human/trusted host executes the emitted exact no-force push, recovery verifies non-production/read-back A, then separately fast-forward A to `main` and verify production. If candidate/preview fails, create a fresh PLAN/RUN on the same non-default branch from exact A, close C2, and archive A2 with a new anchor; require remote pre-state A when A was published and absent otherwise.
