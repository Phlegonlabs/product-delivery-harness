...
- **Activation is read back.** External setup stays outside PLAN/RUN, binds approval to an exact action digest and typed release target, and becomes verified only after independent read-back and behavior evidence. Hybrid Outcome Review uses ordered target rows, target-scoped Activation Outcome Coverage measurements/feedback, and a deterministic aggregate verdict; the seed checker joins staged architecture targets to profiles. Deployment, Activation, Outcome, and SEO reuse the same SHA, artifact, provider/channel, endpoint, and measurement identities.
...
- **Completed runs fold away before promotion.** `archive_run.py` verifies C, current `main`, evidence, and the move list, then writes closed `ARCHIVE_RECEIPT.json` plus a required immutable checkout-external anchor and moves the coordination set. It rolls back failures. Archive-only A is revalidated against that anchor before any separately authorized publication. `push_archived_candidate.py` binds the exact canonical push URL, machine-installed policy ID/hash/principal, and OS-managed verifier hash in immutable request/attempt records, with a deterministic `<anchor-stem>-publication-receipt.json`, and returns `PENDING_TRUSTED_HOST_PUBLICATION`; it rejects caller prose/ref as authority and never invokes `git push`. A trusted host independently reloads and revalidates the request with sanitized config, executes the URL-only exact `git --no-replace-objects push` without force, signs execution evidence, and recovery verifies that evidence before reading back A and closing the receipt. Agents and the local executor must not run the emitted argv without that trusted boundary. Product Definition remains published.
...
9. Under separate authorization, prepare A through `--archive-anchor` plus external request/attempt/receipt/evidence and a pinned trust policy/verifier; a trusted host independently revalidates the request, executes the emitted exact URL-only no-force push, signs evidence, and recovery verifies it plus non-production/read-back A, then separately fast-forwards A to `main` and verifies production. If candidate/preview fails, create a fresh PLAN/RUN on the same non-default branch from exact A, close C2, bind immutable prior publication state, and archive A2 with a new anchor; require remote pre-state A when A was published and absent otherwise.
...
- **0.38.0** — Full zero-to-one contract hardening. Product Definition enforces active CommonMark authority, complete product/architecture/stack joins, typed release classes, repository evidence, and strict Deployment/Activation/Outcome/SEO identity. Hybrid UI surfaces carry per-surface release, capture, and responsive bindings; schema-2 `surfaceContracts` must match each approved `UI-*` identity and its release/capture/responsive contract, while platform and styling remain in the approved stack source. PLAN-v6 commands run only in an observed pinned, non-root, networkless, read-only Docker/Podman sandbox whose descriptor-bound executable/hash/version/RepoDigest is retained in request/evidence; archive mutation paths hold descriptor/handle identities against swaps. Exact-SHA Git reads reject replace refs and grafts; Windows browser argv never crosses `cmd.exe`; archive and installer writes reject links/reparse points; production/public hosted endpoints require HTTPS. RUN closes local-only at C; local archive tooling never pushes, and a failed A enters a fresh C2/A2 correction continuation without history rewrite. Breaking skill-bundle change.
