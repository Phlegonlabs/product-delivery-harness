...
- **完成的 run 会在晋升前收档。** `archive_run.py` 重验 C、当前 `main`、evidence 与 move list，写出关闭的 `ARCHIVE_RECEIPT.json` 以及必需的 checkout 外部 immutable anchor 后，才搬移协作集；失败会 rollback。Archive-only A 必须先针对该 anchor 重验，之后才取得另行授权的 publication。`push_archived_candidate.py` 在不可变 request/attempt 中绑定精确的规范 push URL、已观察的 allowed-signers 策略/hash 与绝对 verifier hash，并使用确定性的 `<anchor-stem>-publication-receipt.json`，返回 `PENDING_TRUSTED_HOST_PUBLICATION`；它拒绝 caller prose/ref 作为授权，也永远不调用 `git push`。Trusted host 必须重新读取并用清理后的配置重新验证 request，执行只含 URL 的精确无 force `git --no-replace-objects push` 并签署执行 evidence；recovery 先验证 evidence 再读回 A 并关闭 receipt。Product Definition 仍留在正式位置。
