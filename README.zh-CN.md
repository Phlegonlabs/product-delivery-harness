...
这是一个私有仓库。你需要具备对 `Phlegonlabs/product-delivery-harness` 的访问权限、完成 GitHub CLI 认证，并且至少有一个会发现 `~/.agents/skills/` 这类用户 skills 目录的宿主——Codex、Claude Code、Pi 或其他都可以。
...
- **0.25.2** — 仓库由 `fullstack-goal-dev` 更名为 `product-delivery-harness`，与产品名一致。README badge、clone 命令与安装路径全部改用新名；skill 行为不变。
