...
- **完成的 run 會在晉升前收檔。** `archive_run.py` 重驗 C、目前 `main`、evidence 與 move list，寫出關閉的 `ARCHIVE_RECEIPT.json` 以及必要的 checkout 外部 immutable anchor，才搬移協作集；失敗會 rollback。Archive-only A 必須先對該 anchor 重驗，之後才取得另行授權的 publication。`push_archived_candidate.py` 只會準備不可變的 request/attempt，回傳 `PENDING_TRUSTED_HOST_PUBLICATION`；會拒絕 caller prose/ref 當授權，也永遠不會呼叫 `git push`。人工或 trusted host 必須執行精確、無 force 的 `git --no-replace-objects push`，再由 recovery 讀回 A 並關閉 receipt。Product Definition 仍留在正式位置。
...
對於有計畫支撐的工作，它會記錄任務範圍、相依關係、worker 歸屬、驗證指令，以及各動作專屬的授權。測試通過並不代表授權推送、移除 worktree 或刪除分支。Harness 0.38 會讓 RUN push 保持 false；archive protocol 從 archive 推導 C 與 branch，驗證精確 C→A relocation 以及 remote pre-state，只準備精確 no-force publication handoff 並交給人工/trusted host；recovery 再讀回 A。Legacy pinned run 只保留舊流程供 recovery。
...
9. 另行授權，以同一個 `--archive-anchor` 加上 checkout 外部 request/attempt/receipt 準備 A；由人工/trusted host 執行精確 no-force push，recovery 驗證 non-production 與 A read-back，再把 exact A fast-forward 到 `main` 並驗證 production。若 candidate/preview 失敗，建立 continuation replacement RUN，關閉 C2 並用新 anchor 收檔 A2。
