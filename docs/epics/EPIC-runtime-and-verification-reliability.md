...
Review of `6a945344068b71ce2654db2285ee091ad8f0bab5` identified a repair-scope issue in the reused Git changed-path reader: line trimming and separator replacement can turn an actual leading-space or backslash filename into a different authorized path. The new same-RUN repair relies on this helper, so publication remains held. The bounded fix must preserve NUL-delimited Git path identities, reject unsupported literal names, and disable rename collapsing so both removed source and added destination participate in scope checks. Real-Git regressions must reject the misleading path and out-of-scope rename while retaining ordinary dynamic-route/Unicode paths. No candidate has been pushed.

The path reader now uses binary `git diff --name-only -z --no-renames`, strict UTF-8 decoding and the canonical literal-path validator. It does not trim names, split lines or rewrite separators. New real-Git regressions prove leading-space scope spoofing and outside-to-inside renames are rejected, while `[slug]`, Unicode and internal spaces remain literal. All five new tests, 23 RUN-v11 tests, nine candidate-resume tests, scoped Pyflakes and diff checks passed; the parent reviewed the helper and tests. Complete exact-SHA review and CI remain pending.
