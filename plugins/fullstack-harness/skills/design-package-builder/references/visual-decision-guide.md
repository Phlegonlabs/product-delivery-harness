...
3. Compute and record the actual contrast ratio for every text-on-background, text-on-surface, and text-on-accent pairing the tokens produce, against WCAG 2.2 AA (4.5:1 for normal text, 3:1 for large text and UI component boundaries). A stated ratio, not "looks readable," is what `visual-acceptance.md`'s contrast gate checks. Adjust the token value when a pairing fails, rather than accepting a color that reads well only in isolation.
