...
- When the gate is `not_required`, the approved UI target row must carry an immutable path or version, SHA-256, routes, states, responsive scope, tolerance, and allowed deviations. Freeze it with the PRD and wireframes. No design-system pair is expected, and its absence is not `missing` or `partial`.
