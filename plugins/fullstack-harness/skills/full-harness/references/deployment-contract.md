...
- Cloudflare Workers, CI-connected: the same workflow deploys with `wrangler deploy` on the production branch and uploads a preview version with `wrangler versions upload` on every other branch — each uploaded version serves its own preview URL once `preview_urls` is enabled in the Wrangler config, and the deployed-commit check still reads `wrangler deployments list`. The pushed head's version preview URL is observed from the workflow's `wrangler versions upload` output or `wrangler versions list`, and each push produces a fresh URL rather than a stable branch alias. Preview versions are still Workers: they bind only non-production D1/R2/KV/Durable-Object/auth/webhook resources and never touch production traffic. Promoting to production is `wrangler deploy` (or `wrangler versions deploy` for a gradual rollout), never a side effect of the upload.
