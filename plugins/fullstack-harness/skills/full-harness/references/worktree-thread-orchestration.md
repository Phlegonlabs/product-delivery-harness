...
The selector does not emit a batch base, an effective budget, or a bundled wave. The parent binds the committed `batch_base_sha`, confirms the budget it applied, records the accepted wave in `RUN.md`, assigns leases, and only then creates branches, worktrees, subagents, or app tasks — through the scripted write path: `harness_transition.py record-observation` (live-Git observed snapshot), `accept-wave` (wave plus batch base), `lease-worker` (graph/mission/task/worker/attempt in one atomic validated write), and `record-integration` (mission integrated plus the new integration head, verified against live Git when `--repo-root` is given). Hand-editing the RUN JSON for these steps is the path the transitions replaced. Scripts must not parse human Markdown tables or mutate Git, Codex, PLAN, or RUN.
