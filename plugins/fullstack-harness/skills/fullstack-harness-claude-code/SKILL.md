...
2. Allocate one collision-resistant branch, lease, and exact-base worktree per mission under `.claude/worktrees/<run>-<mission>-<attempt>/`. Before dispatch, verify the worktree belongs to the expected repository, has the assigned branch/ref and exact base HEAD, and has empty `git status --porcelain`.
3. Build immutable handoffs from `../fullstack-harness-engineering/assets/templates/WORKER_GOAL.template.md`, including the Claude Code worker contract, the effective `CLAUDE.md` repository context paths, and the shared `AGENTS.md` governance paths. Keep automatic `CLAUDE.md` context discovery enabled. Follow the generated `@AGENTS.md` import; when an established `CLAUDE.md` has no import, require the child to read the supplied `AGENTS.md` paths explicitly. Do not apply Codex or Pi worker mechanics.
...
The workflow coordinates sibling mission agents and remains flat. Mission workers and reviewers do not delegate, modify PLAN/RUN, integrate, push, or clean up. After serial integration, the parent starts fresh read-only reviewer agents on the exact unified integration SHA; it does not reuse mission writers or their pre-integration PASS. Only after those reviewers pass does the parent run the one broad final validation on the fixed candidate SHA. If Dynamic Workflow is unavailable, use the recorded fallback rather than an untracked fan-out.
