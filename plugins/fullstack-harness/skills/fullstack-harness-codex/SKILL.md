...
When child capability is unknown, launch a no-production-edit handshake, poll it, record the result, and send an enabled or disabled policy before implementation. With an enabled policy, the post-edit reviewer is mandatory for a non-trivial mission. Children never write, run mutating generators or shared-state services, spawn further agents, edit PLAN/RUN, create Git objects, integrate, land, deploy, or clean up. The app task reconciles their evidence and reports the reviewer's exact head, decision, findings, and `subagent_activity`. A trivial mission or unavailable child runtime still needs an equivalent parent-owned read-only review before integration.
