...
This core bundles `product-design-builder`, but it does not bundle `impeccable`, `frontend-design`, or `feature-dev`. Those are separate skills and require their own installation. Never fabricate their presence.

The normal UI handoff is `prd-builder` product inputs -> `product-design-builder` plus mandatory `impeccable` and `frontend-design` creation mode -> frozen `wireframes.md`, `design-system.md`, and `design-system.json` -> Harness implementation. Any exploration artifact is non-canonical evidence; implementation begins only after the design set is frozen.

- A mission that creates or revises wireframes or the design-system pair lists `product-design-builder`, `impeccable`, and `frontend-design` in `required_skills`. Missing or unloadable `impeccable` or `frontend-design` blocks readiness and execution; creation mode has no fallback.
...
Each run resolves one exact non-default branch from repository governance or the user's instruction, cuts it from the current default branch, and does all its work there. The Harness never adds a fixed prefix. That branch is the deliverable.
...
When a design-source worker loads `product-design-builder`, `impeccable`, and `frontend-design`, its launch prompt must state creation mode, the frozen product inputs, the allowed design-source write scope, the Impeccable concept-generation bridge, and the human direction gates. When a UI implementation worker loads only `frontend-design`, its launch prompt must state the conformance boundary and name the frozen wireframe screen, `design-system.md`, and `design-system.json` inputs. A generic instruction to "make it distinctive" is not a valid handoff in either mode.
