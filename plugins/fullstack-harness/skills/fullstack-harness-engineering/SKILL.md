...
- `assets/templates/CLAUDE_CODEX_PREFLIGHT.template.js` proves a Claude Code parent can invoke `codex:codex-rescue` in read-only foreground mode. `assets/templates/CLAUDE_CODEX_GRAPH_WORKFLOW.template.js` launches one fresh cc-codex Agent per selected node and returns marked result candidates. Write missions use Agent worktree isolation; read-only reviews do not request write authority.
