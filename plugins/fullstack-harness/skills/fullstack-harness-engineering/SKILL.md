...
- That grouping is scoped, not general. A `push` whose target is any branch other than the resolved integration branch is not covered and needs its own authorization moment, and a `push` never reaches a protected default branch even when the run names one as its integration branch. `merge_pr` and `deploy` are outside the bundle entirely under this model: every merge lands on `main` and every release target is production, so both always need their own instruction. A repository that defines its own non-protected integration branch is the only case where a `merge_pr` can ride the loop, and then only for a PR whose base is that exact branch. An active RUN v10 must bind that exception to `landing.base_branch_protection`: the exact full branch ref, `unprotected` status, and the repository policy source. Missing, stale, malformed, or `protected` evidence fails closed and leaves the push or merge target outside the grouped instruction. Completed pre-evidence v10 history remains readable because it cannot dispatch another action.
