...
- `assets/templates/CLAUDE_CODEX_PREFLIGHT.template.js` proves a Claude Code parent can invoke `codex:codex-rescue` in read-only foreground mode. Before launch, `scripts/validate_codex_wave.py` must reload canonical PLAN v4/RUN v9 and emit the mission-only Workflow arguments after checking the preflight or allocated-wave authorizations. `assets/templates/CLAUDE_CODEX_GRAPH_WORKFLOW.template.js` launches one fresh cc-codex Agent per guarded write mission and returns marked result candidates. External Codex reviews are disabled in v1; select another allowed provider or defer.
