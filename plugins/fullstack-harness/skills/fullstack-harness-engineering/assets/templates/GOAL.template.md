...
For ordinary mission work, resolve the target repository's branch model first. Request the exact local branch, commit, review, and integration actions needed to land reviewed worktrees into its integration branch. Keep the run `local_only` unless the user separately requests a remote outcome; then authorize `push` for exactly `branch:<resolved integration branch>` and the current integration head. Under the main-only default, that target is the run's own `branch:refs/heads/codex/<short-name>`. The remote run ends at that push; landing the branch on the default branch is the user's own step, done outside the harness.
