...
For automatic Codex app-task fan-out, request this ordinary execution-intent bundle together with exact scope: `create_user_owned_tasks`, `create_app_managed_worktrees`, `create_local_branches`, `create_local_commits`, `integrate_locally`, and `push` restricted to `branch:<resolved integration branch>` (`branch:refs/heads/codex/<short-name>` under the main-only default). Outer app-task selection does not require or preauthorize `spawn_subagents`; an enabled nested policy requests an exact worker target only after that app task is allocated. This line requests approval; it does not grant it. Keep archival, cleanup, and any push outside the resolved integration branch as a separate ledger entry.
...
When an authorized selector result contains launch directives, do not finish by describing the wave. Follow its recorded runtime route. For Codex app threads, discover lazy-loaded project/thread tools before declaring them unavailable, resolve the project, and create one top-level left-sidebar task with its own app-managed worktree per selected mission. Record real identities and poll results. Nested direct-subagent work is optional in RUN v10: an omitted or disabled policy launches no nested children; an enabled policy is assigned only after the worker is allocated and requires an exact `spawn_subagents` grant for `worker:<id>` (never a run-wide wildcard), plus its task-local reviewer obligations. Never replace requested top-level tasks with coordinator-owned subagents. For Claude Dynamic Workflow, allocate the parent-managed branches/worktrees and invoke the `Workflow` tool once with the template asset as `scriptPath` and the wave as structured `args`; mission agents are flat siblings and are forbidden by this adapter from further delegation. Dynamic Workflow never waits for mid-run user input: return refinement/blocker state to the parent and start a later workflow after canonical state changes. If a preferred driver is unavailable, record the gap and use the deterministic fallback route.
