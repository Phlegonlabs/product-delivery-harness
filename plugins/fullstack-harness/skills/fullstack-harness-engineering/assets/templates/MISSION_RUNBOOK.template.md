...
Set `status: "complete"` only for execution intent after every source is frozen or accepted, every mission is integrated or superseded, every live task is `mission_recorded` with a PASS verifier, blockers and open waves are cleared, and `integration_head_sha` exists. In schema v9, every PLAN batch and final gate must also have a PASS result bound to that integration head with non-empty evidence. Required UI surfaces must have a PASS screenshot for every planned route, breakpoint, and state.
