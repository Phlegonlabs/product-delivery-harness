    ...
    "runtime_capabilities": {
      ...
      "runtime_adapter": {
        ...
        "version_gate": {
          ...
          "required_harness_version": "0.8.0",
          "session_id": null,
...
Use `scripts/harness_transition.py` for `pause`, `resume`, `cancel`, `reconcile-interrupted`, `reconcile-interrupted-reviews`, `record-review-attempt`, and `grant-review-attempts`. These transitions validate the full pair and replace RUN atomically. A resume fails while a worker is still recorded active. Use `scripts/render_review_packet.py` to produce the exact-head reviewer handoff with a bounded diff.
