...
Set `status: "complete"` only for execution intent after every source is frozen or accepted, every mission is integrated or superseded, every live task is `mission_recorded` with a PASS verifier, blockers, active or blocked mission/review workers, and open waves are cleared, and `integration_head_sha` exists. Pull-request mode must already be merged with current-head checks and review, and `merge_pr` authorization must cover every mission plus the exact `pr:<full-PR-URL>` target. PLAN-v4 graph state must have only succeeded, skipped, or superseded nodes and terminal edges, with no retained node blocker; route or supersede failed nodes before closeout. In schema v9, every PLAN batch and final gate must also have a PASS result bound to that integration head with non-empty evidence; a changed integration head invalidates an earlier PASS immediately in every RUN lifecycle state. Required UI surfaces must have a PASS screenshot for every planned route, breakpoint, and state.
...
When a Claude Code parent binds a graph node to cc-codex, first record observed plugin metadata with external-runtime status `unknown`. After exact `runtime:codex` and `worker:preallocation` authorization, run `scripts/validate_codex_wave.py --mode preflight` and pass its complete JSON to `CLAUDE_CODEX_PREFLIGHT.template.js`; record the runtime as available only after the marker succeeds. A write node uses `external_codex_agent`, `subagent`, `app_managed_worktree`, and `agent_result`, and also requires exact worker, worktree, and branch authorization for `spawn_subagents`, `create_app_managed_worktrees`, `create_local_branches`, and `create_local_commits`. It does not use `create_user_owned_tasks`. Allocate the node attempt, worker lease, worktree path, and branch ref, then run the guard in wave mode to emit canonical arguments for `CLAUDE_CODEX_GRAPH_WORKFLOW.template.js`. External Codex reviews are disabled in v1; select another allowed provider or defer. Once the marked result returns, independently verify its Git common directory, path, branch, head, ancestry, diff, scope, and verifier evidence before recording the result in RUN. Keep failed or cancelled runtime worktrees for diagnosis; do not reset or delete them automatically.
