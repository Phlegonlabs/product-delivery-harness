...
- For plan-backed shared-repository execution, inspect the full launch, review, and merge path at Plan Readiness. Request every missing branch, commit, integration, push, PR, review-management, and merge action once with exact scope, then record each approved action under its own ledger key. Include an exact repository-configuration action in that checkpoint only when the observed setup requires it.
...
- After every new push, wait for current-head CI, including the required E2E check, and request review again.
