...
- Ordinary plan-backed work starts `local_only` on the resolved integration branch and moves to `integration_push` when that verified head is pushed. The run is complete there; it does not wait for GitHub. Do not request or infer a protected-branch pull request at Plan Readiness. When the user separately asks for it, request each exact remaining landing action and bind PR creation, review, and merge to `future-pr:<owner>/<repo>:base=<resolved-base>:head=<resolved-head>`; after creation, verify the binding and append the exact `pr:<full-PR-URL>` target.
