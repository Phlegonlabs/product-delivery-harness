...
- Development Worker's Cloudflare dashboard Git connection watches branch: `<resolved-integration-branch>`.
...
- `run.integration.retention` must be `"persistent"` for this model — confirm `<resolved-integration-branch>` is not deleted between runs and `<resolved-protected-base-branch>` is protected from direct pushes.
