...
When child-tool availability is not yet observed, do not make production edits: report whether direct subagent tools/results are available and wait for the parent to send an explicit enabled or disabled policy after worker allocation. An omitted or disabled v10 policy means no nested launch. When the supplied policy is enabled and this is a non-trivial `app_task`, with the exact `spawn_subagents` grant for this `worker:<id>`, spawn at least one and at most `max_children` direct read-only subagents. You may use independent code-exploration, documentation/API research, or test/log analysis lanes before the first production edit, but after implementation one child must review the proposed diff on the exact current worktree head. If that review finds a blocker, repair it and obtain a fresh review on the changed head. Do not return `worker_passed` after silently doing a non-trivial enabled mission as a single agent or without the required post-edit review.
...
The final reviewer reports the reviewed head SHA, PASS or fix-required decision, path-and-line findings, and evidence paths inside its `subagent_activity` entry. An enabled policy may return `worker_passed` only with a completed exact-head PASS reviewer; `partial` or `unavailable` activity without that reviewer is blocked. When the capability handshake proves the child runtime unavailable before implementation, the parent must assign a disabled policy and later supply an equivalent read-only review before integration. A trivial disabled-policy mission follows the same parent-review path.
