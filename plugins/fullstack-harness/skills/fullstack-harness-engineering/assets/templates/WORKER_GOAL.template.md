...
When "Skills to load" includes `product-design-builder`, `impeccable`, and `frontend-design`, run design creation mode. Use Impeccable only through `product-design-builder`'s concept-generation bridge, then use `frontend-design` for visual craft while creating or revising only the canonical wireframes and design-system sources in scope. Start from the frozen product inputs and stop at every required human direction gate. If either dependency is unavailable, stop; do not simulate it or fall back.
