...
- Where the production Worker deploys from its own Cloudflare Git connection, the human's merge is the deploy trigger and the harness runs no deploy command. The exact production `deploy` grant still records the publication consequence. Leave `merge_pr` false because the harness only observes the external human merge.
