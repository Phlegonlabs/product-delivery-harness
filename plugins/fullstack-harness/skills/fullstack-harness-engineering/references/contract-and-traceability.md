...
- A design-source mission lists `product-design-builder` without `frontend-design`, or `frontend-design` cannot be loaded. Creation mode is blocked; it does not fall back.
...
- A current PLAN-v5 graph does not cover every mission's write scope with at least one review-type node (`backend_code`/`frontend_code`/`visual` as applicable). This graph-level review is the only review gate a run has, so it is never optional.
...
- Implementation work continues after a mission has already `integrated`, or after the run already pushed its integration head, and no new mission or task ID has been opened to cover it. Discovered follow-up work needs its own mission (a plan revision on the same PLAN/RUN) opened before edits begin, not an informal parallel checklist (for example a stray `tasks.md` outside `docs/goal/`) that duplicates the status PLAN/RUN already owns — two systems both claiming to be the record of what is done is itself the defect. When the new work is bigger than an isolated follow-up — anchored by a genuinely new upstream contract source rather than incremental discovery — see "Starting A New Plan vs Extending The Current One" above instead of opening just another mission on the old plan.
