...
Schemas v3 through v10 require a `landing` object. Under the default branch model, ordinary implementation, PRD, UI, branch, commit, and integration work uses `integration_push`: the verified integration head is pushed to the run's own `codex/<short-name>` branch, no PR exists, and the run is complete at that push. Use `local_only` when the run must not touch the remote at all; it cannot record a pushed head or PR and does not wait for GitHub CI or review, but every mission worktree still passes its exact-head pre-integration review. Use `pull_request` for the separate, user-started step that lands the run branch on `main`: its head is that same run branch and its base is `main`. RUN v10 may use `integration_pull_request` only when the target repository's own instructions define a separate integration branch that reviewed feature branches must enter through a PR. When the repository defines no other model, new RUN-v10 files set `integration.branch` and `landing.head_branch` to the run's own `codex/<short-name>` branch and `landing.base_branch` to `main`; otherwise record its existing branches. Older schemas retain their recorded branch names.

Do not put the later `main` landing into the ordinary mission run's early authorization bundle. When the user starts that landing, request every missing exact PR creation and review-management action for the resolved head and base branches; bind `create_pr` and `manage_pr_review` to `future-pr:<owner>/<repo>:base=<base>:head=<head>` until the PR exists. After creation, verify those branch fields, append the exact `pr:<full-PR-URL>` target, and run current-head CI and review continuously. A PR whose base is `main` follows the protected-base rule: stop merge-ready and hand it to the human unless a later explicit instruction names that exact existing PR; only that later instruction may supply `merge_pr` for the exact PR. When the human merges without asking the harness to perform it, leave `merge_pr` false and record the merged landing as observed external state. A PR into a separate integration branch the target repository's own instructions define may merge under exact `merge_pr` authorization only when that branch is not also the protected base. Earlier implementation or local-integration authorization never supplies either merge grant.

For every created PR, `pr_head_sha` equals `pushed_head_sha`, checks bind `checks_head_sha` to that PR head, and review binds `review_head_sha` to that PR head with zero blocking findings and unresolved threads. CI and review are independent sibling gates and should run or be observed concurrently for the current PR head. The remaining equality depends on the landing mode. For a `pull_request` landing on `main`, the PR head is the run branch's integration head, so a CI or review PASS and `merge_status: ready` require `checks_head_sha == review_head_sha == pr_head_sha == integration.integration_head_sha`. A later local integration changes that landing candidate and immediately makes its earlier CI, review, and ready evidence stale.
...
In schemas v4 through v9, `auto_merge_requested: true` records that GitHub auto-merge was successfully enabled for the exact `auto_merge_head_sha`. At request time it is valid only when `merge_status` is `ready`, the mode is `pull_request`, `auto_merge_head_sha == pr_head_sha == integration.integration_head_sha`, and an unexpired `merge_pr` authorization covers every run mission plus the exact `pr:<full-PR-URL>` target (or an explicitly run-wide `*`). RUN v10 keeps that equality for a `pull_request` landing on `main`; its `integration_pull_request` mode instead uses the feature-head equality above and forbids auto-merge when the resolved integration branch is also the protected base. Under the default branch model the `pull_request` base is always `main`, so auto-merge is never enabled at all. A schema-v6-through-v9 authorization that began with `future-pr:` must retain that binding, and the exact PR must match its repository, base, and head. Enable auto-merge only after current-head checks and configured review pass. Use an exact-head guard such as `gh pr merge --auto --squash --match-head-commit <sha>`. A new PR-head push, canceled request, or closed-unmerged PR resets `auto_merge_requested` to false and `auto_merge_head_sha` to null until fresh gates pass. Repository-level auto-merge configuration uses `configure_repository`; enabling it on a PR uses `merge_pr`.
