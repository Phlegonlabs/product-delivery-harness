...
`deployments.production.authorized_head_sha` records the integration head SHA that was current when `deploy:production` was authorized at Plan Readiness; it is required (non-null) once production status leaves `not_started`. Before the parent actually fires the production deploy, it compares this recorded value against the live SHA about to be deployed — see `cloudflare-deployment-lifecycle.md`'s Pre-Deploy Confirmation Checkpoint for the full SHA-drift and migration-destructiveness rules, including where that checkpoint moves under the Auto-Deploy Release Model.

Production `PASS` requires all development conditions to remain satisfied, `landing.pr_state: merged`, `landing.merge_status: merged`, `deployments.production.source_sha == landing.merged_sha`, migration `PASS` or `not_required`, production smoke `PASS`, evidence, and matching `environment:production` authorization. A complete schema-v7-through-v9 Cloudflare run requires both targets to pass.

At Plan Readiness, when full deployed delivery is requested, request the exact development and production targets once in the same checkpoint as the missing launch/landing actions, but record them under the separate `deploy` ledger entry. Under the default dispatched-Actions model, an unconditional push-triggered deployment bypasses this state model; use the exact-SHA dispatched Cloudflare workflow only after the parent rechecks authorization and gates. Under the Auto-Deploy Release Model, push-triggered deployment of the persistent integration branch is the sanctioned mechanism instead, and the harness's checkpoint moves to the merge into `landing.base_branch` — see `cloudflare-deployment-lifecycle.md`'s "Auto-Deploy Release Model (Alternative Trigger)".
