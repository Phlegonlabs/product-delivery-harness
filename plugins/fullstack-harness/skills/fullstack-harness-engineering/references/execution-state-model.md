...
This capability depends on the installed `codex:codex-rescue` Agent, not a repository-owned App Server client. A plugin path or version alone is not proof. Selecting it requires `invoke_external_runtime` for `runtime:codex` and `spawn_subagents`. A write mission also requires `create_app_managed_worktrees`, `create_local_branches`, and `create_local_commits`; it does not require `create_user_owned_tasks` because the Codex worker is a child Agent rather than a user-owned app thread.

PLAN v4 runtime-worker nodes may add `provider_options` for any provider in their `allowed_providers`. Each option uses the exact keys `model` and `reasoning_effort`. Model is a non-empty string or null. Reasoning effort is null or one of `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, and `ultra`; selectable effort is supported for Codex and Claude Code, while generic providers keep it null. The selector chooses the provider first, then attaches its options to one immutable runtime binding. A missing Codex option means the destination default; a missing Claude option means `sonnet`. For `external_codex_agent`, keep model and effort null unless PLAN explicitly selected them. The destination host still validates current model and effort support.
...
Per-worker axes may differ from the host axes for an external adapter. A write-capable `external_codex_agent` records `subagent` + `app_managed_worktree` + `agent_result`, while its Claude Code host remains `dynamic_workflow` + `parent_managed_worktree` + `agent_result`. The Agent runtime creates the isolated worktree, returns its path/ref/head as runtime evidence, and controls retention. The parent validates that evidence before recording the concrete allocation or integrating a commit. Read-only Codex reviews use `shared_checkout` and no write authority.

Claude Dynamic Workflow is a wave-level flat script, not a nested mission worker. The parent preallocates one worktree, branch, and lease per selected write mission for native or external-Claude routes. For external Codex, the outer workflow launches one `codex:codex-rescue` Agent per node and requests Agent isolation for write missions. A runtime review instead receives a read-only path, exact reviewed SHA, scope, and evidence contract. The workflow coordinates sibling agents and returns structured result candidates. Omit `nested_subagents` for these flat routes. If a workflow stage needs human sign-off, return a refinement/blocker result and run a later workflow after the parent updates canonical state; never let a workflow script or agent edit PLAN/RUN directly.
