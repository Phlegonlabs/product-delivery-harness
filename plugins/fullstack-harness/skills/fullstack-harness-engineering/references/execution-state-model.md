...
`merge_pr` and `deploy` dropped out of the bundle with the default main-only branch model: every merge lands on `main` and every release target is production, so both always need their own instruction. A repository whose own instructions define a separate non-protected integration branch is the only case where a `merge_pr` can ride the loop, and then only for a PR whose base is that exact branch. RUN v10 records that fact in `landing.base_branch_protection`, binding `unprotected` status and a repository policy source to the exact full base ref. Missing, stale, malformed, or `protected` evidence fails closed. Older active RUN-v10 files without this optional record need reaffirmation or a separate exact target source; only completed history without `action_target_contract` keeps the pre-evidence readability fallback because it cannot dispatch another action. Under the default model the grouped `push` reaches only the run's own disposable `codex/<short-name>` branch; a protected default branch is never in scope for it.
