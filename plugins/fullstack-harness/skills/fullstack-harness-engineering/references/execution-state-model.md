...
Targets use action-specific prefixes: `worker:`, `task:`, `worktree:`, `branch:`, `remote:`, `pr:`, `repository:`, `environment:`, `release:`, `runtime:`, `workflow:`, or `cloud-resource:`. The prefix pattern is shared across actions, so it does not by itself reject a target spelled for the wrong action or the wrong schema version; the action's own validation does. Schema v10 enforces the exact action-to-target-kind table. Schemas v2 through v9 keep their historical generic exact-target acceptance so existing RUN files remain readable. Schemas v6 through v9 also accept `future-pr:<owner>/<repo>:base=<base-branch>:head=<head-branch>` only for `create_pr`, `manage_pr_review`, and `merge_pr` before the PR exists. `expires_when` is `wave_closed`, `run_complete`, or `explicit_revocation` and is evaluated against current RUN state. Use `"*"` only for a dimension the user explicitly authorized run-wide, not as a placeholder for an unborn PR. A selector/coordinator treats missing, expired, or nonmatching scope as unauthorized; authorization is never a global boolean inferred for every mission or target.
