...
Production `PASS` requires all development conditions to remain satisfied, `landing.pr_state: merged`, `landing.merge_status: merged`, `deployments.production.source_sha == landing.merged_sha`, migration `PASS` or `not_required`, production smoke `PASS`, evidence, and matching `environment:production` authorization. A complete schema-v7-through-v9 Cloudflare run requires both targets to pass. Preserve deployment authorization evidence at completed-run closeout when its boundary was `run_complete`, just as exact-head merge evidence is preserved.
...
RUN records them together under `runtime_capabilities`, along with `max_parallel_workers`, a `platform_lifecycle` object, and optional backward-compatible `nested_subagents` and `permission_boundary` objects. Schemas v6 through v9 require `runtime_adapter`; v8 and v9 may also record proven `external_runtimes`. `platform_lifecycle` has `owner` (`parent` or `app`), `automatic_retention_cleanup_possible`, and `durable_branch_required_before_unique_work`.
...
A Claude Code parent records this separate cc-codex capability as `available` only after the isolated read-only foreground preflight succeeds:
...
```json
{
  ...
  "version": "<observed plugin version>",
...
This capability depends on the installed `codex:codex-rescue` Agent, not a repository-owned App Server client. Before preflight, record observed plugin metadata with status `unknown`; `unavailable` and a plugin path or version alone are not enough. `scripts/validate_codex_wave.py --mode preflight` requires exact `runtime:codex` and `worker:preallocation` authorization plus explicit `*` targets for app-managed worktree and branch allocation. The template runs the probe in an isolated Agent worktree. Only a successful marker promotes the record to `available`. Wave mode requires the exact worker target and explicit `*` allocation targets because Agent isolation assigns the concrete worktree and branch at launch. A write mission requires `spawn_subagents`, `create_app_managed_worktrees`, `create_local_branches`, and `create_local_commits`; it does not require `create_user_owned_tasks` because the Codex worker is a child Agent rather than a user-owned app thread.

PLAN v4 runtime-worker nodes may add `provider_options` for any provider in their `allowed_providers`. Each option uses the exact keys `model` and `reasoning_effort`. Model is null or a safe token matching `^[A-Za-z0-9][A-Za-z0-9._-]*$`. Reasoning effort is null or one of `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, and `ultra`; selectable effort is supported for Codex and Claude Code, while generic providers keep it null. The selector chooses the provider first, then attaches its options to one immutable runtime binding. A missing Codex option means the destination default; a missing Claude option means `sonnet`. For `external_codex_agent`, keep model and effort null unless PLAN explicitly selected them. The destination host still validates current model and effort support.

When the parent allocates a PLAN-v4 graph worker in RUN v8 or v9, copy a mission binding into `workers[].runtime_binding` or a read-only verifier binding into `review_workers[].runtime_binding`, with provider, driver, source, model, reasoning effort, and option source. A review worker also records node/attempt identity, graph revision, review path, and exact reviewed SHA; it has no mission lease, writable worktree, branch, or commit authority. For Codex app tasks, pass non-null values through task creation. For external Claude, split nodes into separate waves by model and effort and put both selected values in each wave request. For external Codex, use `provider: codex`, `driver: external_codex_agent`, and `source: external_agent`; split waves by model, effort, and tool profile, then pass only non-null options to cc-codex. Never silently replace a rejected model or effort; replan the affected node and increment the PLAN revision.

For every plan-backed multi-mission run, capture this adapter before the first production edit or worker launch. Capability observation and action authorization are separate facts: record a usable driver even when its launch actions remain false. In particular, do not omit `app_threads` because `create_user_owned_tasks` or worktree authorization is missing, and do not omit a proven `codex_rescue_agent` because external-runtime or worktree authorization is missing. Record the capability, request the launch bundle once at Plan Readiness, and rerun selection after the answer.

Provider means the host session running the Harness, not every CLI installed on the machine. Observe current-session native tools first: Codex project/thread creation and polling for `app_threads`, Claude Code's `Workflow` tool and supported runtime for `dynamic_workflow`, and current-session child-agent tools for `subagents`. Use `explicit` only when the host surface is opaque; otherwise use `generic` + `fallback`. A binary or plugin version may confirm feature compatibility after provider detection, but it does not select the provider or prove an external route.
...
Per-worker axes may differ from the host axes for an external adapter. A write-capable `external_codex_agent` records `subagent` + `app_managed_worktree` + `agent_result`, while its Claude Code host remains `dynamic_workflow` + `parent_managed_worktree` + `agent_result`. Before guarded launch, its worker record keeps `worktree_path` and `branch_ref` null because Agent isolation assigns them at runtime. After the result, the parent independently checks the reported path, branch, head, and Git common directory, then records the exact allocation before worker-result acceptance. External Codex reviews are disabled in v1; the selector must use another allowed provider or defer.

Claude Dynamic Workflow is a wave-level flat script, not a nested mission worker. The parent preallocates one worktree, branch, and lease per selected write mission for native or external-Claude routes. For external Codex, the parent runs the canonical guard and the outer workflow launches one `codex:codex-rescue` Agent per write mission with Agent isolation. External Codex does not run runtime reviews in v1. The workflow coordinates sibling agents and returns structured result candidates. Omit `nested_subagents` for these flat routes. If a workflow stage needs human sign-off, return a refinement/blocker result and run a later workflow after the parent updates canonical state; never let a workflow script or agent edit PLAN/RUN directly.

RUN may record the outer Claude Workflow task/run identity, script digest, node group, options, and status in `workflow_runs`. cc-codex foreground results do not expose a stable inner Codex thread ID, so leave `task_thread_id` null and never invent, poll, resume, or cancel that hidden identity. A retry creates a new graph attempt and uses `--fresh`.
