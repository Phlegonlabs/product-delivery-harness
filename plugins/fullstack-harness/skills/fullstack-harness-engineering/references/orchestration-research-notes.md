...
- Since Claude Code v2.1.172, the host may expose nested subagents. Current RUN-v10 deliberately does not use that capability: Dynamic Workflow remains flat, omits the nested helper policy, and requires worker results to report `subagent_activity: not_applicable` with empty `children`; legacy v6-v9 validation remains compatibility-only.
...
- Codex app tasks can expose delegation after a direct request or applicable skill/`AGENTS.md` rule. That host capability is not a current RUN-v10 permission: v10 app-task workers are sole writers and cannot start child agents. Legacy app-task behavior may be present in v6-v9 records only; any current explorer or reviewer is parent-dispatched as a sibling under the top-level authorization.
