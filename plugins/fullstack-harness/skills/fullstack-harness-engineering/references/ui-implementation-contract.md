...
8. Run `scripts/check_ui_contract.py` against the product source using `design-system.json` as the allowlist. Its source scan covers raw visual values, inline layout styles, page-local controls/surfaces, and call-site motion values. A filtered rule run is not a contract-clean signal by itself.
...
For design-source creation, load `product-design-builder` and `frontend-design` in creation mode; missing `frontend-design` blocks and has no fallback. For UI implementation, load `frontend-design` only when the user explicitly selected the new or high-impact visual surface and use conformance mode. A missing contract entry returns to `product-design-builder` as a design-input delta.
