...
| Content contract conformance | a product component renders contract-governed content | the `requiredContentOrder` in `design-system.json` and never-drop fields intact; limits, formats, empty and long-content rules respected | screenshot or content review |
