...
- Schema-v7 batch/final-gate IDs exactly match PLAN, and every result is PASS with evidence on the exact integration head.
...
- A schema-v4-through-v7 auto-merge request is recorded only after the same current-head landing gate passes, `merge_pr` covers the exact PR, and the request is bound to that PR head SHA. Any changed head resets the request before fresh CI and review.
