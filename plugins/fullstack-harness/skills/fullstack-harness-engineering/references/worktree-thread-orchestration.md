...
Default every plan-backed mission write to `parent_managed_worktree`, whether one mission is worked at a time or several run concurrently — `create_local_worktrees` authorization covers this even for a single sequential mission. Create each worktree from the recorded `batch_base_sha` on the resolved persistent integration branch. The primary integration checkout is a merge target, never a direct implementation surface: each mission's worktree branch receives an exact-head read-only review before it may merge there.
...
- Integrate only exact-head review-passing worker results serially into the persistent integration branch resolved from target-repository instructions. Worker branches and worktrees do not push or open PRs.
...
- In local-only mission work, `integration.branch` and `landing.head_branch` name the resolved persistent integration branch, while `landing.base_branch` names the resolved protected landing branch. If the repository defines no other model, these default to `development`, `development`, and `production`. A pull-request promotion starts only after explicit final user approval and then runs current-head CI and Codex review concurrently.
