...
A pull-request run is complete when the exact current-head CI and review gates pass on the same SHA, the verified PR is handed to the human, and RUN records PR URL, PR head, check head, review head, findings/threads, and evidence. Record a merged SHA only when the user separately authorized that exact merge and GitHub reports the PR merged. Report any unexecuted deployment or cleanup action separately.
