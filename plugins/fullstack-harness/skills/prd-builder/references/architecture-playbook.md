...
| Production | `production_head` after development passes | Production environment (Worker, for Cloudflare); production bindings, secrets, data, auth, and live payment credentials | Migration result when applicable, deployed URL/version, production smoke, monitoring signal, and rollback version |
...
- For Cloudflare delivery, name separate development and production Workers even though both use the same codebase. Define isolated bindings, secrets, data, auth, and payment modes plus the exact `pr_head` or retained `integration_head` to `production_head` promotion path.
