...
- When the Design System Need Gate is `required`, `design-system.md` and `design-system.json` publish together in the same approved move set. The JSON is the allowlist `fullstack-harness-engineering`'s contract check reads, so publishing one without the other leaves that check pointing at a stale allowlist. If only one is validated, stage both and wait rather than publishing half a pair. When the gate is `not_required`, publish neither file.
