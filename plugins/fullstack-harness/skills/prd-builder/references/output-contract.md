...
- All three core artifacts are present in the run-specific staging directory and are ready to publish under `docs/product/`.
...
- For a product with a mobile app or desktop app target, `architecture.md` records the required/selected stack or recommends one per `mobile-stack-selection.md`, separates platform, toolchain, distribution mechanism, backend/API integration, push/offline-sync approach, and testing as distinct layers, explains rejected alternatives, and records official-source verification date.
...
- Any unresolved mobile/desktop stack decision has an owner, deadline, time-boxed spike, and pass/fail criteria; a bare `TBD` does not pass validation.
