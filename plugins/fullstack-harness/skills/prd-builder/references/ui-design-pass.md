...
Choose iconography through an online lookup, not from memory. When the approved structure uses icons — navigation, actions, status, empty states — search for currently maintained icon libraries that fit the selected direction's character (outline versus solid, stroke weight, density), and verify each candidate's license, framework support, and maintenance status against its official source on the pass date. Recommend one primary icon set plus a named fallback, record them in the handoff's `Iconography:` line with the cited official-source URLs and retrieval dates, and mark an unevidenced pick `UNVALIDATED` — the same rule as market research. Skip the lookup only when the product uses no icons or no web search or fetch tool is available, and record which reason applied; never silently default to a remembered library.
