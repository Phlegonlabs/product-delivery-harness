...
- the exact approved selected HTML plus its human approval record, or an explicit `visual_direction_pass.status: "not used"` or `"rejected"` record; an `approved` payload includes one or two unique non-empty `representative_ui_ids`, two or three `candidate_directions` with a unique `direction_id` and one `html_path` per representative screen, `selected_html_path`, `approval_owner`, and `approval_evidence`; the workflow never launches while preference discovery, candidate comparison, selected-HTML consolidation, or approval is still pending;
