...
11. **Extract the system after approval.** Verify the selected files against the approved manifest, inventory actual repeated values and patterns in those bytes, then derive semantic tokens, closed primitive variants, product components, motion variants, recipes, registry entries, and final package mockups. Record source-to-token and source-to-primitive traceability. Candidate-only values from rejected directions do not survive.
