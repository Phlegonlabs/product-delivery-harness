...
if (Get-Command pi -ErrorAction SilentlyContinue) {
    ...
    # These adapter skills were merged into
    # full-harness/references/runtime-adapters.md and are no longer shipped.
    # A standalone copy left in the skills directory shadows the merged
    # reference, so it is backed up and removed rather than replaced.
    $retiredSkills = @(
    ...
    $retiredStandalone = @(
        $retiredSkills | Where-Object {
            Test-Path -LiteralPath (Join-Path $piSkillsDir $_)
        }
    )

    if (($standaloneSkills.Count -gt 0 -or $retiredStandalone.Count -gt 0) -and -not $ReplacePiStandaloneSkills) {
        $parts = @()
        if ($standaloneSkills.Count -gt 0) {
                    $parts += "Standalone Pi Harness skills can shadow the updated package and were left unchanged: " + ($standaloneSkills -join ", ") + "."
        }
        if ($retiredStandalone.Count -gt 0) {
        $parts += "Retired standalone adapter skills shadow the merged runtime adapter reference and were left unchanged: " + ($retiredStandalone -join ", ") + "."
        }
        Write-Warning (
            ($parts -join " ") +
            " Re-run with -ReplacePiStandaloneSkills to back them up, replace the current skills, and remove the retired ones."
        )
    }
    elseif ($standaloneSkills.Count -gt 0 -or $retiredStandalone.Count -gt 0) {
        ...
        if ($retiredStandalone.Count -gt 0) {
            # Retired skills ship no replacement: back the standalone copy up
            # and remove it, restoring in reverse if a move fails partway.
            $retiredMoves = @()
            try {
                foreach ($skillName in $retiredStandalone) {
                    $destinationSkill = Join-Path $piSkillsDir $skillName
                    $backupSkill = Join-Path $backupRoot $skillName
                    Move-Item -LiteralPath $destinationSkill -Destination $backupSkill
                    $retiredMoves += [pscustomobject]@{ Name = $skillName; Backup = $backupSkill; Live = $destinationSkill }
                }
            }
            catch {
                $unrestored = @()
                for ($i = $retiredMoves.Count - 1; $i -ge 0; $i--) {
                    $move = $retiredMoves[$i]
                    try {
                        if (-not (Test-Path -LiteralPath $move.Live)) {
                            Move-Item -LiteralPath $move.Backup -Destination $move.Live
                        }
                    }
                    catch {
                ...
                if ($unrestored.Count -gt 0) {
                    throw ("Removing the retired standalone skills failed. These were NOT restored and must be copied back from $backupRoot manually: " + ($unrestored -join ", ") + ". $_")
                }
                throw "Removing the retired standalone skills failed and the previous ones were restored. $_"
            }
            Write-Host "Removed retired standalone adapter skills. Backup: $backupRoot"
        }
    }
}
else {
