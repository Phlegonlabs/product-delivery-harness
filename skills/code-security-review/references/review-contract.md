...
Use pass only when reviewed_sha is a full SHA, the checkout is clean, required evidence is present, at least one review tool or manual source review has status `passed` or `findings`, coverage gaps do not hide a blocking surface, no validated critical or high finding remains, and `exclusions` is an empty list. An all-skipped or unavailable tool set cannot PASS. Under the current contract, a security PASS cannot carry an exclusion or silently narrow the declared scope. Medium and low findings remain visible and follow the project's declared blocking policy. If no policy exists, do not silently suppress them; report them and state the decision rationale.
