...
For UI work, load `references/ui-implementation-contract.md`. Design-system compilation requires `design-system-compiler` and `frontend-design` after approved Product Definition and UI design contracts, wireframes, HiFi review, and `Design System Need Gate: required`. UI implementation uses `frontend-design` in conformance mode. UI intake and Style Integration are not implementation work. Select exactly one active visual route: a required validated pair or a `not_required` approved page-faithful target. A missing active-source entry is a design-input delta, not a local exception. Run broad regression and browser/UI checks after exact-SHA review and repair converge.
