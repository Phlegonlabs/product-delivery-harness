...
```json
{
  "harness_run": {
    ...
    "plan": {
      "id": "PLAN-<stable-id>",
      "revision": 1,
      "digest_sha256": "6eb48ef128e88d7a9dfac5a68ab7de223b2de24155360c9f58984e1d5509edb9"
    },
    "status": "draft",
    "intent": "plan-only",
...
RUN schema v11 has 12 independent action entries. Keep every entry false unless an explicit user instruction authorizes the exact action and target. `push` remains exact-branch/head-bound; `integration_push` records only the verified run-branch head. It never covers `main` or the retired `development` name. After RUN close, `main` promotion is a new action-time authorization under `branch-promotion-contract.md`. Legacy RUN schemas remain readable; do not copy weaker fields into a new run. RUN-v11 workers never delegate; all reviews are parent-dispatched graph nodes. The integration-stage `security` review loads the project's `code_security_verification` binding in a fresh sibling reviewer, covers every mission at the unified integration SHA, and is never skipped by tree identity. Host-native launch drivers — app tasks, workflow runners, direct subagents, and sequential parent execution — are transport choices, not execution routes; their tool profiles and model fields remain adapter-specific.

A separate explicit remote instruction remains required; keep the phrase `separate remote intent` in the checkpoint. `landing.pushed_head_sha` records only the run-branch head. The `dynamic_workflow` and `app_threads` adapters retain their typed `mission_write` profile; task creation `model` and `thinking` values pass only after the parent proves the route.
...
Include only when UI Design Intake or a `UX-*` trace exists. UI owner approval proves direction conformance only; usability evidence remains a separate gate.
