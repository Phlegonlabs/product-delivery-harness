...
- After RUN completion, prepare verified A only through the separately authorized archive-candidate protocol. It derives the run branch and C from the archive, verifies clean exact A, and checks remote pre-state. A human or trusted host performs the emitted exact `git --no-replace-objects push` without force; recovery then reads back A. The archived RUN grants nothing and local tooling never runs the push.
