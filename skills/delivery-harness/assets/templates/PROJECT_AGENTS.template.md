...
- After RUN completion, publish verified A only through the separately authorized archive-candidate protocol. It derives the run branch and C from the archive, verifies clean exact A, checks remote pre-state, pushes without force, and reads back A. The archived RUN grants nothing.
