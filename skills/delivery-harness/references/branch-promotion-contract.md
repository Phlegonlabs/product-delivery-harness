...
For Harness 0.38, `archive_run.py --apply` also requires absolute external `--anchor-out`. It writes closed `ARCHIVE_RECEIPT.json` in the archive and a matching immutable anchor outside the checkout, binding every pre-move source, C, branch, and observed `main`; failure rolls back. Commit only repository bookkeeping as A. Revalidate the archived PLAN/RUN, receipt, anchor, destinations, deterministic `docs/DOCUMENTS.md`, absent live sources, and unchanged outside paths. If separately authorized, `push_archived_candidate.py` uses `--archive-anchor <same path>` and keeps its request, pre-side-effect attempt, and receipt outside the checkout as one closed lifecycle; it returns `PENDING_TRUSTED_HOST_PUBLICATION` before the receipt is written. It never treats caller-supplied prose or a ticket/ref string as authority and never invokes `git push`. A human or trusted host must execute the exact `git --no-replace-objects push` without force, then `recover`/`verify-receipt` reads back exact A and closes the receipt. Agents and the local executor must not run the emitted argv without that trusted boundary. The archived RUN grants nothing.
