...
```text
Product: objective, users, workflows, must-have and security requirements, non-goals, success criteria
...
UI Design Intake: `ui-design.md` records the human decision owner, experience priority, guidance/control, density, interaction/layout, style, motion/media, references, validation depth, and decisions. Product Definition provides the required behavior and constraints; the UI Design Builder record is authority for how those requirements are structured and expressed. Never invent a missing UI direction during implementation.
Copy completeness: wireframes/5 retains the locale, typed strings and dynamic display sources. Internal structure checks precede direction choice; the owner reviews wording, structure and interactions with the complete HiFi. Historical wireframes/4 keeps its separate Copy Freeze and Wireframe Approval semantics.
Architecture: module boundaries, data model, API/action contracts, auth, permissions, security enforcement, side effects
...
- To start a new plan: first confirm the current PLAN/RUN reached the Closeout Bar. At C, dry-run and apply `archive_run.py` on the same run branch with exact `--expected-main`, `--main-ref`, and absolute checkout-external `--anchor-out`. It moves the coordination set into `docs/goal/archived/<YYYYMMDD-HHMMSS>-<run-id>/`, updates `docs/DOCUMENTS.md`, writes closed `ARCHIVE_RECEIPT.json`, and writes its immutable external pre-archive anchor; any failed step rolls back. Commit only repository bookkeeping as A and verify it. A new action-time authorization may then drive `push_archived_candidate.py --archive-anchor <same path>` to prepare an immutable request and attempt; the request binds the exact canonical push URL, observed allowed-signers policy/hash, and absolute verifier hash, while local tooling never treats caller prose/ref as authority or invokes `git push`. A trusted host independently reloads and revalidates that request, performs the exact URL no-force publication with sanitized config, writes detached signed execution evidence, and recovery verifies the evidence plus remote read-back before closing the receipt. Candidate gates and exact-A `main` promotion follow.
- Archive receipts inventory regular files. Empty optional directories are skipped deliberately. The transaction journal accepts only the PLAN/RUN/optional coordination allowlist and exact receipt-derived destinations. DOCUMENTS writes hold a transaction-wide destination lock and compare an inode/content version at the commit boundary. If an apply is interrupted, recover its durable `.ARCHIVE_TRANSACTION.json` with `python skills/delivery-harness/scripts/archive_run.py --repo-root <checkout> --recover <archive>`; recovery preserves any path whose identity or content is no longer transaction-owned.
...
- When the owner declares the project or initiative complete, archive on the same instruction after the Closeout Bar passes and the exact dry-run move list is authorized. Apply on the resolved run branch with the observed `main` binding, commit only the archive as A, and verify A before any remote action. Publish A to the run branch only through the separately authorized external request/attempt/receipt/evidence protocol: local tooling prepares and verifies the records, while a trusted host revalidates the immutable request, performs the exact URL no-force push with sanitized config, signs execution evidence, and recovery verifies it before reading back A. Never move `docs/product/`; the archive references its PRD only by the frozen source hash. If a candidate or preview check fails after A, freeze A and its evidence, create a fresh PLAN/RUN on the same non-default branch from exact A, import the prior verified scope plus repair and bind A's receipt/anchor and immutable prior publication state as historical inputs, require new grants, close C2, and archive a new-anchor A2. If A was published, A2 requires remote pre-state exactly A; if not, it must remain absent. Never amend A, rewrite its branch, or reuse its anchor/request/attempt/receipt/evidence. Activation, outcome review, and SEO run only after production verification.
...
| Wireframe | <wireframes.html path> | <hash or revision> | UI Design Builder / legacy human owner | draft / validated / approved_legacy / delta_accepted / revision staged | <UI-* screens, labeled regions, states, exact responsive set, sourced copy and display contracts, per-target layout, browser overlap/overflow check> |
| UI design contract | <ui-design.md path> | <hash or revision> | human UI decision owner | approved / provisional / blocked | <intake, copy locale and internal validation, motion/media, wireframe, Style Integration, Impeccable/H1-H9, Visual Approval, Design System Need Gate> |
...
For plan-backed work, the PLAN JSON `sources` array is canonical; the table is its human view. Legacy sources include `content_sha256`, an immutable `source_revision`, or both. Harness 0.38 joined Product/UI/Design sources require `content_sha256` and exact canonical kind/path; current repository bytes must match it, and a supplied full-SHA `source_revision` blob must match the same bytes too. Recompute the PLAN digest and invalidate old attempts whenever any source changes. URLs are never fetched as joined authority.
...
- Harness 0.38 always freezes the canonical PRD, architecture, and stack rows and runs the sibling `check_product_package.py --repo-root <root> --require-filled --require-approved` join. Missing files, a non-approved checkpoint, a `Recommended`/`Provisional` layer, a blocking question, or a blocked trust/AI/security gate stops implementation; raw marker presence never selects whether validation runs.
...
- `PRD.md` is the binding product-behavior and copy-responsibility source for UI work. In initial design or an active enhancement, `ui-design.md` records copy completeness, internal structure validation and visual decisions; `wireframes.html` projects affected structure and sourced copy. Freeze applicable digests. A new product decision returns to `product-definition-builder`; an active wireframe or visual mismatch returns to `ui-design-builder`. Routine maintenance follows the current accepted product and effective requirements without making historical design bytes current.
...
- In active design work, precedence is product rules and the PRD UI surface contract, then validated schema-5 or approved legacy wireframe, then approved UI target, required pair choices, components and page preferences. A mission may not resolve a conflict by picking the more convenient source.
- In active design work, a missing or `blocked` Design System Need Gate blocks implementation. For `required`, a missing or half-present pair is `missing` or `partial`. For `not_required`, a missing approved target or incomplete target scope is `missing` or `partial`. An in-scope route with no `UI-*` entry or matching validated schema-5 or approved legacy wireframe blocks that route. Managed maintenance keeps current PRD and PLAN route/state/target joins while preserving historical UI/Wireframe/HiFi/pair bytes.
- If a PRD UI surface omits its states, invariant `` `responsive` `` anchor, or schema-5 `` `copy` `` status, resolve the gap before implementation or mark the surface `UNVALIDATED`. In initial design and enhancement, PRD, validated `wireframes.html`, every PLAN UI surface, and a required current `design-system.json` use the same responsive set — at least three ascending viewports for web, or at least two size classes for native/desktop. Managed maintenance still checks current PRD and PLAN route/state/targets without joining a retained historical wireframe or pair to a later PRD. Historical `wireframes/2` and `wireframes/3` remain readable; new design work uses `wireframes/5` and requires internal structure and copy completeness before HiFi. `ui-design.md` records internal validation plus exact responsive set and browser matrix. Harness never infers a missing target or product string.
...
- An in-scope security requirement reuses its existing `PRD-*` in task `trace_ids` and its existing required `TEST-*` as the acceptance-matrix `test_id`. Add no `SECURITY-*` family, schema, ledger, role, or duplicate risk document.
...
- The PLAN's `ui_surfaces` and the PRD's `UI-*` surface contract agree exactly on IDs, each entry's one literal route, and states. A localized UI-bearing PRD keeps exactly one non-empty matched `ui-surface-contract` boundary pair around every `UI-*` heading in the document, plus exactly one backticked `route`, `states`, and `responsive` anchor per entry, while translating its human prose. When PLAN freezes PRD.md, wireframes.html, or design-system.json, `scripts/validate_harness_plan.py` requires the matching artifact flag and verifies its bytes against that source row's `content_sha256` before checking the semantic projection. Every frozen PRD row is parsed even when PLAN claims an empty `ui_surfaces` list, so deleting the PLAN projection cannot hide a PRD surface. The transition write path resolves the same frozen rows under `--repo-root` and runs the joins before accepting or integrating work. In initial/enhancement design, frozen `wireframes.html` bytes run through `ui-design-builder`'s full checker — reviewer shell, self-containment, filled data, validated schema-5 structure or historical approval, typed motion/media placeholders, and exact PRD join. Managed maintenance requires the frozen task-record Epic and unchanged historical design bytes; it still joins current Product Definition to PLAN route/state/targets, without treating old design artifacts as current approval.
...
- The canonical frozen Product package fails the full sibling checker or omits its PRD, architecture, or stack source.
...
- A mission's `required_skills` names `design-system-compiler` without `frontend-design`, a design-system-source write scope omits that pair, or a wireframe-source write scope omits `ui-design-builder`.
- A current Wireframe or HiFi authoring dispatch or worker lease omits either `ui-design-builder` or `frontend-design`. This action-time guard covers canonical, staged and registered custom design targets; it does not rewrite or invalidate closed PLAN/RUN history. The handoff carries the declared skills, and the actual writer loads them before authoring.
...
- For initial design or an active UI enhancement, UI Design Intake, Wireframe Validation, Style Integration, Impeccable HiFi review, H1-H9 result, or Visual Approval is missing, or its owner and evidence are unclear. Routine maintenance follows the current product and accepted changes without recreating these historical gates.
- For initial design or an active UI enhancement, the validated schema-5 wireframe contradicts the approved PRD UI surface contract, or the active approved UI target or required design system contradicts either source in a user-visible way. An unchanged legacy approved wireframe keeps its original approval meaning.
- An active required design-system pair fails `design-system-compiler`'s pair checker, run read-only from the repository root: `python skills/design-system-compiler/scripts/check_design_system_pair.py --markdown <design-system.md> --registry <design-system.json> --repo-root <repository-root> --require-filled`. Never run it with `--write` from the harness — that edits a frozen source. Route an active pair failure through `design-input-updates.md`; do not guess which file is right or implement against half a contract. A historical pair retained during routine maintenance is not made current merely because its files exist.
...
- A touched security boundary has no required PRD/TEST contract, control definition, or negative test; return `contract_gap` instead of improvising one.
...
- Implementation work continues after a mission is `integrated`, or after archive candidate A is published, without a new mission or task ID covering it. Open the follow-up through a plan revision before edits begin. A post-archive repair creates a new candidate and must repeat the archive-candidate and downstream exact-SHA gates.
