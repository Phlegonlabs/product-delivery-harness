...
- When the owner declares the project or initiative complete, archive on the same instruction after the Closeout Bar passes and the exact dry-run move list is authorized. Apply on the resolved run branch with the observed `main` binding, commit only the archive as A, and verify A before any remote action. Publish A to the run branch only through the separately authorized external request/attempt/receipt protocol: local tooling prepares and verifies the records, while a human/trusted host performs the exact no-force push and recovery reads back A. Never move `docs/product/`; the archive references its PRD only by the frozen source hash. If a candidate or preview check fails after A, freeze A and its evidence, create a fresh PLAN/RUN on the same non-default branch from exact A, import the prior verified scope plus repair and bind A's receipt/anchor as historical inputs, require new grants, close C2, and archive a new-anchor A2. If A was published, A2 requires remote pre-state exactly A; if not, it must remain absent. Never amend A, rewrite its branch, or reuse its anchor/request/attempt/receipt. Activation, outcome review, and SEO run only after production verification.
