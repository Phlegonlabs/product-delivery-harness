...
| UI design contract | <ui-design.md path> | <hash or revision> | human UI decision owner | approved / provisional / blocked | <intake, motion/media, wireframe, Style Integration, Impeccable/H1-H9, Visual Approval, Design System Need Gate> |
...
- `PRD.md` is the binding product-behavior source for UI work; `ui-design.md` owns approved structure and visual decisions, and `wireframes.html` is their interactive structural projection. Freeze all three digests. A product mismatch returns to `product-definition-builder`; a wireframe or visual mismatch returns to `ui-design-builder` and invalidates downstream targets, design-system inputs, and implementation planning until reapproved.
...
- If a PRD UI surface omits its states or invariant `` `responsive` `` anchor, resolve the gap before implementation or mark the surface `UNVALIDATED`. PRD, approved `wireframes.html`, every PLAN UI surface, and — when present — `design-system.json` use the same set — at least three ascending viewports for web, or at least two size classes for native/desktop. Historical `wireframes/2` and `wireframes/3` remain readable; new work uses `wireframes/4`. `ui-design.md` records the exact set and browser matrix. Harness never infers a missing target.
