...
- When the owner declares the project or initiative complete, archive on the same instruction after the Closeout Bar passes and the exact dry-run move list is authorized. Apply on the resolved run branch with the observed `main` binding, commit only the archive as A, and verify A before any remote action. Publish A to the run branch only through the separately authorized external request/attempt/receipt protocol, then run candidate gates and separately promote exact A to `main`. Never move `docs/product/`; the archive references its PRD only by the frozen source hash. Activation, outcome review, and SEO run only after production verification.
