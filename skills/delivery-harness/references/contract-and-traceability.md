...
UI structure and copy: routes, screens, navigation, regions, data-to-UI mapping, states, Copy Freeze, exact static/action/alternate-state copy, dynamic display contracts
...
The `PRD.md` UI surface contract remains the product authority for structure, behavior, and copy status, and approved `wireframes.html` is its structural interactive and Copy Freeze projection. `PRD.md`'s approved UI Design Handoff names the active visual route: either a scoped immutable page-faithful target when the Design System Need Gate is `not_required`, or a frozen design-system pair when it is `required`. Harness consumes that active route, never candidate previews, and preserves the Copy Freeze verbatim. A wireframe-only package with `Visual design phase: not requested` is a valid PRD stopping point but is not yet a visual implementation source.
...
| Approved wireframe | <wireframes.html path> | <hash or revision> | human copy and product/design owners | draft / approved / delta_accepted / revision staged | <Copy Freeze and --require-copy-approved evidence first; later structural decision, UI-* screens, states, responsive layouts, browser check> |
...
- `PRD.md` is the binding structural, behavioral, and copy-status source for UI-bearing work; `wireframes.html` is its interactive structure and Copy Freeze projection. Freeze both digests and the PRD's separate Copy Freeze and later Wireframe Approval records. The Copy Freeze decision and checker evidence must precede UI grading, browser layout review, and structural approval on the same HTML hash. A mismatch, structural change, wording change, or dynamic display-contract change returns to `product-definition-builder` and invalidates downstream direction selection, previews, the approved UI target, design-system inputs, and implementation planning until the revised HTML passes both ordered gates.
...
- If a PRD UI surface omits its states, invariant `` `responsive` `` anchor, or invariant `` `copy` `` anchor, resolve the gap before implementation or mark the surface `UNVALIDATED`. PRD, approved `wireframes.html`, every PLAN UI surface, and — when present — `design-system.json` use the same responsive set — at least three ascending viewports for a web package, or at least two size classes for native or desktop; historical wireframes/2 and wireframes/3 files stay readable but must not be re-frozen as new packages. A schema-4 wireframe also requires matching approved copy anchors and Copy Freeze. The target-conformance handoff records the exact set and its passing browser matrix. The harness carries no default set and never infers a missing target or wording.
