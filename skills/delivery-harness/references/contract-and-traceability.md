...
Copy Freeze: `ui-design.md` records the human copy owner, primary locale, approval date, and approved status before structural approval; schema-4 wireframes bind exact static/action/feedback/alternate-state copy and dynamic source/order/format/count/length/fallback contracts.
...
| Approved wireframe | <wireframes.html path> | <hash or revision> | human decision owner | draft / approved / delta_accepted / revision staged | <UI-* screens, labeled regions, states, exact responsive set, exact copy and display contracts, per-target layout, browser overlap/overflow check> |
| UI design contract | <ui-design.md path> | <hash or revision> | human UI decision owner | approved / provisional / blocked | <intake, Copy Freeze owner/locale/date, motion/media, wireframe, Style Integration, Impeccable/H1-H9, Visual Approval, Design System Need Gate> |
...
- `PRD.md` is the binding product-behavior and copy-responsibility source for UI work; `ui-design.md` owns Copy Freeze plus approved structure and visual decisions, and `wireframes.html` is their interactive structural and copy projection. Freeze all three digests. A product, wording, or display-contract mismatch returns to `product-definition-builder`; a wireframe or visual mismatch returns to `ui-design-builder` and invalidates downstream targets, design-system inputs, and implementation planning until reapproved.
...
- If a PRD UI surface omits its states, invariant `` `responsive` `` anchor, or schema-4 `` `copy` `` status, resolve the gap before implementation or mark the surface `UNVALIDATED`. PRD, approved `wireframes.html`, every PLAN UI surface, and — when present — `design-system.json` use the same responsive set — at least three ascending viewports for web, or at least two size classes for native/desktop. Historical `wireframes/2` and `wireframes/3` remain readable; new work uses `wireframes/4` and requires Copy Freeze before structural approval. `ui-design.md` records the copy gate plus exact responsive set and browser matrix. Harness never infers a missing target or product string.
...
- UI Design Intake, Wireframe Approval, Style Integration, Impeccable HiFi review, H1-H9 result, or Visual Approval is missing for UI-bearing work, or its owner and evidence are unclear.
