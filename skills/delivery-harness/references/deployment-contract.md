...
- When `docs/product/architecture.md` is available, `docs/DEPLOYMENT.md` also carries **Release Target Status**, one row per exact architecture target. The architecture `release_targets.py` parser is authoritative: no invented, missing, or duplicate target IDs may pass. The `Provider / channel` cell is the exact stage-specific `Provider;Exact channel / track` pair. Production hosted web/API targets and every publicly discoverable hosted target require a credential-free HTTPS URL or hostname. Plain HTTP is valid only for a non-public development target on `localhost` or an IP loopback address. Worker/job/webhook/realtime rows require a substantive typed resource identity. Native/browser-extension PASS rows allow only a credential-free HTTPS listing/download URL, exact `n/a — artifact-only:<artifact>; no network endpoint`, or (only when artifact is `n/a`) exact `n/a — channel-only:<Provider;channel>; no network endpoint`. A current PASS binds Release target ID, surface, stage, provider/channel, target-specific endpoint/domain, Expected SHA, Deployed SHA, artifact/build identity, availability evidence, checked time, and status, with Expected equal to Deployed. Artifact `n/a` is valid only when the architecture target's `Artifact kind` is exactly `no independent artifact`; all other targets require a concrete artifact/build identity.
