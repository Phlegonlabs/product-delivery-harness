...
After RUN close, `archive_run.py` writes closed `ARCHIVE_RECEIPT.json` plus a required immutable checkout-external anchor from validated pre-move bytes, then moves coordination. An authorized bookkeeping commit creates A. `push_archived_candidate.py` requires that same anchor, verifies the archived PLAN/RUN and exact C-to-A change, then prepares external request/attempt records and a no-force argv for a human or trusted host; local tooling never pushes, and recovery closes the receipt only after exact read-back. Candidate gates and exact-A `main` promotion follow. A post-A failure starts a fresh continuation PLAN/RUN from exact A on the same non-default branch, with one frozen `prior archive candidate` source bound to A's archived receipt blob; it closes replacement C2 and archives A2 with a new anchor, never rewriting A or reusing A's records.
