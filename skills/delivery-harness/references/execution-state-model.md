...
Reserve every non-mission graph attempt before its side effect with `scripts/harness_transition.py --plan <PLAN.md> --run <RUN.md> --session-id <id> reserve-node-attempt --node-id <id> --attempt-id <id> [--evidence <text>]`. This covers `approval`, `external_wait`, `lifecycle`, and deterministic batch/final (`local_command` or `harness_parent`) verifier nodes. For a local verifier, also pass `--repo-root <root> --request-out <file>`; RUN persists the exact branch, head, checkout, complete request, request digest, attempt nonce, and any required PLAN-bound container preflight before the output file is created. If exclusive output creation fails, that durable request remains recoverable under `attempt_log[].verifier_dispatch.request`. Request and result artifacts stay outside the reviewed checkout. `verifier_runtime.py --request <file>` verifies the branch, HEAD, clean status, tracked-file fingerprint, and declared execution identity before invoking the command. Container execution additionally checks runtime path/hash/version and image RepoDigest; its preflight is part of the execution key and retained sandbox evidence. Host execution records its local executable identity and never claims container isolation. The RUN and declared generated tasks paths excluded from dirty status remain protected: the runtime hashes their exact bytes and fingerprints each file identity before and after execution, then attests the starting hashes. Finish the same receipt with `record-node-result --node-id <id> --attempt-id <id> --outcome <outcome> --evidence <text>` (repeat `--evidence`; add `--blocker` for a blocker and exactly one `--verifier-result` for a completed local verifier). Result recording with verifier evidence also requires `--repo-root`, rehashes the protected RUN, rechecks the live checkout, and rejects a different root, request, reservation, runtime identity, retargeted head, or changed coordination file. If a local verifier was interrupted before producing a result, close it as `blocked` with explicit evidence and a blocker but no result file. The transition derives the graph phase from the declared outcome and records only lifecycle evidence; it never executes the lifecycle action itself.
...
- `lease-worker`: `--mission-id`, `--node-id`, `--worker-id`, `--lease-id`, `--attempt-id`, `--branch-ref`, and `--worktree-path`. Optional `--provider`, `--driver`, `--model`, `--reasoning-effort`, `--worker-runtime`, `--workspace-mode`, and `--completion-channel` are compatibility assertions and must match the selector-derived binding. Optional `--task-thread-id` is required only for `app_task`. The transition materializes exact lease targets only from an active wildcard grant without changing authorization.
- `reserve-node-attempt`: `--node-id`, `--attempt-id`; optional repeatable `--evidence` records the pre-side-effect receipt. A local batch/final verifier also requires top-level `--repo-root` and may use `--request-out` to write its directly executable verifier request.
- `record-node-result`: `--node-id`, `--attempt-id`, `--outcome`, repeatable `--evidence`; optional repeatable `--blocker` and `--verifier-result`. It requires the matching reserved attempt and derives the graph phase from the declared outcome.
- `record-observation`: requires global `--repo-root`; optional `--available-worker-slots <n>`, `--isolation-capacity <n>`, and `--capacity-evidence <text>` must be supplied together from current host observations. Non-negative counts are recorded with an attempt-log entry; omitted flags preserve existing capacity and do not discover agent slots.
...
- `record-review-attempt`: `--lineage`, `--attempt-id`, `--worker-id`, `--result`, repeatable `--evidence`; bind the reviewed tree with `--tree-sha` or `--repo-root`; optional `--mission-id`, repeatable `--finding`, `--failure-family-id`, `--failure-primitive`, repeatable `--equivalence-class`, `--strategy`. A `security` node requires `--security-result <json>` and derives findings from that validated object instead of `--finding`.
...
- `reconcile-candidate-head`: `--candidate-sha`, `--source`, `--repair-node-id`, `--repair-attempt-id`, `--repair-task-id`; requires global `--repo-root` and a held `--session-id` lock. Records an already committed, authorized repair after mission integration; it never edits Git or grants repair authority.
...
Retained execution history and current PASS coverage are separate checks. Earlier executions keep their original checkpoint, attempt, lease and checkout facts; a later task commit, integration head or worktree observation does not rewrite them. Historical integration executions must belong to recorded integration history. Current task and gate PASS coverage still requires the applicable current lease and exact checkpoint or integration SHA. An old PASS is history, not permission to skip a fresh check on changed code.
...
PLAN authoring is a separate owner-gated discipline from a running RUN. A parent may draft and revise PLAN content, then validate that candidate with `scripts/validate_harness_plan.py` (and result validators where applicable) before treating a new revision as authoritative. Validators are read-only diagnostics: passing one proves the proposed bytes are well formed; it does not authorize a rewrite, recompute retained history, rebind a grant, erase receipts or attempts, or replace lock/receipt/exact-SHA evidence. Routine RUN operations supported by `harness_transition.py` use those guarded subcommands and their durable lock, live-state recheck, receipts, attempts, and atomic replacement. Initial readiness, capability/authorization recording, formal revisions, and final closeout still need their documented parent-owned procedure; do not invent a subcommand or use those authoring steps to bypass an available transition. There is no general “validate and write back” permission for RUN.

The tools have no guarded formal PLAN-revision mutation. If a substantive revision is required while a RUN is live, stop new dispatch at a safe boundary, reconcile active workers and reviews, preserve every worktree, dirty state, failed attempt, receipt, and historical RUN record unchanged, release the run lock when the session ends, and obtain a fresh owner instruction for the new revision and its exact actions. Do not edit the live RUN to make old grants or evidence look current. Prepare the revised PLAN as a new candidate, validate it, and continue only through the documented revision/recovery procedure with exact new authorization, or a newly authorized run pair. Keep an unchanged copy of the previous pair and review the complete state diff before writing; a validator PASS alone is insufficient. Never reset attempt counts, discard verifier records, or relabel an old result as new-revision evidence.

The non-canonical tasks view is projected from validated RUN state at fixed checkpoints, not on heartbeat or lease churn. `new_run.py --out <RUN.md> --repo-root <target-root>` writes the initial view. `accept-wave`, `record-worker-result`, `reject-worker-result`, `record-integration`, `reconcile-candidate-head`, `reconcile-interrupted`, `reconcile-interrupted-reviews`, and `close-wave` refresh the exact `docs/tasks.md` path declared in `integration.coordination_paths` when their top-level invocation supplies `--repo-root`. Projection reuses the transition's validated pair, preserves the Update Log, and refuses a non-generated file or a path outside that repo root. If projection fails after RUN commits, the command reports a distinct warning and the exact standalone repair command; it does not roll back RUN or report the transition failed. Run `scripts/render_tasks_view.py --plan <PLAN.md> --run <RUN.md> --out docs/tasks.md --repo-root <target-root>` to repair, or add `--check` to diagnose staleness.

Only RUN and the declared `docs/tasks.md` carrying this run's generation marker/identity are coordination exceptions to the clean-checkout check. A hand-authored, undeclared, retargeted, or other-run view remains ordinary dirt, as do product files. Local verifier requests include both allowed-dirty paths in their protected hashes and file-identity checks; changing either during execution or before result recording rejects the evidence. No coordination commit is needed between checkpoints. Automatic refresh does not rewrite a product README. After an authorized formal PLAN revision or closeout outside these transitions, explicitly render and check the view.
...
## Recover Before Creating Another RUN

For an existing delivery, inspect the canonical PLAN/RUN, live parent Git state and owning runtime before starting work. Match run ID, PLAN revision/digest, mission, task, attempt, lease and exact SHA; a native task title or a prose completion note is not that binding. Unknown process state requires observation, not another launch.

Keep an unchanged PLAN in the same RUN through retries and authorized repairs. Preserve completed missions, original receipts and cumulative budgets. Use guarded recovery transitions; never hand-reset succeeded gates, copy old PASS evidence to a new SHA, or open a replacement RUN to escape a checker or exhausted budget. A new initiative, substantive PLAN revision or incompatible legacy binding follows the explicit revision/replanning boundary and preserves the old pair unchanged.

Separate historical evidence from current coverage. Reconcile only the parent and currently relevant workspaces, then reserve a fresh attempt for each required check on the current candidate. A failed or blocked observation is not permission to skip reconciliation. Refresh the declared tasks view from RUN and verify it with `--check` at handoff. Native completion, manifest validity and exact-candidate verification are separate results.

## Resume Reconciliation Gate

The inspector reports live parent Git identity separately from recorded observations and the canonical integration identity. A branch/head mismatch or missing integration object needs reconciliation; a blocked Git authority check is an observation failure. Unknown Git or process state is never proof of alignment or liveness. This diagnostic summary does not replace formal PLAN/RUN validation.

A `running` RUN can be picked up by a different session than the one that last touched it — the same host resuming after an interruption, or a different agent entirely. Before that parent selects or launches any ready node, it must reconcile the observed working tree against canonical `task_states`/`mission_states`, not just trust the last recorded checkpoint. Start with `python "<delivery-harness-skill-root>/scripts/inspect_harness_run.py" --repo-root <target-root>` — it exits 1 when any mission needs reconciliation, and its summary shows the recorded run lock and control state. Check the owning runtime separately to establish whether that session is still live. Before dispatching work that consumes a project-bound skill slot, also verify that binding's pin with `python "<delivery-harness-skill-root>/scripts/check_skill_bindings.py" --stage <stage>` using `references/installed-commands.md`; choose the actual upcoming stage from the approved scope, never reuse a product-definition result for delivery. A mismatch is a review moment, not a silent swap.

The inspector's attempt-recovery summary is read-only. It counts only retained `kind: dispatch` entries whose recorded result is a declared failure and shows the most recent retained failure evidence in list order; it never infers counts from attempt-ID suffixes. `worker_running` and dirty output are not proof that a process is alive or dead: runtime process liveness remains unknown until observed by the owning runtime. Treat `integrated` or `superseded` drift as historical evidence, keep active-worker drift separate, preserve all retained attempts, grants, and verifier contexts, and keep the existing reconciliation flags and exit codes. Confirm interruption with the owning runtime before using `reconcile-interrupted` or `reconcile-interrupted-reviews` under the required RUN lock. Use `record-node-result` or `record-worker-result` only for a matching current reserved attempt or lease; new work gets a new attempt. Never rewrite or rebind old evidence.
...
A resume snapshot is usable only when the parent checkout path, branch, head, and clean/dirty state are all known. Enumerate linked worktrees, then reconcile the current parent and currently bound mission/review workspaces to their exact paths, branches, heads and clean state. A missing, dirty or mismatched relevant workspace blocks the frontier. Unrelated linked worktrees and retained historical workers do not become current dependencies merely because they appear in the inventory. Apply the same checks to a `ready` RUN that already contains active work. A stale RUN snapshot is not a safe default.
...
At resume, a worker recorded as `leased` or `worker_running` requires observation of its owning runtime before the parent treats it as live or interrupted. A phase alone does not block unrelated ready work during a live wave: passing siblings can enter streaming review while other verified bindings remain active. After a confirmed stopped review fan-out, use `harness_transition.py reconcile-interrupted-reviews` once with every affected review worker ID. The transition records each interrupted attempt as blocked, restores lineage counts from the append-only attempt log, resets the current review nodes, removes incoming edge traversals that have no current or retained source attempt, and persists `control.desired_state: paused`. It does not grant another review attempt or resume dispatch. When the interrupted worker was a security reviewer, the matching attempt receipt records `interrupted_review_reconciliation`; only that exact receipt permits the blocked worker to omit `security_result`. It is interruption evidence, never a security decision or PASS. After a later current reviewer supplies the exact structured PASS, the receipt-bound historical worker may remain without blocking closeout.

## Repairing The Integrated Candidate

For a repair within the unchanged PLAN after all missions have integrated, keep the existing RUN. After separately authorized edits, verification and commits, observe the clean parent checkout and run `harness_transition.py --plan <PLAN.md> --run <RUN.md> --repo-root <root> --session-id <id> reconcile-candidate-head --candidate-sha <full-live-HEAD> --repair-node-id <failed-node> --repair-attempt-id <failed-attempt> --repair-task-id <mission/task> --source <repair-evidence>`. The transition requires the exact observed non-default integration checkout, forward ancestry, the exact current failed gate/review attempt, one declared repair task and its write scope, existing exact commit/integration grants, no active workers or attempts, and remaining revalidation budgets. A completed RUN cannot be reopened by this command.

The old candidate enters `integration.prior_head_shas`; mission/task completion and historical receipts remain intact. Current gates and integration reviews must establish fresh coverage for the new head. Attempt counts and authorization grants are preserved, including exhausted limits. Reconcile and rerun the affected current gates instead of replaying completed missions, replacing the RUN, or relabeling old evidence. A change to product scope or PLAN still follows the formal revision procedure.

This recovery accepts a concrete `fix_required` or `retryable_failure` verifier result, or an integration review's `fix_required` result. A `contract_gap` or an unexplained `blocked` result is not a repair grant. An inactive routed integration review already marked `skipped` cannot carry an old candidate's tree proof forward; candidate reconciliation refuses that unsupported graph shape without changing RUN. Resolve the graph through the formal refinement boundary instead of keeping a stale tree hash or silently superseding the review.
...
Non-mission nodes use a two-step attempt invariant: `reserve-node-attempt` changes a dispatchable `approval`, `external_wait`, `lifecycle`, or deterministic batch/final verifier node to `running` and appends a `node_attempt` receipt; `record-node-result` may terminate only that matching running receipt. Its phase projection is deterministic — `pass` becomes `succeeded`, `blocked` or `contract_gap` becomes `blocked`, and `fix_required` or `retryable_failure` becomes `failed` — while the declared outcome remains the route signal. Evidence is required at result time, and a lifecycle result records what happened outside the lock rather than invoking the lifecycle action. A stopped or otherwise interrupted non-runtime attempt is completed as `blocked` through `record-node-result`; approval, external-wait, and deterministic-verifier `retryable_failure` may retry directly while budget remains, but lifecycle retry requires an explicit bounded route because its side effect may be unknown.
...
Run `python skills/delivery-harness/scripts/harness_transition.py --plan docs/goal/PLAN.md --run docs/goal/RUN.md --repo-root <absolute-root> record-observation` before `select_ready_nodes.py` for PLAN v6 with RUN v11. Global flags precede the subcommand. The transition probes every declared container runtime/image and durably records the canonical executable path, executable SHA-256, version-output SHA-256, machine trust/ownership proof, exact RepoDigest, host fingerprint, PLAN revision, and digest in RUN; `validate_harness_plan.py --probe-sandboxes` is diagnostic only. `accept-wave` and selector readiness reject missing, unavailable, stale, host-mismatched, or substituted observations. Every authority-bearing Git read disables replacement objects and rejects replace refs plus legacy graft metadata before trusting a SHA. The selector's strict current-pair entrypoint rejects other schema pairs before compatibility dispatch. Every task, worker, mission-integration, batch, and final verifier declaration explicitly selects `execution.isolation: "host"` (the new default) or `container`; missing/live/git-snapshot modes remain invalid. A host-only PLAN retains the host/PLAN observation but needs no Docker/Podman probe. Host verifiers run fresh and serially per runner in the declared checkout with source/Git guards; worktrees do not confine their operating-system permissions. Mutable, network, browser, and external checks use external-wait, lifecycle, or browser graph nodes. It computes graph readiness before runtime binding, then applies the existing write-conflict and worker-budget rules to ready mission nodes. The derived selector-only `execution_route` is `managed_sequential` for fewer than two actually selected safe write missions and `parallel_graph` for two or more; `runtime_driver` remains a separate transport fact. See `graph-orchestration.md` for the complete routing contract.

For container declarations, the retained runtime probe also includes the administrator-installed path's owner/mode and non-reparse proof; a missing or substituted trust object defers readiness before any container probe.
...
After RUN close, `archive_run.py` writes closed `ARCHIVE_RECEIPT.json` plus a required immutable checkout-external anchor from validated pre-move bytes, then moves coordination. An authorized bookkeeping commit creates A. `push_archived_candidate.py` requires that same anchor, verifies the archived PLAN/RUN and exact C-to-A change, then prepares external request/attempt records bound to the exact canonical URL, an observed allowed-signers policy/hash, and an absolute verifier hash; it returns a URL-only no-force argv for a trusted host, local tooling never pushes, and recovery closes the receipt only after detached signed execution evidence and exact read-back. Candidate gates and exact-A `main` promotion follow. A post-A failure starts a fresh continuation PLAN/RUN from exact A on the same non-default branch, with one frozen `prior archive candidate` source bound to A's archived receipt blob and immutable prior publication state; it closes replacement C2 and archives A2 with a new anchor, never rewriting A or reusing A's records.
...
Each mission lease and worker record repeats the lease ID, plan revision/digest, and batch base so stale results can be rejected without inference. The run-level `runtime_adapter` records provider/driver routing. Worker records name runtime/workspace/completion axes, the selected runtime binding when graph-backed, task/thread identity when applicable, worktree path, branch/ref, optional nested-subagent policy, optional report path, phase, and observed head SHA. `lease-worker` copies the selector's complete runtime binding and axes into that record, validates any compatibility flags, and accepts `task_thread_id` only for an `app_task`/`app_managed_worktree`/`thread_poll` binding. Exact worker, task, worktree, and branch targets are execution receipts materialized only from an already active wildcard authorization; lifecycle targets, especially push, must already be exact and are never materialized by the lease transition.
