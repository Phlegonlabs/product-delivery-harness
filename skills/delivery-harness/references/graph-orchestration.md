...
Before mission launch, `record-observation --repo-root <root>` probes each declared Docker/Podman runtime and records the exact pinned local RepoDigest, host fingerprint, and PLAN revision/digest in RUN. `scripts/validate_harness_plan.py --plan <PLAN.md> --probe-sandboxes` is a read-only diagnostic only; it does not satisfy the durable RUN preflight required by `accept-wave`. A missing, unavailable, stale, or host-mismatched observation blocks readiness and is never discovered only after implementation work has started. Every task, worker, mission-integration, batch, and final verifier declaration must carry the full `execution.isolation: "container"` policy. Mutable, network, browser, and external checks are modeled as `external_wait`, lifecycle, or browser nodes instead of local candidate subprocesses.
