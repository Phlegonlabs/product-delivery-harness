...
Task/worker, mission-integration, batch, and final verifiers use explicit `host` or `container` policies. New declarations default to host execution with fresh exact-SHA evidence and source/Git guards. Host mode needs no container runtime; selected containers still require current PLAN-bound runtime/image observations and never fall back to host. Worktrees and read-only declarations do not provide process confinement.

Harness 0.38 PLANs contain no `push` lifecycle node and RUN push stays false. Archive candidate A receives a post-RUN publication handoff through `push_archived_candidate.py`, using the receipt-bound external archive anchor plus external request/attempt/receipt/evidence. The immutable request carries the exact canonical URL and observed trust-policy/verifier hashes; local tooling never invokes `git push`, and a trusted host independently revalidates and signs execution evidence for the exact no-force argv before recovery reads A back. An explicitly pinned pre-0.38 recovery PLAN may retain its legacy exact-target push node; wildcard or protected targets remain invalid.
...
Before mission launch, `python skills/delivery-harness/scripts/harness_transition.py --plan docs/goal/PLAN.md --run docs/goal/RUN.md --repo-root <absolute-root> record-observation` records the host and PLAN revision/digest. It probes administrator-installed Docker/Podman runtimes and pinned image RepoDigests only for declared container verifiers. Global flags precede the subcommand. `validate_harness_plan.py --probe-sandboxes` remains diagnostic only. Missing or stale required observations block readiness. Every verifier explicitly declares `host` (the new default) or `container`; browser and external/mutable checks retain their own nodes and authorization.

The selector first computes the logical graph frontier, then resolves runtime bindings and action authorization. It applies the existing write-scope and runtime-resource conflict rules to ready mission nodes. Write-capable `runtime_worker` nodes draw from the minimum of PLAN capacity, runtime capacity, and currently available worker slots. Runtime reviewers and writers share the observed worker-slot budget; reviewers do not consume writer isolation capacity. Approval and external-wait directives do not consume an agent slot unless their declared executor launches one. Read-only nodes never bypass action authorization. The selector returns a derived selector-only `execution_route`: `managed_sequential` for fewer than two actually selected safe write missions and `parallel_graph` for two or more. This label is not a PLAN/RUN field and does not replace the separate `runtime_driver` transport binding. Non-runtime nodes remain explicit directives such as `run_verifier`, `await_approval`, `poll_external`, or `run_lifecycle_action`.
