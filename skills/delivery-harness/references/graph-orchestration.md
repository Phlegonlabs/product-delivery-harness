...
Current task/worker, mission-integration, batch, and final verifiers execute through the declared pinned local container policy. A missing or stale runtime/image RepoDigest observation defers the node until `record-observation` records a current PLAN-bound preflight; `validate_harness_plan.py --probe-sandboxes` is diagnostic only. A declaration-only read-only flag or copied Git snapshot is not process isolation; an external provider is only a future separately implemented route.
...
Before mission launch, `python skills/delivery-harness/scripts/harness_transition.py --plan docs/goal/PLAN.md --run docs/goal/RUN.md --repo-root <absolute-root> record-observation` probes each declared administrator-installed Docker/Podman runtime and records its owner/mode/reparse trust proof plus exact pinned local RepoDigest, host fingerprint, and PLAN revision/digest in RUN. Global flags precede the subcommand. `scripts/validate_harness_plan.py --plan <PLAN.md> --probe-sandboxes` is a read-only diagnostic only; it does not satisfy the durable RUN preflight required by `accept-wave`. A missing, unavailable, stale, or host-mismatched observation blocks readiness and is never discovered only after implementation work has started. Every task, worker, mission-integration, batch, and final verifier declaration must carry the full `execution.isolation: "container"` policy. Mutable, network, browser, and external checks are modeled as `external_wait`, lifecycle, or browser nodes instead of local candidate subprocesses.
