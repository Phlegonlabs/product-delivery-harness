...
Harness 0.38 PLANs contain no `push` lifecycle node and RUN push stays false. Archive candidate A is published only after RUN close through `push_archived_candidate.py`, using the receipt-bound external archive anchor plus external request/attempt/receipt. An explicitly pinned pre-0.38 recovery PLAN may retain its legacy exact-target push node; wildcard or protected targets remain invalid.
