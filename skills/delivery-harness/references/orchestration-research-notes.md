...
The portable default favors an isolated worktree per mission because it enables exact-head review before controlled integration into the run branch. The default branch `main` and retired name `development` remain outside ordinary worktree execution. Both initial and enhancement run branches start from observed remote `main`. After RUN close, the exact candidate completes every required candidate and isolated non-production-environment gate, then fast-forwards to `main` under separate authorization. Read-only work may fan out freely; parallel writes still require complete resource claims, observable completion, a fixed base, and explicit action authorization.
...
The skill uses a parent-to-mission-writer shape and keeps task implementation sequential within each mission. An earlier revision let an app-task mission writer use bounded direct subagents for independent read-only exploration, research, test analysis, and review. RUN v10/v11 removed that nesting: workers are sole writers, cannot start child agents, and any explorer or reviewer is parent-dispatched as a sibling (see the permission note below).

The selector now emits deterministic, tool-agnostic launch directives for selected missions. It remains read-only. In a Codex app session, project and thread tools may need to be found through the current tool-discovery surface before use. After accepting the wave, the parent creates one top-level app-managed worktree thread per selected mission, so each mission is a separate conversation in the left sidebar. Earlier revisions described each task coordinating its own bounded read-only Multi-agent helpers; under the current contract that coordination belongs to the parent, one worktree thread per mission. Coordinator-owned subagents are not a substitute for that outer task/worktree boundary. The parent polls completion and validates results. This model-driven tool loop is the portable in-app launcher. A separate App Server client is needed only for event-driven orchestration outside the interactive parent task.

In Claude Code with its native Workflow runner available, the selector also emits one wave-level launch bundle. The parent allocates mission branches/worktrees first, then runs a flat JavaScript workflow that starts sibling mission agents and returns structured result candidates. The workflow is an adapter over the same mission, authorization, result, and integration gates; it is not a second planning system.
...
- The Workflow runner has no mid-run user input. Human approval or contract refinement must end the current workflow and continue in a later workflow after the parent updates canonical state.
...
- Since Claude Code v2.1.172, the host may expose nested subagents. Current RUN-v11 deliberately does not use that capability: the Workflow runner remains flat, omits the nested helper policy, and requires worker results to report `subagent_activity: not_applicable` with empty `children`; legacy v6-v9 validation remains compatibility-only.
...
- Codex app tasks can expose delegation after a direct request or applicable skill/`AGENTS.md` rule. That host capability is not a current RUN-v11 permission: v10 app-task workers are sole writers and cannot start child agents. Legacy app-task behavior may be present in v6-v9 records only; any current explorer or reviewer is parent-dispatched as a sibling under the top-level authorization.
...
- Claude Code Workflow-runner version gate, workflow directory, `agent()`/`pipeline()` contract, result schemas, pause/resume behavior, tool inheritance, and current capacity ceilings.
