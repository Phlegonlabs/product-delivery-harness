...
- Since Claude Code v2.1.172, the host may expose nested subagents. Current RUN-v11 deliberately does not use that capability: Dynamic Workflow remains flat, omits the nested helper policy, and requires worker results to report `subagent_activity: not_applicable` with empty `children`; legacy v6-v9 validation remains compatibility-only.
