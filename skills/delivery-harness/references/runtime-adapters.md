...
6. Subscribe or block on terminal child/status events instead of fixed-interval polling. Record each terminal result immediately with `record-worker-result` and stream its ready pre-integration review while sibling workers continue; close the wave only after every selected worker result is recorded. Before the host deadline, request a checkpoint after the current tool returns when a slice is not converging; do not use timeout as the checkpoint. A missing process plus a dirty or advanced worktree is interrupted evidence, not `worker_running` proof.
