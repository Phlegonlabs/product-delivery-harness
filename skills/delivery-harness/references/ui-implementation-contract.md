...
Every route gets product behavior and its exact responsive set from `PRD.md`; implementation architecture and technology come from `architecture.md` and `stack-decisions.md`; approved `wireframes.html` supplies reviewed structure; and `docs/design/ui-design.md` supplies Style Integration, Motion and Media Intent, HiFi evidence, Visual Approval, and the Design System Need Gate. No downstream source overrides Product Definition.
...
1. Read `PRD.md`, `architecture.md`, `stack-decisions.md`, approved `ui-design.md`, the matching approved page in `wireframes.html`, and the active visual source before writing. A missing or mismatched route, state, responsive set, approval, target scope, browser evidence, component foundation, or styling approach is a blocker. A route whose PRD entry records SEO metadata implements exactly that `<title>`, meta description, canonical URL, and recorded extras; the rendered `<head>` is part of the deliverable. A missing SEO record is a PRD contract gap routed to `product-definition-builder`, never an implementation-time invention.
2. Follow the PRD's responsibilities and behavior plus the wireframe's region order, grouping, element inventory, state placement, and responsive rearrangement. Do not let the visual source change product structure.
...
5. Run the Final Visual Parity Loop across the recorded responsive set and states in normal and reduced motion. Every matrix entry is free of unintended overlap, clipping, occlusion, and horizontal overflow. Target-conformance compares the approved HiFi reference and implementation side by side; system-conformance checks the pair against real product source plus the screenshot matrix. Differences outside tolerance enter the bounded repair cycle. After parity closes, run the Impeccable final page-quality pass from `verification-gates.md`.
6. Stop when implementation requires unsupported authority. Route product behavior, architecture, or stack changes to `product-definition-builder`; route wireframe, style, motion/media, or UI-target changes to `ui-design-builder`; route pair-only changes to `design-system-compiler`.
...
For design-system work, load `design-system-compiler` and `frontend-design` in compilation mode. For UI implementation, load `frontend-design` in conformance mode. Do not reopen UI intake, Style Integration, motion/media selection, or visual direction during implementation.
