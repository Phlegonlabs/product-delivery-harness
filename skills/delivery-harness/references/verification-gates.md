...
- **Target-conformance mode (HTML target):** open the approved all-screens HTML reference in the browser, select the route, breakpoint, and state in its reviewer shell, capture it, and capture the implemented route at that same combination. Store both under `docs/goal/evidence/` (for example `parity/<route>-<state>-<breakpoint>-target.png` and `-actual.png`). Compare layout, element inventory, and observable visual values item by item within the PRD handoff's recorded tolerance. The reference render is the comparison source, not an aspiration.
