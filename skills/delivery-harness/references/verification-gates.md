...
| Content contract conformance | a UI route or product component renders wireframe- or design-system-governed content | exact static strings, action labels, and alternate-state messages match the approved Copy Freeze; dynamic examples are not hard-coded; dynamic source, order, format, count, length, fallback, empty, and long-content rules are respected; when a formal pair exists, `requiredContentOrder` renders, in that order — those fields never drop | screenshot plus DOM/text assertion or content review |
...
For a web mission, capture through a browser: Playwright/headless-browser screenshots, browser DevTools, or the `agent-browser` CLI, at the planned viewports. The route-by-breakpoint-by-state matrix and the runbook's web UI Evidence columns apply.
...
Record the run's accepted deviations in the RUN `deviation_ledger` — runs pinned to harness 0.35.0 or later are machine-checked: every `deviation` verdict's `differences` must appear as one ledger row (`surface_id`, `route`, `breakpoint`, `state`, `difference`, `citation`) citing the PRD handoff's allowed-deviation clause that admits it, and a row with no matching deviation is rejected. The E2E verification expansion's Deviation Ledger table may mirror it for humans; the RUN structure is authoritative. A difference that changes regions, states, actions, routes, responsive behavior, frozen wording, or a dynamic display contract is never inside tolerance; it is structural or copy drift and routes upstream as a design-input delta. When a later run accepts the same difference on the same surface again, cite the frozen design-input delta that authorized it or route the drift upstream; accumulated in-tolerance drift never substitutes for a doc update. Each citation is a recorded attestation (see the attestation caveat under `layout_check`).

Static screenshots never close a motion change. When a mission's diff touches transition, animation, duration, or easing code or CSS, its parity evidence adds a motion check row: the authority being followed (the PRD Motion Need Gate decision and its registered variant, or the approved target's observable motion), what the reviewer confirmed in normal and reduced motion, and the verdict. A motion change with no authority citation routes upstream as a design-input delta instead of passing on still images.
