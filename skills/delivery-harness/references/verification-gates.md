...
- The RUN `deviation_ledger` is complete — machine-checked on runs pinned to harness 0.35.0 or later: every accepted parity deviation is recorded with its allowed-deviation citation, no accepted deviation changes structure, regions, states, actions, routes, or responsive behavior, and any repeated deviation on the same surface cites its frozen design-input delta.
- The RUN `ui_impact_summary` is complete — machine-checked on runs pinned to harness 0.35.0 or later: every mission on a UI run carries its `none`/`style`/`structure`/`both` classification, and every `structure` or `both` row names the accepted upstream design-input delta.
