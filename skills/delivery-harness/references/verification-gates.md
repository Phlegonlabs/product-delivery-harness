...
- Harness 0.38 closes this gate locally at candidate C. It performs no RUN push; archive candidate A is later reverified and receives a separate checkout-external request/attempt/receipt handoff. Local tooling never invokes `git push`; a human/trusted host performs the exact no-force publication and recovery reads A back.
