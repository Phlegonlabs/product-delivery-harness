...
Record the run's accepted deviations in a deviation ledger. Every `deviation` verdict's `differences` carry into one ledger table — in the E2E verification expansion when one exists, otherwise in the RUN evidence notes — each row citing the PRD handoff's allowed-deviation clause that admits it. A difference that changes regions, states, actions, routes, or responsive behavior is never inside tolerance; it is structural drift and routes upstream as a design-input delta. When a later run accepts the same difference on the same surface again, cite the frozen design-input delta that authorized it or route the drift upstream; accumulated in-tolerance drift never substitutes for a doc update.
...
- The deviation ledger is complete: every accepted parity deviation is recorded with its allowed-deviation citation, no accepted deviation changes structure, regions, states, actions, routes, or responsive behavior, and any repeated deviation on the same surface cites its frozen design-input delta.
