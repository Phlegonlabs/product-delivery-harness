...
Container verifier results are never reused from a session cache. Any retained cache root remains repository-external for compatibility and is never a sandbox escape. The exact execution key binds run ID, PLAN revision/digest, graph revision, batch base, exact head, changed-file digest, trust domain, checkout role, cwd, ordered argv, pinned image policy, observed runtime executable/hash/version/RepoDigest, OS/architecture, pass signal, and selected environment-value digests. `session_exact` remains a legacy declaration shape but cannot bypass the container boundary.
