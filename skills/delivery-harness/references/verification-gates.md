...
- Harness 0.38 closes this gate locally at candidate C. It performs no RUN push; archive candidate A is later reverified and may be published through the separate checkout-external request/attempt/receipt protocol.
...
For a hosted-browser mission (`capture_mode: hosted-browser`), capture through a browser: Playwright/headless-browser screenshots, browser DevTools, or the `agent-browser` CLI, at the planned viewports. The route-by-breakpoint-by-state matrix and the runbook's hosted-browser UI Evidence columns apply. `scripts/parity_capture.py` is only this hosted-browser helper.
...
For `capture_mode: browser-extension`, `native`, or `desktop`, do not run `parity_capture.py` and do not claim URL parity. Capture the approved target and implemented surface with the platform-specific extension/native/desktop tooling (or an explicitly labeled manual capture), bind both to the accepted Git head, and record the platform capture method and exact authority source in each `target_comparison` row.
