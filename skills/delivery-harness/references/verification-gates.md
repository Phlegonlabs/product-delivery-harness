...
- Harness 0.38 closes this gate locally at candidate C. It performs no RUN push; archive candidate A is later reverified and receives a separate checkout-external request/attempt/receipt/evidence handoff bound to an exact canonical URL, observed allowed-signers policy, and absolute verifier hash. Local tooling never invokes `git push`; a trusted host independently reloads and revalidates the request, performs the exact no-force publication with sanitized config, signs execution evidence, and recovery verifies it before reading A back.
...
| Visual design | initial design or active enhancement has an approved UI source | matches the PRD UI surface contract, validated schema-5 wireframe (or approved legacy wireframe), and active visual source (required design-system pair or approved page-faithful target) within stated tolerance; routine maintenance checks the current accepted product and effective requirements | screenshots or human approval |
| Page-to-PRD conformance | an initial or enhanced route has a `PRD.md` `UI-*` entry and matching copy-complete validated wireframe | the built route matches PRD behavior and affected wireframe region order, grouping, element inventory, states, responsive rearrangement, actions, exact static/action/feedback/alternate-state wording, and dynamic source/order/format/count/length/fallback contracts at every required target; no unintended overlap, clipping, occlusion, or horizontal overflow; routine maintenance checks current affected behavior and regression consumers without regenerating historical wireframes | screenshots + browser geometry/reviewer note |
...
| Design-system pair check | the active initial/enhancement visual route requires a current pair | `python skills/design-system-compiler/scripts/check_design_system_pair.py --markdown <design-system.md> --registry <design-system.json> --repo-root <repository-root> --require-filled` exits 0 against the frozen pair, run from the repository root and never with `--write`; a failure is a stop condition per `contract-and-traceability.md`. A retained historical pair in routine maintenance is not revalidated against a later accepted PRD solely because it exists; preserve its bytes and check the current product instead | checker output |
...
Before the final source/Git checks, host execution ends its owned process group or job after success, failure, or timeout. Process ownership setup or cleanup failure cannot produce PASS. Windows starts the command suspended, assigns it to a Job, then resumes it. POSIX uses a new process group; host commands must stay in that group and must not daemonize with `setsid` or `setpgid`. Commands that need to escape that lifetime need the container route or a separately managed service. This manages verifier lifetime; it does not grant filesystem or network confinement.
...
Container verifier results are never read from a disk session cache. Within one batch, read-only task/worker declarations may opt into PASS reuse with `session_exact` and `deterministic_local: true`. Equivalent concurrent requests share one execution; each consumer freshly verifies its live guard and runtime/image identity and keeps its own reservation and attestations. The origin must arrive in the same parent-observed result batch, with matching context and output hashes; historical RUN rows cannot authorize new reuse. After acceptance, RUN retains both origin and consumer for later validation. Any retained cache root remains repository-external for compatibility and is never a sandbox escape. The exact execution key binds run ID, PLAN revision/digest, graph revision, batch base, exact head, changed-file digest, trust domain, checkout role, cwd, ordered argv, pinned image policy, observed runtime executable/hash/version/RepoDigest, OS/architecture, pass signal, and selected environment-value digests. The in-memory cache also binds the canonical checkout and protected Git inputs; it cannot bypass the container boundary.

Only PASS with exit code 0 is reusable. A changed input, dirty checkout, malformed entry, failure, or timeout runs the command fresh. A failed origin releases waiting consumers without caching its result. Logical attribution — verifier ID, layer, mission, task, attempt, and lease — is not in the execution key, so equivalent opted-in task and worker gates may cite one exact execution while each keeps its own PASS record, context, and evidence key. Never use this cache for runtime review, browser capture, migration, mutable-environment smoke, network/shared-database checks, or time/random-dependent commands.
...
Capture the full matrix once on the mission's worktree head. After serial integration, recapture the full matrix bound to the integration head only when the integration diff intersects that surface's write scope; when it does not, rebind the existing evidence to the integration head and record why. A merge that touched only backend files cannot change a rendered UI surface, and recapturing the whole route-by-breakpoint-by-state matrix to prove that is the slowest verifier class in the ladder. A merge that did touch the surface recaptures in full. Schema-v9 `ui_evidence` records the surface ID, route, breakpoint, state, repo-relative image path under `docs/goal/evidence/`, lowercase SHA-256, exact integration head SHA, and status. Harness 0.38.0+ rows additionally record the PLAN `capture_mode`-compatible `capture_method` and a complete immutable `target_comparison`: `baseline`, exact `authority_sources` path/hash identities, a real hashed `baseline_artifact` image under `docs/goal/evidence/`, its `baseline_artifact_sha256`, verdict, and differences. The closeout validator checks the matrix, current-head binding, file existence, non-empty image signature, and hash. Runs pinned to harness 0.34.0 or later also record `layout_check` on every row: `pass` from a real-browser DOM geometry scan at that breakpoint and state — no unintended element overlap, clipping, occlusion, or horizontal overflow over visible peers, containers, and viewport bounds, the same criteria as the grading rubric's Technical Hard Gate; `fail — <selectors>` records the offending elements and blocks the gate until the surface is repaired and the entry recaptured; `manual — <basis>` records a labeled manual review or native UI-test layout assertion; `n/a — <reason>` records why no geometry check applies. A `layout_check` value — like a `deviation_ledger` citation or a `ui_impact_summary` row — is a recorded attestation, not a validator-proven fact: the validators prove completeness and shape, never that the scan ran or that a citation is true, so each attestation names a verifiable anchor (`ui-design.md` clause, delta ID, or manual basis) a reviewer can check. A screenshot alone never satisfies the layout check.
...
For a browser-extension mission (`capture_mode: browser-extension`), use extension-aware browser automation against the installed extension and its declared contexts. Record `capture_method: browser-extension`; do not substitute a hosted URL or `--base-url` parity capture.

For a native iOS/Android/Flutter/macOS/Windows mission (`capture_mode: native`), capture through the platform's own UI-test tooling instead of a browser, using the device/OS-by-state matrix and the runbook's native UI Evidence columns. Record `capture_method: native-ui-test` or `manual-native`:
...
For a desktop mission (`capture_mode: desktop`), use the platform desktop UI-test tooling or a labeled manual desktop capture. Record `capture_method: desktop-ui-test` or `manual-desktop`; desktop evidence never uses a hosted URL or browser parity helper.
...
- **Target-conformance mode (hosted-browser HTML target):** run `scripts/parity_capture.py --plan <PLAN.md> --reference <approved design-reference HTML> --base-url <app origin> --route-map <route-map.json>` only when every PLAN UI surface has `capture_mode: hosted-browser`. It enumerates the matrix from PLAN `ui_surfaces`, captures the reference render and the implemented page at the same viewport as `-target.png`/`-actual.png` pairs under `docs/goal/evidence/parity/`, runs a DOM geometry probe per app page for the `layout_check` attestation, and writes `manifest.json` plus a self-contained `parity-board.html`. The route map is a small JSON the parent writes once per run: each route's reference selector plus optional `app_eval`/`reference_eval` state triggers; the ready state captures without a trigger, other states are captured only when triggered and otherwise listed as manifest gaps for the reviewer. Judge every pair on the board item by item and record the verdicts as the `target_comparison` rows below. Manual capture remains valid when the CLI is unavailable. Compare layout, element inventory, and observable visual values item by item within `ui-design.md`'s recorded tolerance. The reference render is the comparison source, not an aspiration.
...
Record accepted deviations in the RUN `deviation_ledger`. Every `deviation` difference has one ledger row (`surface_id`, `route`, `breakpoint`, `state`, `difference`, `citation`) citing the `ui-design.md` clause that admits it. A difference that changes regions, states, actions, routes, or responsive behavior is structural drift and routes upstream. Repeated differences cite the frozen design-input delta; accumulated in-tolerance drift never substitutes for a doc update.

Static screenshots never close a motion change. When a mission's diff touches transition, animation, duration, easing, or generated motion assets, its parity evidence adds a motion check row: the authority from `ui-design.md`'s Motion and Media Intent (plus its registered variant or approved target behavior), what the reviewer confirmed in normal and reduced motion, and the verdict. A motion change with no authority citation routes to `ui-design-builder` instead of passing on still images.
...
Every run with at least one PLAN `ui_surfaces` entry follows the Final Visual Parity Loop with one final read-only page-quality review over the delivered design-reference surfaces, bound to the exact integration head. Hosted-browser surfaces use browser and source evidence; extension, native, and desktop surfaces use their platform capture and source evidence. The planned reviewer covers heuristic UX, accessibility, responsive/device behavior, and performance per delivered route in one round. Impeccable is not the default for this node: its pinned workflow has disclosed subagent, browser/server, snapshot-write, and optional-download side effects. It may run only as a separately authorized UI-quality workflow, and its report may be cited as evidence without replacing the independent Harness reviewer.
...
|---|---|---|
| UI Design Intake conformance | Did the result follow the named human owner's approved UI direction? | `ui-design.md`, approved target comparison, implementation review, screenshots |
...
UI owner approval proves only direction conformance. Agent judgment, heuristic review, screenshots, and automated E2E may find problems, but none of them alone is representative-user usability evidence.
...
- Closeout passes `--prd <path to PRD.md>` whenever PLAN freezes a PRD source. The given bytes must equal that row's `content_sha256`; a source-revision-only row is not enough for this file join. A UI-bearing PRD has exactly one matched `ui-surface-contract` boundary pair and exactly one `route`, `states`, `responsive`, and, for schema-5 wireframes, `copy` anchor per `UI-*` entry. The PLAN's surface IDs, routes, states, and breakpoints must equal those anchors. An active initial/enhancement wireframe source requires byte-matching `--wireframes <path to wireframes.html>` and must pass the full checker, including sourced copy, schema-appropriate validation and exact PRD join. Managed maintenance instead freezes its task-record Epic, preserves historical design bytes, and still joins current PRD/PLAN route/state/targets. The transition write path runs the same applicable joins under `--repo-root`.
- Every skipped gate is justified.
- The RUN `deviation_ledger` is complete — machine-checked on runs pinned to harness 0.35.0 or later: every accepted parity deviation is recorded with its allowed-deviation citation, no accepted deviation changes structure, regions, states, actions, routes, or responsive behavior, and any repeated deviation on the same surface cites its frozen design-input delta. Each citation is a recorded attestation (see the attestation caveat in the UI Evidence Gate).
- The RUN `ui_impact_summary` is complete — machine-checked on runs pinned to harness 0.35.0 or later: every mission on a UI run carries its `none`/`style`/`structure`/`both` classification, and every `structure` or `both` row names the accepted upstream design-input delta. The row records the strongest task impact the mission's workers reported (ui-implementation-contract.md rule 8) and, like the ledger, is a recorded attestation.
...
- Harness 0.38 landing is `local_only`: `landing.continuity` is `preserved` at C, `pushed_head_sha` is null, push authorization is false, and no push lifecycle node exists. Explicitly pinned pre-0.38 RUN-v11 files retain their legacy closeout rule for recovery.
- Archival is post-closeout. A must pass archived PLAN/RUN validation, receipt-bound external-anchor verification, exact C-to-A relocation, and the full suite again. If separately authorized, its external push receipt binds that anchor, immutable request and attempt, and fresh A read-back; it never changes the archived RUN.
