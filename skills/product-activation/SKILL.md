...
6. Map every PRD metric and required `TEST-*` expected signal into **Outcome Coverage**, repeating the structured definition/obligation, baseline, target/guardrail, measurement window, and expected-signal values exactly. Define an `MS-*` source for each measurable signal or mark the signal blocked. A source is not verified merely because an SDK, tag, property, or dashboard exists.
