...
const draft = await agent(
  "You are the synthesis role in a PRD org graph. Reconcile the role results into complete Markdown bodies for PRD.md, architecture.md, and stack-decisions.md, plus implementation-plan.md only when requested. PRD.md always records explicit Monetization Infrastructure and Partner Channel gate results; pricing never automatically selects RevenueCat, and affiliate, referral, and reseller remain distinct. Applicable architecture and stack decisions keep store/billing, entitlement, paywall/checkout, merchant-of-record/tax, and partner operations separate. For a UI-bearing product, PRD.md must include the complete UI surface contract, exactly one `responsive` and one `copy` anchor per UI-* entry, and separate Copy Freeze then Wireframe Approval records. Also return wireframes_html_data_json as a valid JSON string using schema wireframes/4 with the same global viewports or sizeClasses, canvasWidths, copyFreeze record, and complete screens, regions, copyStatus values, states, neverDrop lists, and per-target responsiveLayouts required by WIREFRAMES.template.html. Static product copy uses exact text, role, status, and source; dynamic items use a realistic example plus source, order, format, count, length, and fallback; actions carry exact label, status, and source; feedback flows carry structured exact or dynamic feedback copy; every alternate state carries visible or assistive copy. Every visible region action must match exactly one flow with presentation page, overlay, or feedback; page and overlay target another UI-* screen in the file. When the frozen owner answers already decide a media or motion treatment, include mediaIntent with treatment, a dedicated draftPrompt, the owner decision source, and generationStatus deferred; never generate media. The parent embeds the data into the supplied self-contained HTML shell and verifies it against PRD.md. Return the HTML data field as null only when ui_bearing is false. Do not create design-system artifacts or start visual design; the parent presents the copy inventory for Copy Freeze while structural status remains draft, then grades and browser-checks that copy-frozen artifact before later Wireframe Approval. It stops unless the owner explicitly asks to continue. " +
...
const reviewers = [
  ...
  },
  {
    ...
    task: "Check the complete draft package for contradictory scope, unsupported claims, missing states, hidden assumptions, and invalid implementation or usability claims. Verify both monetization and partner-channel gates are explicit; no pricing decision silently selects RevenueCat; affiliate, referral, and reseller are distinct; and provider claims cite current official sources. For UI products, verify wireframes/4 JSON maps every PRD UI-* entry, responsive and copy anchors, state, never-drop region, working action flow, Copy Freeze, exact static and action copy, alternate-state messages, complete dynamic display contracts, deferred media intent, and per-target layout without implementation code. Verify every expected surface has stable development and production targets whose release names match the supplied canonical production name and exact -dev development pair, with no release name reused across surface IDs. Confirm development builds from the exact candidate run branch/ref, production from remote main only after internal exact-SHA PASS and separately authorized fast-forward, and both initial and enhancement runs start from observed remote main. Reject missing surfaces, draft copy in an approved artifact, upload as availability, or invalid native rollback claims.",
  },
];
