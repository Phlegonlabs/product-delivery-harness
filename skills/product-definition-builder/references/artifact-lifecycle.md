...
- Read the existing `PRD.md`, `architecture.md`, `stack-decisions.md`, `market-research.md`, `research-assessment.md`, and every retained `docs/product/outcomes/*.md` record in full before drafting. Read `docs/design/ui-design.md`, its wireframe, and its design-system pair only to identify downstream impact; never carry them into Product Definition staging or rewrite them here. Read an existing `docs/ACTIVATION.md` for operational context, but do not overwrite it.
...
If the core `docs/product/` package or a retained `docs/product/outcomes/*.md` record clearly describes a different, unrelated product, do not carry that content or its trace IDs into the new package.
...
- `ui-design-builder` owns every `docs/design/` artifact, copy completeness, and their separate publication and archival gate. A PRD `` `copy` `` anchor records only product-owned copy responsibility; full HiFi approval may confirm validated schema-5 wireframe copy without a circular PRD rewrite. `revision_requested` or `blocked` PRD copy stops active UI design work. Product Definition only records whether the later design phase is pending or not required.
...
- Never use `docs/product/archived/` as the canonical input or publication location for the current package. Historical PRDs may be read as decision references, with their path/revision and superseded status explicit; they never override the live approved PRD. Keep these links in `docs/DOCUMENTS.md`. Ending a delivery round never archives the live PRD; only an authorized validated replacement supersedes it.
...
- Research, meeting notes, source material, test evidence, and unrelated product documents — unless the unrelated document occupies one of this run's exact final publish paths, per "Handle an Unrelated Document at a Fixed Publish Path" above, in which case it must still be inventoried and archived.
  ...
  `docs/product/outcomes/` contains immutable post-deployment records, not drafting-package files. Each release set gets a dated file; an explicitly continued record must name the exact prior digest and pass `--prior-outcome`, which permits only append-only target/source/measurement history plus one typed `## Verdict History` row derived from the immediately prior authoritative `## Verdict` section. Every prior history row remains byte-identical and ordered; placeholder, duplicate, rewritten, or relocated verdict history fails. A package that ships without a new review leaves prior files published. Multi-target reviews keep ordered per-target rows and one deterministic aggregate verdict; a later target never overwrites earlier evidence.
