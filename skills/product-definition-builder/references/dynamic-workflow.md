...
A workflow result does not authorize file creation, overwrite, archive, or publication. The parent applies the normal staging lifecycle and repairs unresolved findings. For a UI-bearing product, `PRD.md` contains the frozen UI surface contract and its invariant responsive and copy-status anchors, while `wireframes_html_data_json` supplies the matching `wireframes/4` structural and Copy Freeze projection with complete per-target layouts, exact static strings, structured action labels, alternate-state copy, dynamic display contracts, and working action flows. The parent embeds that data into `WIREFRAMES.template.html`, verifies it against `PRD.md`, and presents the copy inventory to the copy owner first. After revisions, it records `### Copy Freeze` and passes `--require-filled --require-copy-approved` while structural status remains draft. Only then does the parent grade and exercise every page-target-state combination, action, reviewer-inspector view, and runtime layout QA in a real browser before the later human Wireframe Approval and final `--require-approved` check. The checker retains `wireframes/2` and `wireframes/3` read compatibility for unchanged historical files. Copy Freeze followed by structural approval completes this phase. The parent runs `ui-design-pass.md`, Taste, `design-system-compiler`, or Harness only after the owner explicitly requests a later visual-design or implementation phase. Research findings are applied by the parent, not the role: a finding that would widen product scope goes back to the user as a recommendation. Only after the complete requested package passes the output checklist does the parent present exact mutations for approval.
