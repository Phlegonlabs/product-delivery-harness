...
For a UI-bearing product, include the complete UI surface contract in `PRD.md`, project it into one interactive `wireframes.html` containing every page and its implementation-bound copy, obtain Copy Freeze approval first, then grade and approve structure, and stop. Run `ui-design-pass.md`, Taste, UI preview generation, `design-system-compiler`, or Harness only after a separate explicit request to continue into visual design or implementation. A product with no UI surface records the skip in `PRD.md` and publishes no placeholder wireframe or design artifacts.

This contract covers the product spec, approved structural screen structure and behavior, exact static and action copy, alternate-state messages, and dynamic display contracts. It does not require a visual direction, preview, token set, closed component variants, or design-system rules. Those belong to a later optional visual-design phase.
...
It specifies what each UI surface must show, say, and do and shows its structure and Copy Freeze in a self-contained HTML reviewer studio. Do not turn `wireframes.html` into a design-reference mockup or add token schemas, component catalogs, or product styling rules here.

`PRD.md` is the canonical source for product scope, routes, screen purpose, visible-region responsibility, content, actions, states, responsive behavior, copy status, flows, trace IDs, and the Builder UX Direction Decision. `wireframes.html` is the only structural and Copy Freeze review projection and never overrides `PRD.md`. If a later visual-design phase is explicitly requested, its approved handoff and optional design-system pair add visual implementation authority without taking ownership of product structure, behavior, or wording.
...
| `wireframes.html` | The human owners reviewing the full UI map and product copy | How all pages, labeled sections, viewport arrangements, states, exact static copy, and dynamic content contracts fit together in one interactive file |
...
- `copy`: [draft / approved / revision_requested / blocked] — [Static strings are implementation-bound in `wireframes.html`; dynamic values follow its approved source/order/format/count/length/fallback contracts]
...
Decision: [approved / revision_requested / blocked], decided by [human copy owner] on [YYYY-MM-DD]

Primary locale: [BCP 47 tag]

Approved copy scope: [Every UI-* static string, action label, feedback message, alternate-state visible or assistive message, and dynamic source/order/format/count/length/fallback contract]

Copy checker: [PASS from `check_wireframe_html.py --require-filled --require-copy-approved` while structural `approvalStatus` remained `draft`]

Copy authority: [Approved static wireframe strings are reused verbatim by UI Design Pass and implementation; dynamic examples are non-binding and their approved display contracts are binding. Any later wording change is a copy delta that updates PRD.md and wireframes.html and repeats this gate.]

### Wireframe Approval

Artifact: [the same copy-frozen wireframes.html path and SHA-256]

Decision: [approved / revision_requested / blocked], decided by [human product/design owner] on [YYYY-MM-DD]
...
Frozen copy basis: [Approved `### Copy Freeze` decision, owner, locale, date, wireframes.html SHA-256, and checker result; no structural-review wording changes]

Wireframe references consulted: [None — reason the Reference Pass was skipped / stable WREF-* entries, one per source: direct URL, publisher, retrieval date, exact screen or flow, viewport and state, observed/partial/blocked status, structural mechanic, Adopt/Adapt/Avoid decision, product-fit reason, and limitation]

WREF-001
- Source: [Publisher — direct URL, retrieved YYYY-MM-DD]
- Scope: [Exact screen or flow; viewport; state]
- Evidence: [observed / partial / blocked] — [structural mechanic directly visible]
- Decision: [Adopt / Adapt / Avoid] — [product-fit reason]
- Limitation: [What this source cannot establish]
...
Direction checkpoint: [approved / selected from alternatives / revision_requested / compare alternatives, decided by owner after reviewing the direction thesis, complementary inspected reference set, first-viewport or primary-workspace composition, type and density intent, interaction signature, and explicit avoid list]
...
Copy basis: [Approved Copy Freeze owner, locale, date, wireframes.html path and SHA-256; no design-stage rewrite]
...
6. exact implementation-bound static and feedback copy, approved dynamic display contracts with realistic examples, working PRD actions, and complete alternate-state copy;
7. a separate reviewer-only inspector for purpose, priority, traces, responsive rules, sources, and media intent, plus a complete copy inventory;
8. runtime region-overlap and horizontal-overflow QA for the selected matrix entry; and
9. Copy Freeze and structural statuses that agree with the separate, ordered `PRD.md` approval records.

The embedded page data must agree exactly with `PRD.md`; `PRD.md` wins on conflict. New and structurally revised files use `wireframes/4`; unchanged historical `wireframes/2` and `wireframes/3` remain readable. Every visible region action maps to exactly one `page`, `overlay`, or `feedback` flow and works locally without a network request. Schema 4 carries top-level `copyFreeze`, per-screen `copyStatus`, structured static and dynamic copy items, structured action labels, and alternate-state copy. The data block may also project optional deferred `mediaIntent` and `UX-*` traces described in `wireframe-guide.md`; they inherit the same agreement rule. Use inline CSS and JavaScript only for the structural review studio. The canvas may use neutral typography, spacing, and content silhouettes to make hierarchy readable, but it includes no product brand styling, design-reference direction, generated imagery, final animation, production component, or design-system token decision. Human Copy Freeze approval fixes the words first; the same copy-frozen artifact then passes layout review and human structural approval to complete the wireframe phase. Do not start visual design or implementation unless separately requested.
...
- Each artifact opens with human-readable content and keeps ID matrices and decision records at the end: `PRD.md` opens with `## At a Glance`, and for a UI-bearing product closes with `## Builder UX Direction Decision`, including `### Copy Freeze` before `### Wireframe Approval` and the optional `### UI Design Handoff` only when visual design was requested; `architecture.md` opens with `## Architecture Summary` and closes with `## Architecture Trace Index`; `stack-decisions.md` closes with `## Options And Open Decisions`.
...
- For a UI-bearing complete package, `### Copy Freeze` appears before `### Wireframe Approval`, records a passing `--require-copy-approved` check while structural status was draft, and is already approved when browser grading and structural approval begin.
...
- For a UI-bearing product, `wireframes.html` maps every `UI-*` entry exactly once in one self-contained `wireframes/4` file with working overview, page, responsive-target, state, section-label, copy-inventory, reviewer-inspector, runtime layout-QA, and PRD action controls; the checker keeps `wireframes/2` and `wireframes/3` read compatibility for unchanged historical files. Every visible region action in schema 4 maps to exactly one local `page`, `overlay`, or `feedback` flow. Every PRD entry has exactly one `` `responsive` `` and one `` `copy` `` anchor; both match the HTML. The responsive set contains at least three ascending web viewports or at least two native/desktop size classes, and every screen supplies complete per-target order, visibility, columns, spans, reflow, and interaction rules. Static strings and action labels are exact implementation-bound copy; dynamic items carry realistic examples plus approved source/order/format/count/length/fallback contracts; every alternate state carries visible or assistive copy. The top-level Copy Freeze names its human owner, primary locale, approval date, and approved status, and every screen and copy item agrees. Optional `mediaIntent` entries are validated deferred MCP-generation handoffs, not generated output. The file contains no product brand styling, design-reference direction, or implementation code, agrees with `PRD.md`, passes `wireframe-guide.md`'s quality check and `check_wireframe_html.py --require-filled --require-approved`, and has human structural and copy approvals recorded in `PRD.md` before the wireframe phase completes. The approval record includes the real-browser matrix result and the PRD-bound diagnostic wave, consolidated ledger, and bounded repair/re-review outcome, or the exact capability-unavailable skip when the host cannot run that stage. No unintended overlap, clipping, occlusion, or horizontal overflow exists at any UI surface, responsive target, or non-`n/a` state, and every intentional overlay names its stacking, focus, and dismissal rule. `PRD.md` also records the stable `WREF-*` set or the reason the Reference Pass was skipped.
- The wireframe gate history shows `--require-copy-approved` passing before UI grading and `--require-approved` passing only after the full browser matrix and human structural decision.
- An enhancement package records its UI-impact classification — `none`, `structure`, `style`, or `both`. When the impact is `structure` or `both`, the refreshed `### Copy Freeze` and `### Wireframe Approval` records cover the changed `UI-*` scope. When it is `style` or `both`, the package carries the owner's recorded decision to re-run the UI Design Pass or keep the existing direction; a style-impacting enhancement with an unchanged handoff or design-system pair and no recorded owner decision does not validate.
...
- In enhancement mode, unaffected sections, `PRD-*`, `ARCH-*`, `UI-*`, `UX-*`, `TEST-*`, `MR-*`, `RA-*`, `WREF-*` IDs, Copy Freeze entries, and stable release target IDs from the prior package were carried forward unchanged rather than regenerated, and the diff is scoped to what the new discovery actually added, changed, or removed; new TEST IDs cover only obligations that were previously uncovered, and new release target IDs cover only destinations that were previously uncovered.
