...
This rubric defines the PRD-bound diagnostic and grading stage that runs after the first `wireframes.html` or design-reference HTML draft and before its human UI gate. It supplements the browser matrix check and static checker; it never replaces either one or the human approval decision. Correctness comes from explicit hard-gate evidence. Numeric scores summarize comparative visual quality and never create another repair round by themselves.
