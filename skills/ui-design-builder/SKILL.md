...
8. Run the **Impeccable Quality Review and PRD-bound Grading Gate**. Apply `impeccable critique` and `impeccable audit` to the exact candidate under its own authorization, then score `H1`–`H9`. The HiFi surface receipt uses the closed `sandboxed-offline-browser` method and retains its console/network/navigation transcript; any request, navigation, popup, form attempt, or console error blocks approval. Combine findings once, let `frontend-design` own one repair batch, then re-review the new SHA-256.
