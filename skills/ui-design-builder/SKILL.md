...
4. Use `frontend-design` as the frontend-authoring resource and `references/wireframe-guide.md` to create one grayscale, self-contained `wireframes.html` for the complete `UI-* × responsive target × non-n/a state` matrix. New or structurally revised packages use `wireframes/4`, including exact static, action, feedback, and alternate-state copy plus bounded dynamic display contracts. A hybrid wireframe uses `responsiveBySurface` and preserves each PRD surface's release, class, capture, and responsive identity instead of inventing one global platform. Typed media regions appear only as labeled placeholders; no final image, animation, palette, typography system, or token set is produced.
5. Run the **Copy Freeze Gate** before UI grading or structural approval. The human copy owner approves the complete schema-4 copy inventory, primary locale, and dynamic source/order/format/count/length/fallback contracts while structural status remains draft. Then run the real-browser matrix, the static checker, and the `W1`–`W5` scope in `references/ui-grading-rubric.md`. The wireframe is ready for the later human structural gate only when Copy Freeze remains approved, its overall score is at least 80, every dimension is at least 60, and no block remains. Obtain explicit human Wireframe Approval. A wording, behavior, responsibility, or structural correction changes the affected PRD contract first and reopens Product Definition Approval.
...
8. Run the **Impeccable Quality Review and PRD-bound Grading Gate**. Apply `impeccable critique` and `impeccable audit` to the exact candidate under its own authorization, then score `H1`–`H9`. The HiFi surface receipt uses the closed `sandboxed-offline-browser` method and retains its console/network/navigation transcript; schema-2 ui-output/2 evidence must cover real product-control click and keyboard destinations at every target; any external request, undeclared navigation, popup, form attempt, or console error blocks approval. Combine findings once, let `frontend-design` own one repair batch, then re-review the new SHA-256.
9. Present only a passing schema-2 candidate for **Human Visual Approval**. Schema-1 references are inspection-only and cannot pass publication or compiler preflight. Readiness requires an overall score of at least 90, `H2`, `H4`, and `H8` each at least 90, `H5`, `H7`, and `H9` each at least 80, every dimension at least 60, and no block or disputed dimension. Record the selected `VD-*` direction, target hash, exact scope, tolerances, evidence, advisories, and owner decision in `ui-design.md`.
