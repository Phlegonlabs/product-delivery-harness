...
4. Use `frontend-design` as the frontend-authoring resource and `references/wireframe-guide.md` to create one grayscale, self-contained `wireframes.html` for the complete `UI-* × responsive target × non-n/a state` matrix. New or structurally revised packages use `wireframes/4`, including exact static, action, feedback, and alternate-state copy plus bounded dynamic display contracts. Typed media regions appear only as labeled placeholders; no final image, animation, palette, typography system, or token set is produced.
