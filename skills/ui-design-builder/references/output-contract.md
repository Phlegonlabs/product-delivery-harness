...
The CSP denies remote media, forms, frames, objects, base navigation, and connections while allowing inline CSS/script and embedded image/font/media. The browser must separately enforce the exact local-page allowlist; same-origin CSP alone is insufficient. This check does not apply the wireframe schema or canonical reviewer shell. Untouched `ui-hifi/1` single-file references keep the prior policy with `navigate-to 'none'` and `ui-output/1`; new or revised HiFi uses schema 2.
