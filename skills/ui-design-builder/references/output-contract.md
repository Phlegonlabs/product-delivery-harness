...
- Product menus and tabs are declared in product DOM. A menu button carries `data-product-menu`, `aria-controls`, and `aria-expanded`; its target panel id must exist. A tab carries `data-product-tab`, `aria-controls`, `aria-selected`, and targets a real panel. Runtime and receipts must cover click, Enter/Space, Arrow/Home/End where applicable, selected/expanded state, panel visibility, Escape and focus return. Menu-open state must survive width-only resize and review-canvas geometry changes. Reviewer links and panels never count as these product controls.
