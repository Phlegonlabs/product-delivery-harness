...
For bundles containing this shell, the existing `ui-output/2` offline transcript adds exactly one `reviewer` object. It contains `defaultSurface`, `defaultVisible: true`, `overviewInitiallyHidden: true`, `overview`, `views`, `navigation` and `specimens`. The `hifi_reviewer.py` contract derives the required matrix from current DOM and manifest bytes. `views` covers Overview and Design Tokens at every target with click/keyboard, visibility, focus and PASS. Reviewer `navigation` covers every page-to-page sidebar link at each target, with the same observations. `overview` matches surface/route/state/target coverage. Every specimen records its exact DOM identity plus nonempty equal `sourceValue`, `specimenValue` and `displayValue`, measured in the browser for that variant/state. Restore the default product view after review. Product `interactions` and their navigation transcript remain separate and mandatory.
