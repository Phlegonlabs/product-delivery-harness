...
The read-only summary derives HEAD, current changes, record identity, existing active-task files, unresolved checkboxes and next work. Preserved page changes are findings. Inspect product DOM/style and rendered before/after evidence too: file hashes cannot distinguish an intentional shell migration from an unintended product redesign. A new hash never proves semantic equivalence.
