...
This rubric defines the Product-Definition-bound diagnostic and grading stage that runs after the first `wireframes.html` or HiFi design-reference HTML draft and before its human UI gate. It supplements the browser matrix check, static checker, and required Impeccable HiFi review; it never replaces them or the human approval decision. Numeric scores summarize comparative quality and never create another repair round by themselves.

## Frozen Grading Basis

Before dispatch, the parent records the exact PRD and `ui-design.md` paths plus revisions or SHA-256 values and the candidate HTML path plus SHA-256. Every grader receives the same frozen inputs: the `UI-*` surfaces, `UX-*` traces, actions, flows, states, responsive targets, UI Design Intake, Motion and Media Intent, approved copy or display contracts, selected direction when applicable, and the candidate HTML. Graders score only conformance to those inputs. They do not add screens, flows, visual preferences, media, or motion.
...
- `H1 Product and direction conformance`: every included surface follows the frozen PRD, `ui-design.md`, approved wireframe, and selected `VD-*` direction. Login, registration, recovery, and authentication-error previews are excluded only when the UI design contract records them as `n/a` for this visual pass.
...
- The parent preserves every Impeccable finding, rubric `block`, and grader disagreement. A design-reference candidate is below threshold when its overall score is below 90, `H2`, `H4`, or `H8` is below 90, any dimension is below 60, any `block` remains, or any dimension is `disputed`.
