...
if (!nodes.some(n=>n.dataset.token==="--wf-body" && n.style.fontSize==="19px")) throw Error("type token has no applied specimen");
const declaredTokens = new Set([...html.matchAll(/--wf-([a-z-]+):/g)].map(m=>"--wf-"+m[1]));
const sampledTokens = new Set(nodes.map(n=>n.dataset.token).filter(Boolean));
if (declaredTokens.size!==sampledTokens.size || [...declaredTokens].some(t=>!sampledTokens.has(t))) throw Error("declared prototype token has no specimen");
